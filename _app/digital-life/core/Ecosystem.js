...
class Ecosystem {
    ...
    createPlanet(digit, holeX, holeY) {
        ...
        if (this.planets.length >= this.config.maxPlanets) {
            console.log(`[globe] Planet limit reached (${this.config.maxPlanets}). Recycling oldest planet...`);
    ...
    handlePlanetCollapse(planet) {
        console.log(`[door] ${planet.type.name} collapsing into mini black hole at (${Math.round(planet.x)}, ${Math.round(planet.y)})`);
    ...
    recycleOldestPlanet() {
        ...
        console.log(`[refresh] Recycling ${oldestPlanet.type.name} (age: ${Math.round(oldestAge / 1000)}s)`);

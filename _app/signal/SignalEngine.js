...
const SignalEngine = (() => {
    ...
    /** Build the briefing disclosure on a view page.
     *
     * Explains what a Curated View is (vs a section), how the predicate
     * works, where the projects came from, and that progress here is the
     * same record as elsewhere in Signal — no separate progress per view.
     *
     * Receives the view object and the raw (un-href-rewritten) project
     * list so we can summarize which source sections contributed.
     */
    function _buildViewBriefing(view, projects) {
        const predicate = view.predicate || {};
        // Human-readable predicate summary
        const predicateBits = [];
        if (predicate.anyOfSkills && predicate.anyOfSkills.length) {
            predicateBits.push('any of these skills: <code>' +
                predicate.anyOfSkills.map(s => s.replace(/[<&>]/g, '')).join('</code>, <code>') +
                '</code>');
        }
        if (predicate.allOfSkills && predicate.allOfSkills.length) {
            predicateBits.push('all of these skills: <code>' +
                predicate.allOfSkills.join('</code>, <code>') +
                '</code>');
        }
        if (predicate.platforms && predicate.platforms.length) {
            predicateBits.push('platforms: <code>' +
                predicate.platforms.join('</code>, <code>') +
                '</code>');
        }
        if (predicate.difficulties && predicate.difficulties.length) {
            predicateBits.push('difficulty: <code>' +
                predicate.difficulties.join('</code>, <code>') +
                '</code>');
        }
        const predicateText = predicateBits.length
            ? predicateBits.join(' AND ')
            : '(no predicate declared — empty view)';

        // Source-section distribution
        const sectionCounts = {};
        (projects || []).forEach(p => {
            const sid = p._sourceSection || '(unknown)';
            sectionCounts[sid] = (sectionCounts[sid] || 0) + 1;
        });
        const distribution = Object.entries(sectionCounts)
            .sort((a, b) => b[1] - a[1])
            .map(([sid, n]) => '<code>' + sid + '</code> &times; ' + n)
            .join(', ');

        const wrap = document.createElement('details');
        wrap.className = 'se-briefing';
        wrap.innerHTML = `
            <summary class="se-briefing-summary">What is this view? Where did these projects come from?</summary>
            <div class="se-briefing-body">

                <h3>What is a Curated View?</h3>
                <p>A view is a <strong>cross-section index</strong>. It is not a hand-built section of its own; it is auto-generated by filtering every project on the Signal hub against a predicate. New projects that match the predicate appear here automatically — no editing required.</p>

                <h3>This view's predicate</h3>
                <p>${predicateText}</p>

                <h3>Source sections</h3>
                <p>The ${projects ? projects.length : 0} projects shown come from: ${distribution || '(none)'}.</p>
                <p>Click any project to jump to its original section page. The "back" link there takes you to that section's normal landing, not back to this view.</p>

                <h3>Progress tracking</h3>
                <p>Marking a project complete here is the same as marking it complete on its source section page — there is one canonical completion record per project. If you are signed in, your completions sync across devices via Firestore.</p>

                <h3>Other views</h3>
                <p>The full list of views lives on the Signal hub landing page (the <strong>Curated Views</strong> panel). Use the <em>The Signal</em> link in the header to return there.</p>
    ...
    function renderView(viewId) {
        ...
        // View briefing — explains "what is a Curated View" + provenance.
        // Auto-injects when this is a synthesized view, not a normal section.
        main.appendChild(_buildViewBriefing(view, rawProjects));

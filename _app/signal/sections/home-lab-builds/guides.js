...
window.SignalGuides = {
    ...
    // ========================================================================
    'sg-43': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. This one is the base setup (headless SSH/OS) &mdash; do it once on whatever Linux host you pick and reuse it for the rest of the track.' },
    ...
    'sg-44': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. One caveat: Pi-hole only blocks ads once devices actually use it for <strong>DNS</strong>, so point a device (or your router&#39;s DNS) at your host&#39;s IP to see it work.' },
    ...
    'sg-45': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. PXE serves boot images over your <strong>LAN</strong>, so the host and the machine you PXE-boot must be on the same network segment (a second VM on a host-only network is a clean way to test it).' },
    ...
    'sg-46': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. Samba shares are reachable from any machine on the network &mdash; mount the share from your laptop to verify.' },
    ...
    'sg-47': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. A VPN gateway needs a reachable <strong>endpoint</strong>: a cloud instance with a public IP is actually easier than a home Pi behind NAT for this one.' },
    ...
    'sg-48': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. Jellyfin is just a container/service &mdash; point a browser at the host&#39;s IP:8096 to stream.' },
    ...
    'sg-49': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. Grafana + its data source run as containers; the dashboard is served over HTTP from the host.' },
        ...
        steps: [
            ...
            },
            {
                ...
                content: '<p>Log into Grafana at <code>http://PI_IP:3000</code>. The first thing to do is add Prometheus as a data source, then import a pre-built dashboard for the Node Exporter metrics.</p>' +
                         '<ol>' +
                         '<li>In the left sidebar, look for <strong>Connections</strong> (newer Grafana) or the <strong>gear icon</strong> (older versions). Click it, then <strong>Data Sources</strong> &gt; <strong>Add data source</strong>. If you do not see the gear icon, the sidebar may be collapsed &mdash; click the hamburger menu (three lines) at the top-left to expand it.</li>' +
                         ...
                         '</ol>' +
                         '<p><strong>IMPORTANT &mdash; Network access:</strong> You must access Grafana from a browser. If your Pi and laptop are on <strong>campus WiFi</strong>, client isolation will block device-to-device traffic. Your options: (1) use a phone hotspot and connect both devices, (2) open a browser directly on the Pi at <code>http://localhost:3000</code>, or (3) SSH tunnel: <code>ssh -L 3000:localhost:3000 pi@PI_IP</code> then open <code>http://localhost:3000</code> on your laptop. Run <code>ip -4 addr show wlan0</code> to find your Pi\'s real IP &mdash; ignore any 172.17.x.x addresses (those are Docker/container networks).</p>' +
                         '<p>Now import the community Node Exporter dashboard:</p>' +
                         '<ol start="5">' +
                         '<li>Go to <strong>Dashboards</strong> (four-squares icon in sidebar) &gt; <strong>New</strong> &gt; <strong>Import</strong></li>' +
        ...
        troubleshooting: '<ul>' +
                         '<li><strong>Cannot access Grafana (http://PI_IP:3000) from laptop but localhost works on the Pi:</strong> This is almost always a <strong>network isolation</strong> issue, especially on campus WiFi. Most campus/enterprise networks enable client isolation (AP isolation) which blocks device-to-device traffic. Fixes: (1) Connect both devices to a phone hotspot instead, (2) open a browser on the Pi itself at <code>http://localhost:3000</code>, (3) use an SSH tunnel: <code>ssh -L 3000:localhost:3000 pi@PI_IP</code> then browse to <code>http://localhost:3000</code> on your laptop. Also verify you are using the correct IP: run <code>ip -4 addr show wlan0</code> and use the <code>192.168.x.x</code> address. Ignore <code>172.17.x.x</code> addresses &mdash; those are Docker/container virtual networks, not your real LAN IP.</li>' +
                         '<li><strong>Pi feels very slow even though CPU usage is low:</strong> Check swap/zRAM usage: <code>swapon --show</code> and <code>free -h</code>. If you see <code>/dev/zram0</code> at 80-100% usage, your Pi is memory-constrained and compressing RAM. Fixes: (1) Increase Prometheus scrape interval to 30s or 60s in <code>/etc/prometheus/prometheus.yml</code>, (2) disable the GUI desktop: <code>sudo systemctl set-default multi-user.target</code> then reboot (frees 200-300MB RAM), (3) add a real swapfile: <code>sudo fallocate -l 1G /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile</code>, (4) reduce swappiness: add <code>vm.swappiness=10</code> to <code>/etc/sysctl.conf</code> then run <code>sudo sysctl -p</code>. For best results on a 1-2GB Pi, use <strong>Raspberry Pi OS Lite</strong> (no desktop) instead of the Desktop edition.</li>' +
                         '<li><strong>apt install errors (apt-transport-https or software-properties-common not found):</strong> On Raspberry Pi OS Bookworm (Debian 12+), <code>apt-transport-https</code> is built into apt natively and <code>software-properties-common</code> may not exist. You only need <code>wget</code> and <code>gpg</code> for adding third-party repos. These packages are pre-installed on most Pi OS images.</li>' +
                         '<li><strong>Grafana sidebar does not show a gear icon for Configuration:</strong> Newer Grafana versions (10+) moved the menu. Look for <strong>Connections</strong> in the left sidebar, or click the hamburger menu (three horizontal lines) at the top-left if the sidebar is collapsed. Data Sources is under Connections &gt; Data Sources.</li>' +
                         ...
                         '<li><strong>Grafana dashboard shows "No data" on all panels:</strong> The Prometheus data source is misconfigured. Go to Grafana &gt; Connections &gt; Data Sources &gt; Prometheus. The URL should be <code>http://localhost:9090</code>. Click "Save &amp; Test" and verify it says "Data source is working". If Grafana and Prometheus are on different machines, use the actual IP instead of localhost.</li>' +
                         '<li><strong>Prometheus uses too much RAM or disk:</strong> Prometheus stores 30 days of metrics by default. On a Pi with limited RAM, this can push the system into zRAM/swap. Reduce retention: change <code>--storage.tsdb.retention.time=7d</code> in the Prometheus systemd service file. Also reduce scrape frequency from 15s to 30s or 60s for non-critical targets.</li>' +
                         ...
                         '<li><strong>Raspberry Pi Imager asks for an auth token:</strong> Skip it. The auth token is only needed for Raspberry Pi Connect (cloud remote access). For lab use, leave it blank and continue. Just configure hostname, WiFi, SSH, and a username/password in the Advanced Options.</li>' +
                         '</ul>',
    ...
    'sg-50': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'A Docker Swarm <strong>cluster</strong> wants multiple nodes, but they don&#39;t have to be Pis: spin up 3 small Linux VMs (or cloud instances) on one network and treat them as the cluster. No Pi hardware required.' },
    ...
    'sg-51': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. Nginx reverse-proxying is pure config &mdash; run it in a container/VM and proxy to any backend you like.' },
    ...
    'sg-52': {
        // Wokwi wave 3: N/A (full Linux server build, not a microcontroller) — no special hardware needed; any Linux VM/host works.
        simulator: { available: false, label: 'No Hardware to Buy — Run It on Any Linux', note: 'You do <strong>not</strong> need a Raspberry Pi for this &mdash; it is a Linux software build, so a spare PC, a VM on your laptop (VirtualBox/UTM), or a cheap cloud instance works just as well. The backup station is scripts + cron + storage; a VM with a second virtual disk exercises the whole flow.' },

...
**Module slug:** `fundamentals-generative-ai`
**Source:** https://learn.microsoft.com/en-us/training/modules/fundamentals-generative-ai/
**Units:** 7
...
3. Prompts
4. AI agents
5. Exercise - Explore generative AI agent scenarios
6. Module assessment
7. Summary
...
Usually, the system prompt is set by the application that uses the model. User prompts can be entered by a human user in a chat application; or in some cases generated by the application on the userâs behalf. The model responds to user prompts while obeying the overall guidance in the system prompt.

## Conversation history

To keep a conversation consistent and relevant, generative AI apps often keep track of the conversation history; and include summarized versions of it in subsequent prompts. This ensures thereâs an ongoing context for the conversation that the model can build on.
For example, suppose the model responds to the system and user prompts described previously with the following completion:

> Key considerations for adopting Generative AI include: Establish Clear Governance and Responsible AI Policies Prioritize High-Value Use Cases Across Corporate Functions Mitigate Risks Around Privacy, Security, and Compliance Foster Organizational Readiness and Cultural Adaptation Measure Impact and Build Accountability Stay Ahead of Strategic and Competitive Shifts
...
- Be clear and specific â prompts with explicit instructions or questions work better than vague language.
...
Using well-designed prompts can make a huge difference to the results youâll get from your generative AI model.
...
Note
See the Text and images tab for more details!
Imagine having a digital assistant that doesnât just answer questions, but actually gets things done! Welcome to the world of AI agents.
...
- Instructions : A system prompt that defines the agentâs role and behavior. Think of it as the agentâs job description.
...
Agents can also work with one another, in multi-agent systems. Instead of one agent doing everything, multiple agents can collaborateâeach with its own specialty. One might gather data, another might analyze it, and a third might take action. Together, they form an AI-powered workforce that can handle complex workflows, just like a human team.
...
- Agent Factory: The new era of agentic AIâcommon use cases and design patterns

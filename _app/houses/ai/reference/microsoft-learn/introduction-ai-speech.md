...
**Module slug:** `introduction-ai-speech`
**Source:** https://learn.microsoft.com/en-us/training/modules/introduction-ai-speech/
**Units:** 7

## Table of Contents

1. Introduction
2. Speech-enabled solutions
3. Speech recognition
4. Speech synthesis
5. Exercise - Explore an AI speech scenario
6. Module assessment
7. Summary
...
Speech recognition begins when a microphone converts sound waves into a digital signal. The system samples the analog audio thousands of times per secondâtypically 16,000 samples per second (16 kHz) for speech applicationsâand stores each measurement as a numeric value.
...
- Divide audio into frames: Split the signal into overlapping 20â30 millisecond windows.
- Apply Fourier transform: Convert each frame from time domain to frequency domain, revealing which pitches are present.
- Map to Mel scale: Adjust frequency bins to match human hearing sensitivityâwe distinguish low pitches better than high ones.
- Extract coefficients: Compute a small set of numbers (often 13 coefficients) that summarize the spectral shape of each frame.
The result is a sequence of feature vectorsâone per frameâthat captures what the audio sounds like without storing every sample. These vectors become the input for acoustic modeling.
...
Acoustic models learn the relationship between audio features and phonemes âthe smallest units of sound that distinguish words. English uses about 44 phonemes; for example, the word "cat" comprises three phonemes: /k/, /Ã¦/, and /t/.
...
The output of acoustic modeling is a probability distribution over phonemes for each audio frame. For instance, frame 42 might show 80% confidence for /Ã¦/, 15% for /É/, and 5% for other phonemes.
...
By separating concerns, modern speech recognition systems achieve high accuracy across languages, accents, and acoustic conditions. When transcription quality falls short, you can often trace the issue to one stageâpoor audio capture, insufficient language model training, or overly aggressive post-processingâand adjust accordingly.
...
Note
See the Text and images tab for more details!
Speech synthesisâalso called text-to-speech (TTS)âconverts written text into spoken audio. You encounter speech synthesis when virtual assistants read notifications, navigation apps announce directions, or accessibility tools help users consume written content audibly.
...
For example:
- The word "though" converts to /Î¸oÊ/
- The word "through" converts to /Î¸ruË/
- The word "cough" converts to /kÉËf/
...
When determining phonemes, linguistic analysis often uses a transformer model to help consider context . For example, the word " read " is pronounced differently in "I read books" (present tense: /riËd/) versus "I read that book yesterday" (past tense: /rÉd/).
...
The prosody predictions create a target specification: "Produce the phoneme /Ã¦/ at 180 Hz for 80 milliseconds with moderate intensity, then pause for 200 milliseconds."
Important
Prosody dramatically affects naturalness. Robotic-sounding speech often results from flat, monotone prosodyânot from imperfect phoneme pronunciation.
...
Modern systems use neural vocodersâdeep learning models that generate audio samples directly. Popular vocoder architectures include WaveNet, WaveGlow, and HiFi-GAN.

#### The synthesis process:

- Acoustic feature generation : An acoustic model (often a transformer) converts phonemes and prosody targets into mel-spectrogramsâvisual representations of sound frequencies over time
...
The vocoder essentially performs the inverse of what automatic speech recognition doesâwhile speech recognition converts audio into text, the vocoder converts linguistic representations into audio.
...
- Linguistic analysis converts it to phonemes: /ËdÉktÉr ËtÊÉnz ÉËpÉÉªntmÉnt Éªz Ã¦t Î¸ri ÉËklÉk pi Ém/

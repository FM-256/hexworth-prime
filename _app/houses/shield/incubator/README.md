# Shield Incubator — Graduation Log

Created: 2026-07-31 (Stragglers branch)

## Initial population

| Sub-cluster | Modules | Graduation target |
|---|---:|---|
| `threat-*` | 30 | Eligible — promote to dedicated hub |
| `crypto-*` | 20 | Eligible — promote to dedicated hub |
| `linux-*` | 15 | Eligible — promote to dedicated hub |
| `pbq-*` | 12 | Eligible — promote to dedicated hub |
| `sp-*` | 9 | Need 1 more |
| `pis-*` | 6 | Need 4 more |
| `sy0-*` | 4 | Need 6 more |
| `malware-*` | 3 | Need 7 more |
| `cse-*` | 3 | Need 7 more |
| `hash-*` | 3 | Need 7 more |
| `social-*` | 2 | Need 8 more |
| `osint-*` | 2 | Need 8 more |
| `stego-*` | 2 | Need 8 more |
| `hashing-*` | 2 | Need 8 more |
| `data-*` | 2 | Need 8 more |
| `cryptomatch-*` | 1 | Need 9 more |
| `hangman-*` | 1 | Need 9 more |
| `crime-*` | 1 | Need 9 more |
| `secure-*` | 1 | Need 9 more |
| `cve-*` | 1 | Need 9 more |
| `pentest-*` | 1 | Need 9 more |
| `cookies-*` | 1 | Need 9 more |
| `cyberscramble-*` | 1 | Need 9 more |
| `ethical-*` | 1 | Need 9 more |
| `hatmatch-*` | 1 | Need 9 more |
| `laws-*` | 1 | Need 9 more |
| `presentations-*` | 1 | Need 9 more |
| `security-*` | 1 | Need 9 more |
| `aaa-*` | 1 | Need 9 more |
| `access-*` | 1 | Need 9 more |
| `block-*` | 1 | Need 9 more |
| `checksum-*` | 1 | Need 9 more |
| `diffie-*` | 1 | Need 9 more |
| `digital-*` | 1 | Need 9 more |
| `factor-*` | 1 | Need 9 more |
| `gpg-*` | 1 | Need 9 more |
| `encryption2-*` | 1 | Need 9 more |
| `encrypt-*` | 1 | Need 9 more |
| `xor-*` | 1 | Need 9 more |
| `career-*` | 1 | Need 9 more |
| `ethics-*` | 1 | Need 9 more |
| `physical-*` | 1 | Need 9 more |
| `privacy-*` | 1 | Need 9 more |
| `cube-*` | 1 | Need 9 more |
| `cookie-*` | 1 | Need 9 more |
| `hat-*` | 1 | Need 9 more |
| `scramble-*` | 1 | Need 9 more |
| `eh-*` | 1 | Need 9 more |
| `browser-*` | 1 | Need 9 more |
| `eap-*` | 1 | Need 9 more |
| `nat-*` | 1 | Need 9 more |
| `protocol-*` | 1 | Need 9 more |
| `handshake-*` | 1 | Need 9 more |
| `wireless-*` | 1 | Need 9 more |
| `change-*` | 1 | Need 9 more |
| `scenario-*` | 1 | Need 9 more |
| `pspg-*` | 1 | Need 9 more |
| `crisc-*` | 1 | Need 9 more |
| `buffer-*` | 1 | Need 9 more |
| `ransomware-*` | 1 | Need 9 more |
| `google-*` | 1 | Need 9 more |
| `attack-*` | 1 | Need 9 more |
| `compliance-*` | 1 | Need 9 more |
| `cfr310-*` | 1 | Need 9 more |
| `cyber-*` | 1 | Need 9 more |
| `dont-*` | 1 | Need 9 more |
| `sql-*` | 1 | Need 9 more |
| `incident-*` | 1 | Need 9 more |
| `tor-*` | 1 | Need 9 more |
| `hydra-*` | 1 | Need 9 more |
| `exploit-*` | 1 | Need 9 more |
| `dr-*` | 1 | Need 9 more |
| `life-*` | 1 | Need 9 more |

## Graduation history

_(none yet — append entries when sub-clusters move out)_

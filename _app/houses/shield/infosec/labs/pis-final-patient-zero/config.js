...
const PISFinalConfig = {
    ...
    boot: {
        biosLines: [
            ...
            'Outlook (recovered): accounts@crimson-dawn.net inbox restored',
            ...
            '/verify': {
                ...
                <div style="font-family:'JetBrains Mono', monospace, system-ui; max-width:680px; margin:30px auto; padding:0; background:#0a0a0a; border:3px solid #ff003c; border-radius:6px; box-shadow:0 0 40px rgba(255,0,60,0.45); color:#fff; overflow:hidden;">
                    ...
                    <div style="padding:32px 28px 24px 28px; text-align:center;">
                        ...
                        <button data-action="restart-lab" onclick="(function(){try{Object.keys(localStorage).filter(function(k){return k.indexOf(&quot;hexworth_lab_pis_final&quot;)===0;}).forEach(function(k){localStorage.removeItem(k);});}catch(e){}window.location.reload();})();" style="padding:14px 36px; background:#ff003c; color:#fff; border:none; border-radius:4px; font-weight:900; font-size:1rem; letter-spacing:0.08em; cursor:pointer; font-family:inherit; box-shadow:0 0 16px rgba(255,0,60,0.5);">RESTART INVESTIGATION &rarr;</button>
    ...
    // Pure-JS SHA-256 (synchronous). Used by the sha256sum terminal
    // command when called via pipe to hash stdin. Standard FIPS-180-4
    // implementation — small enough to ship inline; no external dep.
    _sha256Hex: function(message) {
        // UTF-8 encode
        var bytes = [];
        for (var i = 0; i < message.length; i++) {
            ...
            if (c < 0x80) bytes.push(c);
                ...
                i++; var c2 = 0x10000 + (((c & 0x3ff) << 10) | (message.charCodeAt(i) & 0x3ff));
                bytes.push(0xf0 | (c2 >> 18)); bytes.push(0x80 | ((c2 >> 12) & 0x3f)); bytes.push(0x80 | ((c2 >> 6) & 0x3f)); bytes.push(0x80 | (c2 & 0x3f));
            }
        }
        // Pad
        ...
        for (var i = 0; i < 4; i++) bytes.push(0);  // upper 32 bits of length (always 0 for our inputs)
        bytes.push((bitLen >>> 24) & 0xff); bytes.push((bitLen >>> 16) & 0xff);
        bytes.push((bitLen >>> 8) & 0xff); bytes.push(bitLen & 0xff);
        // Initial hash values
        ...
        for (var blockStart = 0; blockStart < bytes.length; blockStart += 64) {
            var W = new Array(64);
            for (var t = 0; t < 16; t++) {
                W[t] = (bytes[blockStart + t*4] << 24) | (bytes[blockStart + t*4 + 1] << 16) |
                       (bytes[blockStart + t*4 + 2] << 8) | bytes[blockStart + t*4 + 3];
                W[t] = W[t] >>> 0;
            }
            for (var t = 16; t < 64; t++) {
    ...
    commands: {
        ...
        'sha256sum': function(args, term, engine) {
            // Stdin handling: when called via pipe (e.g. `echo "x" | sha256sum`)
            // the Terminal stores the previous command's output on
            // term._pipedStdin. Compute the real SHA-256 of that input and
            // return it as the standard sha256sum format: `<hash>  -`.
            // This is what the walkthrough's Phase 7 synthesis command relies on.
            if (term && term._pipedStdin) {
                var input = term._pipedStdin;
                var stdinHash;
                try {
                    // Pure-JS SHA-256 (synchronous). See PISFinalConfig._sha256Hex.
                    stdinHash = PISFinalConfig._sha256Hex(input);
                } catch (e) {
                    return 'sha256sum: unable to compute hash (' + e.message + ')';
                }
                return stdinHash + '  -';
            }

            if (args.length === 0) return 'Usage: sha256sum <file>\nExample: sha256sum /home/ir-lead/downloads/Nakamura-Q1-2026-CORRECTED.docx\n\nFiles in /home/ir-lead/downloads/ will be hashed.\nDownload attachments from webmail first.\n\nAlso supported: stdin via pipe — `echo "string" | sha256sum`';

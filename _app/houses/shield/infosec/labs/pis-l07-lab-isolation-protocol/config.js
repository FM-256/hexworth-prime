...
const PISL07Config = {
    ...
    lore: {
        ...
        goals: [
            "Use VLAN segmentation to break a flat network into isolated broadcast domains",
            "Write ACL rules that block compromised segments from reaching clean labs while preserving admin access",
            "Apply firewall rules that enforce one-way trust: clean → admin OK, compromised → anywhere blocked",
            "Verify isolation works (compromised cannot reach clean) AND operations work (clean still talks to clean and admin)",
            "Practice the speed-vs-correctness tradeoff: contagion is spreading; rushed segmentation breaks operations"
        ],

        toolkit: [
            { name: "network-map", purpose: "Show current network topology, VLAN assignments, lab status", sample: "network-map" },
            { name: "vlan", purpose: "Create or modify VLAN definitions", sample: "vlan create 30 lab-3-quarantine" },
            { name: "acl", purpose: "Add ACL rules to control traffic between VLANs", sample: "acl add deny vlan-30 to vlan-10" },
            { name: "firewall-rule", purpose: "Add edge firewall rules that enforce inter-segment policy", sample: "firewall-rule deny src=vlan-30 dst=vlan-10" },
            { name: "show", purpose: "Inspect current VLAN, ACL, or firewall state", sample: "show acls" },
            { name: "verify-isolation", purpose: "Confirm compromised segments cannot reach clean ones", sample: "verify-isolation" },
            { name: "verify-operations", purpose: "Confirm clean segments retain inter-lab + admin connectivity", sample: "verify-operations" },
    ...
    commands: {
        ...
        'network-map': function(args, term, engine) {
            const vlanCount = Object.keys(engine.config._state.vlans).length;
            const aclCount = engine.config._state.acls.length;
            const status = vlanCount >= 7 ? 'SEGMENTED' : 'FLAT (unsegmented)';

            return 'HEXWORTH CONTAINMENT -- NETWORK TOPOLOGY\n' + '='.repeat(50) + '\n\nCORE SWITCH: CORE-SW-01 (Cisco Catalyst 9300)\n  VLANs configured: ' + vlanCount + '/7\n  Status: ' + status + '\n\nFIREWALL: FW-PERIMETER-01 (Palo Alto PA-3260)\n  ACL rules: ' + aclCount + '\n  Firewall rules: ' + engine.config._state.firewallRules.length + '\n\nLAB SEGMENTS:\n  10.0.1.0/24  Lab 1 -- Pathogen Storage    [CLEAN]       ' + (engine.config._state.vlans[10] ? 'VLAN 10 OK' : 'NO VLAN') + '\n  10.0.2.0/24  Lab 2 -- Analysis Suite      [CLEAN]       ' + (engine.config._state.vlans[20] ? 'VLAN 20 OK' : 'NO VLAN') + '\n  10.0.3.0/24  Lab 3 -- Breach Origin       [COMPROMISED] ' + (engine.config._state.vlans[30] ? 'VLAN 30 OK' : 'NO VLAN') + '\n  10.0.4.0/24  Lab 4 -- Secondary Spread    [COMPROMISED] ' + (engine.config._state.vlans[40] ? 'VLAN 40 OK' : 'NO VLAN') + '\n  10.0.5.0/24  Lab 5 -- Sequencing Lab      [CLEAN]       ' + (engine.config._state.vlans[50] ? 'VLAN 50 OK' : 'NO VLAN') + '\n  10.0.6.0/24  Lab 6 -- Containment R&D     [CLEAN]       ' + (engine.config._state.vlans[60] ? 'VLAN 60 OK' : 'NO VLAN') + '\n  10.0.100.0/24 Admin -- Management          [ADMIN]       ' + (engine.config._state.vlans[100] ? 'VLAN 100 OK' : 'NO VLAN') + '\n\nSee ~/notes.txt for VLAN target scheme and command reference.';
        },
        ...
        'vlan': function(args, term, engine) {
            ...
            engine.config._state.vlans[id] = { name, subnet };

            const count = Object.keys(engine.config._state.vlans).length;
            ...
            if (count >= 7 && engine.config._state.acls.length > 0 && !engine.config._flag1Awarded) {
        ...
        'acl': function(args, term, engine) {
            ...
            engine.config._state.acls.push({ name, action, src, dst });

            let output = `ACL entry added:\n  Name:   ${name}\n  Action: ${action.toUpperCase()}\n  Source: ${src}\n  Dest:   ${dst}\n  Applied to: CORE-SW-01 and FW-PERIMETER-01\n\nTotal ACL entries: ${engine.config._state.acls.length}`;

            // Check flag1: all VLANs created + at least some ACLs present
            if (Object.keys(engine.config._state.vlans).length >= 7 && engine.config._state.acls.length >= 1 && !engine.config._flag1Awarded) {
                engine.config._flag1Awarded = true;
                engine.awardFlag('flag1');
        ...
        'firewall-rule': function(args, term, engine) {
            ...
            engine.config._state.firewallRules.push({ action, src, dst });

            let output = `Firewall rule added:\n  Action: ${action.toUpperCase()}\n  Source: ${src}\n  Dest:   ${dst}\n  Status: Pushed to FW-PERIMETER-01\n\nTotal firewall rules: ${engine.config._state.firewallRules.length}`;
        ...
        'show': function(args, term, engine) {
            ...
            if (sub === 'vlans') {
                const vlans = engine.config._state.vlans;
        ...
        'verify-isolation': function(args, term, engine) {
            ...
            const vlanReady = Object.keys(engine.config._state.vlans).length >= 7;
            const hasQuarantineRules = engine.config._state.acls.some(r =>
            ...
            engine.config._state.isolationVerified = true;
        ...
        'verify-operations': function(args, term, engine) {
            if (!engine.config._state.isolationVerified) {
                return 'OPERATIONS VERIFICATION -- BLOCKED\nMust verify isolation first.\nRun: verify-isolation';
            }

            const hasPermitRules = engine.config._state.acls.some(r => r.action === 'permit') ||
                engine.config._state.firewallRules.some(r => r.action === 'allow') ||
                // If the analyst only denied quarantine subnets, clean-to-clean is implicitly allowed
                engine.config._state.acls.some(r => r.action === 'deny' && (r.src.includes('10.0.3') || r.src.includes('10.0.4')));
            ...
            engine.config._state.operationsVerified = true;

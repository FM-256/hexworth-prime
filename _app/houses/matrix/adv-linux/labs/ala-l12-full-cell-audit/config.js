    ...
    commands: {
        ...
        'ip': function(args, term, engine) {
            ...
            if (sub === 'link' && obj === 'show') {
                const eth1State = engine._eth1Up ? 'UP' : 'DOWN';
                const eth1Flags = engine._eth1Up ? 'BROADCAST,MULTICAST,UP,LOWER_UP' : 'BROADCAST,MULTICAST';
        ...
        'vi': function(args, term, engine) {
            ...
            if (target.includes('00-config.yaml') || target.includes('netplan')) {
                engine._netplanFixed = true;
                engine.filesystem['/'].children.etc.children.netplan.children['00-config.yaml'].content =
                    'network:\n  version: 2\n  renderer: networkd\n  ethernets:\n    eth0:\n      dhcp4: true\n    eth1:\n      addresses:\n        - 10.0.1.100/24\n      nameservers:\n        addresses: [10.0.1.1]\n      routes:\n        - to: default\n          via: 10.0.1.1\n';
            ...
            if (target.includes('resolv.conf')) {
                engine._dnsFixed = true;
                engine.filesystem['/'].children.etc.children['resolv.conf'].content =
            ...
            if (target.includes('named.conf.options') || target.includes('named.conf')) {
                engine._bindSecured = true;
                engine.filesystem['/'].children.etc.children.bind.children['named.conf.options'].content =
            ...
            if (target.includes('/etc/logrotate.d/cell-ops')) {
                engine._logrotateConfigured = true;
        ...
        'resolvectl': function(args, term, engine) {
            ...
            if (sub === 'status') {
                return `Global\n  DNS Servers: ${engine._dnsFixed ? '10.0.1.1' : '1.2.3.4 (unreachable)'}\n  DNSSEC: no\n\nLink 2 (eth0)\n  DNS Servers: 10.0.0.1\n\nLink 3 (eth1)\n  DNS Servers: ${engine._eth1Up ? '10.0.1.1' : '(not configured)'}`;
            }
        ...
        'userdel': function(args, term, engine) {
            ...
            if (user === 'svc-ghost') {
                engine._ghostRemoved = true;
                // Remove from filesystem passwd
                engine.filesystem['/'].children.etc.children.passwd.content =
                    'root:x:0:0:root:/root:/bin/bash\ndaemon:x:1:1:daemon:/usr/sbin:/usr/sbin/nologin\nbind:x:107:113::/var/cache/bind:/usr/sbin/nologin\noperator:x:1000:1000:Grid Operator:/home/operator:/bin/bash\n';
                // Remove svc-ghost crontab
                engine._beaconCronRemoved = true;
                delete engine.filesystem['/'].children.var.children.spool.children.cron.children.crontabs.children['svc-ghost'];
        ...
        'rm': function(args, term, engine) {
            ...
            for (const f of files) {
                if (f.includes('/tmp/beacon.sh') || f === 'beacon.sh') {
                    delete engine.filesystem['/'].children.tmp.children['beacon.sh'];
                    engine._beaconCronRemoved = true;
                }
                if (f.includes('cell-beacon.service')) {
                    engine._beaconServiceRemoved = true;
                    delete engine.filesystem['/'].children.etc.children.systemd.children.system.children['cell-beacon.service'];
                }
            }
            return '';
        },
        ...
        'systemctl': function(args, term, engine) {
            ...
            if (sub === 'status') {
                if (unit === 'cell-beacon') {
                    if (engine._beaconServiceRemoved) {
                ...
                if (unit === 'named' || unit === 'bind9') {
                    return `\u25CF named.service - BIND Domain Name Server\n     Loaded: loaded (/lib/systemd/system/named.service; enabled)\n     Active: active (running)\n\n${engine._bindSecured ? 'recursion no; allow-query restricted. SECURE.' : 'WARNING: open resolver -- recursion yes; allow-query { any; }'}`;
                }

                if (unit === 'grid-sync') {
                    return engine._gridSyncRunning
                ...
                if (unit === 'auditd') {
                    return engine._auditdConfigured
            ...
            if (sub === 'start' || sub === 'restart') {
                if (unit === 'grid-sync') {
                    if (!engine._eth1Up && !engine._netplanFixed) {
            ...
            if (sub === 'list-units') {
                const beaconLine = engine._beaconServiceRemoved
                    ? ''
                    : '\n  cell-beacon.service             loaded   active     running  Cell Beacon Service [ROGUE]';
                const syncLine = engine._gridSyncRunning
        ...
        'logrotate': function(args, term, engine) {
            ...
            if (config.includes('cell-ops') || config === '/etc/logrotate.conf') {
                if (!engine._logrotateConfigured && config.includes('cell-ops')) {
                    return 'error: /etc/logrotate.d/cell-ops not found\nCreate it first, then re-run.';
                }
                if (force || engine._logrotateConfigured) {
                    engine._logrotateConfigured = true;
        ...
        '/opt/verify/check-network.sh': function(args, term, engine) {
            const checks = [
                { label: 'eth1 UP and configured (10.0.1.100/24)',  pass: engine._eth1Up        },
                { label: 'netplan config valid (no typos)',          pass: engine._netplanFixed  },
                { label: 'DNS resolver reachable (not 1.2.3.4)',     pass: engine._dnsFixed      }
            ];
        ...
        '/opt/verify/check-security.sh': function(args, term, engine) {
            const checks = [
                { label: 'sshd: PermitRootLogin no, PasswordAuth no, PermitEmptyPasswords no', pass: engine._sshdHardened       },
                { label: 'svc-ghost account removed (no UID 0 duplicates)',                    pass: engine._ghostRemoved        },
                { label: 'svc-ghost removed from sudoers',                                     pass: engine._ghostSudoRemoved    },
                { label: 'svc-ghost beacon cron removed',                                      pass: engine._beaconCronRemoved   },
                { label: 'UFW enabled with baseline rules',                                    pass: engine._ufwEnabled          }
            ];
            const failed = checks.filter(c => !c.pass);
        ...
        '/opt/verify/check-services.sh': function(args, term, engine) {
            const checks = [
                { label: 'BIND: recursion no, allow-query restricted',     pass: engine._bindSecured          },
                { label: 'cell-beacon.service stopped and disabled',        pass: engine._beaconServiceRemoved },
                { label: 'grid-sync.service active and running',            pass: engine._gridSyncRunning      }
            ];
            const failed = checks.filter(c => !c.pass);
        ...
        '/opt/verify/check-integrity.sh': function(args, term, engine) {
            const checks = [
                { label: 'AIDE installed and baseline initialized',              pass: engine._aideInstalled      },
                { label: 'auditd running with at least 2 watch rules',           pass: engine._auditdConfigured   },
                { label: 'logrotate configured for /var/log/cell-ops/',          pass: engine._logrotateConfigured }
            ];
            const failed = checks.filter(c => !c.pass);
        ...
        'ls': function(args, term, engine) {
            ...
            if (path === '/tmp' || path === '/tmp/') {
                if (!engine.filesystem['/'].children.tmp.children['beacon.sh']) {
                    return '';
                }
                if (hasL) {

...
${engine.config._cronFixed ? '' : 'NOTE: PID 9001/9002 (find + sha256sum) run every 4 minutes via cron as root'}`;
        ...
        'crontab': function(args, term, engine) {
            ...
            if (eFlag) {
                ...
                term.fs['/'].children.var.children.spool.children.cron.children.crontabs.children.root.content =
        ...
        'systemctl': function(args, term, engine) {
            ...
            if (sub === 'cat') {
                if (unit === 'cell-cache') {
                    return term.fs['/'].children.etc.children.systemd.children.system.children['cell-cache.service'].content;
                }
                if (unit === 'cell-stream') {
                    return term.fs['/'].children.etc.children.systemd.children.system.children['cell-stream.service'].content;
                }
            ...
            if (sub === 'edit') {
                if (unit === 'cell-cache') {
                    ...
                    term.fs['/'].children.etc.children.systemd.children.system.children['cell-cache.service'].content =
                        '[Unit]\nDescription=Cell Cache Service\nAfter=network.target\n\n[Service]\nType=simple\nUser=svc-cache\nExecStart=/opt/cell-services/cell-cache\nRestart=on-failure\nRestartSec=10\nMemoryMax=256M\n\n[Install]\nWantedBy=multi-user.target\n';
        ...
        'rm': function(args, term, engine) {
            ...
            if (target.includes('grid-index.sh')) {
                engine.config._cronFixed = true;
                delete term.fs['/'].children.opt.children['cell-services'].children['grid-index.sh'];
                return '';
            }
            return '';
        },

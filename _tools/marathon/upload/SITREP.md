...
**Auto-updated:** 2026-08-11 01:19 EDT | branch: master | dirty files: 109

### Latest commits (auto)
- 2fafab9f1 Server-issued course badges: one that is provable, four that are not
- 84f4e3ff9 The slot pool gets a release path, which it has never had
- 266c02a28 SITREP: consolidate today into one current-state block, Lagrange board clear
- 829a82f70 #309 closed
- 465bb8f63 #309: the boxes are solid now, and two bugs the ticket did not know about
<!-- AUTO:END -->

**Manual section updated:** 2026-08-11 01:40 EDT. #275 slot pool: release path LIVE, badge signal LIVE.

## #275 SANDBOX SLOT POOL. Two of three pieces deployed. Pool 50 slots, 4 bound, 46 free.
   THE TICKET WAS A SYMPTOM. It said "reclaim-idle-slots.py crashes on import". True, and the
   ...
   a lifetime cap on distinct users, students included. QC throwaways were just the fastest
   consumers of a pool that never gave anything back.

   OPERATOR POLICY 2026-08-11: released on course completion; the student tears down their own
   sandbox as part of finishing.

   DEPLOYED 1 — /release on bc2 (claim_service.py, backed up as .bak-release-1786422338).
     Verified live through the real HTTP surface: BAD_SECRET without the secret; a uid holding
     no slot returns NO_SLOT_BOUND (idempotent, completion events can repeat); and a REAL
     student with 1 server + 2 volumes was REFUSED with counts and remained bound. That refusal
     is the safety property the whole design rests on.
     THE EMPTINESS GUARD IS WHY /release can auth on the bridge secret rather than a student
     id_token: a mis-targeted call cannot strand work, worst case an empty-slot student
     re-claims. If the guard is ever loosened, that route needs an id_token like /claim.
     ⚠ UNBINDING FREES NOTHING. Servers and volumes stay and keep counting against FLOOR_MB in
     claim(), so a recycled pool can report free slots while every claim returns CLOUD_FULL.
     Released the 4 genuinely idle bindings by hand: 8 bound -> 4 bound, nothing destroyed.

   DEPLOYED 2 — awardCourseBadge (live v2 callable) + functions/course-badges.js.
     Operator's idea, and better than what I was building. Completion is COUNTABLE from evidence
     the server produced: awardMissionBadge re-grades each Observatory mission against bc1's
     service-key-gated /grade-for and writes users/{uid}/server_awards/{badgeId}, a
     subcollection with no client write rule (CF-only by default-deny).
     ⚠ ONE COURSE, NOT THE FIVE I DRAFTED. server_awards has exactly one writer and one caller,
     the Observatory. OpenStack, Forge, Linux Mastery, Databases and Cell-Sigma have grading but
     ...
     Counts DISTINCT MISSIONS, not documents: a re-award writes the same badgeId with merge, so
     document-counting is right today and wrong the first id reuse. test-course-badges 8/8.

   ⚠ NOT DONE — the release hop. The badge lands and NO SLOT MOVES. Needs a route on bc1 that
   forwards to bc2, because Cloud Functions cannot reach a tailscale address.
   THE PATH EXISTS AND I SAID IT DID NOT: GCP -> bc1 is proven (awardMissionBadge calls
   /grade-for that way) and bc1 -> bc2:9711 verified at HTTP 401 in 8 ms. I had only tested

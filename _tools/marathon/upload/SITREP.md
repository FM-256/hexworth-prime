...
**Auto-updated:** 2026-08-07 20:44 EDT | branch: master | dirty files: 105

### Latest commits (auto)
- 1e93eb671 SITREP: rewrite the cursor to point forward instead of backward
- 80a7448a6 SITREP: pushed, nothing open
...
**Manual section updated:** 2026-08-07 21:15 EDT — TRIAL RUN named. M1 PASSED, M2 built + UNRUN.

## NOW: TRIAL RUN (Career Launchpad, first premium service). M1 passed. M2 built, gate unrun.

  Repo: ~/hexworth-shared/carreer_launchpad/trial-run/   Spec: ../TRIAL_RUN_BUILD_SPEC.md v0.3
  hexworth-prime is UNCHANGED by this work. Its only future artifact here is a static
  card/cartridge in _app/career/.

  Name settled 2026-08-07: **Trial Run** (spec §13.1 has the rejected options and why).
  Durable ids already applied: dir/cartridge/progress-key `trial-run`, env `TRIAL_RUN_*`.

  | milestone | gate | status |
  | M1 skeleton | end-to-end walking | PASSED 28/28 — docs/m1_gate.md |
  | M2 claims   | recall >=0.8, agreement >=0.75 | BUILT, GATE UNRUN — docs/m2_gate.md |
  | M3, M4      | | not started (§12.1 blocks M3 until M2 records a PASS) |

  BLOCKED ON ONE THING: an ANTHROPIC_API_KEY. The M2 gate refuses to score against a stub,
  so it exits 2 rather than print a number that measures nothing.
    ANTHROPIC_API_KEY=sk-... .venv/bin/python tests/test_m2_gate.py

  Runs with no key: `.venv/bin/python tests/test_m1_gate.py`
                    `ALLOW_STUB_LLM=1 .venv/bin/python tests/test_m2_gate.py --self-check`

  DO NOT re-derive the design from this file. trial-run/docs/ has it: decisions.md (every
  choice + what was rejected), m1_gate.md and m2_gate.md (what each gate proves and, more
  usefully, what it does NOT).

  Two superseded dirs remain, each with a SUPERSEDED.md holding its own rm command:
  interview-ops/ (pre-rename) and trial-run-staging/ (debris). Not load-bearing.

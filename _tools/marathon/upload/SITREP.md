...
**Auto-updated:** 2026-08-10 14:06 EDT | branch: master | dirty files: 114
...
- 06d9b9859 Fly, then work the console: the two-phase mission shape
- b9a6c560f The ground segment console becomes a place you can walk into
<!-- AUTO:END -->
...
## MISSION ACTS. Every z1 level now makes you GO AND GET the evidence.
   ...
   ⚠ SHIPS WITH ONE STAND-IN, #308. m12's corroborator IS mission 1's sortie word for word
   ("your own infrared survey of the three panels"), so it is correct. m6 (antenna topology)
   and m8 (cable count) currently credit against that SAME sortie because a dead end is worse
   than a stand-in. Flying an IR survey does not establish a cable count. Written into the
   code comment as well as the taskboard so it cannot quietly become permanent. They need
   their own flight environments, which is what extracting lib/rsv-flight.js made buildable.

   NEXT: #306 server-side revealGate (deploy 2, operator-sequenced), then #308 environments.

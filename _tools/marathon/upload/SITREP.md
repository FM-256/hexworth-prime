...
**Auto-updated:** 2026-08-08 01:09 EDT | branch: master | dirty files: 103
...
- 83f086368 #272: the AWS CCP final is unreachable AND wrong for that course
- a6c2a387d SITREP: #297 reproduced, fix designed, awaiting green light
<!-- AUTO:END -->

**Manual section updated:** 2026-08-08 01:05 EDT — M2+M3 RE-RUN and improved. M4 still needs 3 humans.

## NOW: Trial Run — gates re-run and better. M4 is the only thing left, and it needs people.

  LAST ACTION (f1bc7c3, in the carreer_launchpad repo): fixed the interviewer, the verifier,
  and the M3 gate that hid both. Numbers below are from live re-runs, not the 08-07 run.

    M3   grounded follow-ups   1.000 (40/40)   NEW metric, replaces lexical overlap
         one question per turn 0.225 -> 0.975  persona rule 3, was never checked
         no quote-back openers 0.000 -> 1.000  persona rule 5, added
         lexical overlap (old) 1.000 -> 0.625  the number that FELL as quality ROSE
    M2   verdict agreement     0.919 -> 0.973  and both over-generous verdicts are gone;
                                               the one residual now errs STRICT, the safe way

  WHY THE OLD M3 NUMBER FELL AND THAT IS GOOD: the old metric scored a question by whether
  it reused a word from the prior answer, so the surest way to score 1.000 was to quote the
  candidate back — it REWARDED the tic that made the interviewer robotic, and never noticed
  78% of questions were also stacking two asks into one turn. The replacement verifies the
  model's own `references` field appears VERBATIM in the answer it claims to follow (read
  out of llm_call, no schema change). 1 of 40 still stacks; left, not tuned away.

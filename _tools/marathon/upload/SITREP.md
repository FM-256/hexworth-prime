...
**Auto-updated:** 2026-08-15 16:16 EDT | branch: master | dirty files: 44
...
**Manual section updated:** 2026-08-15 (Phase 1 code complete; loop closes from a surface grant).
...
## ★ NOW: CHAOS PHASE 1 CODE COMPLETE — THE LOOP CLOSES FROM A SURFACE GRANT.
   Repo @ 4dd50eb pushed to tntxtra/chaos · 376 checks · 0 gaps · host bc4.
   Grant a surface and CHAOS derives the rest, nothing hand-supplied:
     SURFACE   1 harness in grant, 0 reachable deps outside it
     HARNESS   tokener_parse_ex_fuzzer (only confirmed entry point)
     SELECTION 3 of 44 candidates; atlantis-c-bullseye excluded UNOBTAINABLE
     GENERATED generated-crs-libfuzzer.yaml; 42-directed NOT AUTHORIZED
     gate -> select[1/3] -> prepare -> build-target -> run -> collect -> reproduce -> correlate
     2 confirmed findings, json_tokener.c:651 and :800
   AUTHORIZATION IS ENFORCED AT THREE LAYERS: selection narrows crs_enabled and never widens;
   compose generation refuses to name an unauthorized CRS; mode gates the substrate BEFORE it is
   touched (observe = empty executor trace, not merely unproductive).
   MODES (operator's nmap model): scope=deps is DEFAULT — promiscuous OBSERVATION, action never
   widens with it. A dependency is visible at every action level and actionable at none.
   Ladder observe < build < fuzz; NOTHING above fuzz, an ABSENT rung not a disabled flag.
   Presets survey/verify/standard/thorough, every field overridable.

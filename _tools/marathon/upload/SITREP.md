...
**Auto-updated:** 2026-08-07 12:07 EDT | branch: master | dirty files: 104

### Latest commits (auto)
- c186afda0 Task 1 could not read an ordinary machine name, and the suites now live in git
- 3c9193a6d SITREP: ch25 done and live, and the two mistakes that cost ten review rounds
...
## NOW: ch25 live. Name-detection fix at Chris, deploy authorized on pass.

  IN FLIGHT: commit c186afda0. guessOS() only allowed SPACES, so Windows-11, Win-11,
  Windows_11, Win.11 and W11 all read as Other/Unknown and task 1 could not be satisfied.
  Reported from the live lab. Punctuation now folds to a space; \b keeps sw11/vmware11 out.
  WHY NOTHING CAUGHT IT: every example name I ever wrote was "Win11-Lab", hyphen AFTER the
  digits, so no test or doc ever exercised the separator. My tests agreed with my examples.

  THE SUITES ARE NOW IN GIT: _tools/lab-tests/, 9 suites + run-all.js, 249 checks.
  Paths derive from the repo root (0 absolute home paths) so they survive a fresh clone.
  run-all.js proven to exit 1 when a suite fails, not just report success.
    lab-check 43 · lab-s3 60 · lab-cheat 29 · lab-s2 29 · quiz-qc 24 · lab-flow 19
    lab-brief 14 · core1-vm 31 · lab-layout (no-scroll at 1600x900 and 1366x768)
  Run node _tools/lab-tests/run-all.js before touching any A+ lab.

  LIVE ON HEXWORTH.COM: ch25 deck + 3-stage lab + server-graded quiz; Core 1 #294 gates.
  Lab has Use as Lab VM + Remove, a self-driving task rail, and Settings > General rename.

  THE TWO MISTAKES THAT COST ELEVEN REVIEW ROUNDS (mine, both):
   1. Grading that trusts state re-read or frozen at the wrong moment. Five instances.
   2. Proving a claim for the path I edited and asserting it everywhere else. The quiz would
      have failed every perfect student at 60%; #294's fix was undone by Start Over; the
      name bug survived because my fixtures shared my blind spot.
   RULE: enumerate every path that writes the state a gate reads, and test from each.
   Write assertions for what the check CLAIMS, and vary the fixture, not just the code.

  OPEN
    #295 gradeQuiz cannot score a drawn subset (415 serverGrading files, needs a functions
         deploy + regression sweep). #296 sweep labs for the finishStage double-award shape.
    Consider wiring run-all.js into deploy.sh once it has run clean for a few days.

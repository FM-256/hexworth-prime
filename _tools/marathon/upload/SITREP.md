...
**Auto-updated:** 2026-08-08 11:39 EDT | branch: master | dirty files: 102

### Latest commits (auto)
- 0ca297d9c SITREP: provider errors no longer surface as HTTP 500
- 873c371bf SITREP: rewrite the NOW block, 101 lines to 57
- 8382d03c9 SITREP: Trial Run product round + multi-provider BYOK
...
**Manual section updated:** 2026-08-08 11:15 EDT — Trial Run: product round + multi-provider done.

## NOW: Trial Run is feature-complete for M4. M4 needs 3 humans and a consent call.

  ~/hexworth-shared/carreer_launchpad/  — its OWN git repo, LOCAL ONLY, still no remote.
  Run:   ALLOW_STUB_LLM=1 .venv/bin/uvicorn api.main:app --port 8137
  Tests: ./run-tests.sh   — 12 offline suites, no key needed, all passing
  Key:   grep ANTHROPIC_API_KEY ~/job-campaign-stack/.env   (Lodestar's, works)
  READ FIRST before recruiting anyone: trial-run/docs/sample_report.md — a REAL report
  from a live run. It is what an M4 subject is handed. Decide it is worth their half hour.
  It now carries a note at the top: the scripted candidate answers a FIXED list regardless
  of what is asked, so Q and A often do not line up. That is the harness, not the product —
  without the note a cold reader would think the interviewer is incoherent.

### M4 IS BLOCKED ON YOU. Two things, neither is code.
  (a) 3 students with real Hexworth history. I will not simulate their reactions — it is
      the only gate that decides whether this product should exist.
  (b) CONSENT (spec §13.3). item.attempt/item.complete carry consentScope="performance
      data", status="instructor-facing". Routing that into a service a student PAYS FOR is
      a different purpose than it was collected under.
  tools/export_tier_a_corpus.py builds a subject's corpus. READ-ONLY by construction, NOT
  RUN against production, and refuses without --consent-confirmed <initials> or for a
  student with no consent record (absence of a decline is not consent).
  Protocol + the exact question to ask + empty results table: trial-run/docs/m4_thesis.md.

### GATE STATE (all live-measured on claude-opus-5)
  M1 PASSED · M2 PASSED recall 1.000, agreement 0.946, residual errs STRICT (the safe way)
  M3 PASSED grounded 40/40 · one question per turn 40/40 · zero quote-back openers
  M4 NOT RUN — needs (a) and (b) above.

### WHAT SHIPPED SINCE THE GATES (ef3d2f2, 2136d24, 9c84a61)
  - Setup dropdowns: seniority / round / pressure. Each CHANGES THE PROMPT (asserted).
  - MULTI-PROVIDER BYOK: Anthropic, OpenAI, Google. Provider decided by KEY PREFIX, not a
    second dropdown. Key/model mismatch REFUSED, never silently corrected. Every
    non-Anthropic option labelled UNMEASURED — only claude-opus-5 has ever been gated.
  - Voice: browser-native Web Speech. Read-aloud is local; dictation is Chrome/Edge and
    CHROME SENDS AUDIO TO GOOGLE, said in-page before the mic opens (this app holds resumes).
  - PDF + markdown download. Serves the STORED report, never generates (2 downloads = 0 calls).
  - Practice sheet ("Answer these again"), a §6.3 extension. Deterministic, no model call.
    NOW TRIAGES (2092e05): was flagging every answer with every gap — 6 entries, up to 5
    fixes each, 30 undifferentiated items. Nobody rehearses 30 things. Ranked by severity
    (contradicted > unsupported > answer shape), worst first, capped at 3 answers x 3 fixes.
    The cap is REPORTED: "6 of your answers have something to fix, and the 3 worst are
    below". It nearly shipped saying "3 of your answers" because it counted the POST-cap list.
  - FOUND + FIXED: MIN_QUESTIONS was dead config, so a candidate who had said everything at
    Q6 could not stop, typed filler, and the report counted it against them. The product was
    manufacturing the weakness it reported.
  - FOUND + FIXED (020c84d): a provider failure came out as HTTP 500. A real out-of-credit
    OpenAI key returned RateLimitError, which reached FastAPI as "Internal Server Error" —
    telling a BYOK candidate OUR service was broken when THEIR balance was empty. Now one
    taxonomy for all 3 providers: 401 bad key / 402 no credit / 429 rate limit / 503 their
    outage / 502 anything else. The 402-vs-429 split matters: OpenAI uses RateLimitError for
    both, and one means "go and pay" while the other means "wait" — the quota message
    deliberately contains no "try again", asserted by a test.

### TRAPS WORTH REMEMBERING
  - build_client now falls back to OPENAI_API_KEY / GOOGLE_API_KEY too. An ambient key sent
    the OFFLINE suites at a real API. THE GATES HAVE THE SAME EXPOSURE: with an ambient
    OpenAI key, M2/M3 would run on GPT-5 while the artifact says claude-opus-5. Both gates
    PRINT the model they used — check that line before trusting any gate number.
  - OpenAI client HAS now been reached live (there is an OPENAI_API_KEY in this env; the
    key is valid but the ACCOUNT HAS NO CREDIT). That proved the ERROR path only. A
    SUCCESSFUL OpenAI or Google call is still unexercised — one real session each before
    anyone trusts them.
  - TestClient only fires startup events inside a `with` block. A throwaway probe without
    init_db() reported /candidate returning 500 and the app was fine. Check the harness.

### WAITING ON YOU
  1. 3 M4 subjects + the consent ruling (§13.3).
  2. Hosting. Analysis committed at _docs/operations/trial-run-hosting.md, NOT decided.
     Recommended: static frontend in _app/career/trial-run/, API on Cloud Run behind a
     Firebase rewrite at /api/trial-run/** — SAME-ORIGIN is what keeps the Sec-Fetch-Site
     trust boundary working. NOT before M4.
  3. A REMOTE for the carreer_launchpad repo. It survives an rm now, not a disk loss.

...
**Auto-updated:** 2026-08-14 14:48 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 7211ea2c0 Karl: a LIVE graded question keys an unverifiable stat, and its own course disputes it
- ce6d2c2e3 BUG-111 gated and BUG-110 generated: assessment integrity for a live course
- 2882470e6 BUG-096: build the prevention, and NOT the way that would have broken 2,473 pages
- 43cb63088 BUG-099 FIXED: two live courses could never record progress, and nothing wrote the key
- f6c20922b BUG-107 round 6: my "fix" was a tautology, and it went green on a broken hub
<!-- AUTO:END -->

**Manual section updated:** 2026-08-13 (deploy freeze recorded; BUG-107 blocked and documented).

## ⛔ DEPLOY FREEZE IN FORCE. OPERATOR RULING 2026-08-13:
   "because we are currently in class and the students are actively working on the cloudmaster
   we will not be messing with deploys."
   A class is IN SESSION on the cloudmaster (OpenStack) course — the exact surface every open
   item below touches. NOTHING SHIPS TO PRODUCTION until the operator lifts this. It is a
   live-students freeze, not a technical block: `a1cddce05` is ready to go the moment it is
   lifted AND Chris's block is cleared. Board is documented and parked; next work is a
   DIFFERENT project, operator's pick.

## ⏳ NOW: CHRIS IS ON ROUND 5 OF BUG-107. NO VERDICT. RECORD NOTHING AS A PASS.
   Five rounds. THE PRODUCT CHANGE HAS BEEN CLEAN SINCE ROUND 1 -- every single block has
   been in MY VERIFICATION WORK, never in a1cddce05. That is the honest signal from this
   whole bug: the code was fine, the thing proving the code was not.

   ROUND 4 WAS THE TURN. I asked him directly whether the approach was wrong rather than
   the instances, and told him to say so even if the code passed. He said YES, REBUILD.
   Three rounds of blocks in one file were not three instances but ONE design defect: a
   regex asked to answer questions that need a PARSER, so every fix revealed the next
   syntax it could not see -- full-line //, then HTML comments, then trailing //, then a
   string containing '/*' that ate 26 lines of live code. His words: "there is no round 5
   that ends this, because the list is the grammar of two languages." He did not just
   assert it, he BUILT the alternative and ran it against four cases first.

   REBUILT in 3bdbf7a80: stripComments() DELETED; first paint now read by loading the hub
   with setJavaScriptEnabled(false), so THE BROWSER'S HTML PARSER IS THE COMMENT STRIPPER
   and there is nothing to get wrong; the onComplete source scrape DELETED not repaired
   (the runtime playthrough dominates it); the lab-id scrape replaced by a VALUE read of
   the hub's hoisted labIds. Zero regex-over-source remains in the file.
   Verified against his whole case table: legit 2nd `const total` PASSES (it was a false
   FAILURE, and the hub ALREADY has a second total at index.html:884), stale first paint
   FAILS, stale literal inside an HTML comment PASSES with no strip code at all,
   duplicated id FAILS, glob-in-a-string PASSES.
   Now hub 54/54 (was 56: three source-scrape assertions retired ON PURPOSE), path 10/10,
   lab-credit 15/15.

   ⚠⚠ I DESTROYED COVERAGE MID-FIX AND REPORTED SUCCESS. My first attempt spliced by
   string index and silently ate the failing-student phase (BUG-106), the cross-device
   phase (BUG-101), the page-errors check and the summary -- TWELVE checks -- while
   printing 44/44. Caught only because the summary line stopped appearing. Reverted to
   the committed 56/56 and redid it with verbatim anchors plus an assertion counting the
   check( calls in each deleted block; that assertion then caught a SECOND over-wide
   boundary before it landed. This is feedback_repairing_a_test_can_drop_its_coverage,
   which I WROTE THIS SESSION.
   ⚠ And one of my own mutations was a SILENT NO-OP: I planted a duplicate id using
   `<span id="activityCount"` when the element is a `<div>`, then read a PASS that had
   tested nothing. Anchors are asserted present now.
   BECAUSE OF THAT, round 5's main ask is not a re-review: it is an INDEPENDENT COVERAGE
   DIFF against df0b3ddb1. My own 54/54 is not evidence.

   ⚠ BUG-113 LOGGED: 12 hand-rolled comment-strip regexes across _tools/ (Chris counts 17
   incl. JS-only), when _tools/eduscan/utils/strip-noncode.js is the hardened order-aware
   single source. access-guard-placement-test.js:46 carries the OPPOSITE bug -- it strips
   all // including inside URLs. I wrote another variant without searching the catalog,
   which is the rule that exists to prevent exactly this.

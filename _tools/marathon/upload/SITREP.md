...
**Auto-updated:** 2026-08-07 21:18 EDT | branch: master | dirty files: 106
...
## NOW: Trial Run — M1 done, UI usable, M2 built but its gate has not been run.

  ~/hexworth-shared/carreer_launchpad/trial-run/    spec: ../TRIAL_RUN_BUILD_SPEC.md
  Read trial-run/docs/ for anything you need. In hexworth-prime only one thing changed:
  the Career Launchpad card (36f973e3a), coming-soon, deliberately not a link.

  Run it:  ALLOW_STUB_LLM=1 .venv/bin/uvicorn api.main:app --port 8137   then http://127.0.0.1:8137
  Tests:   tests/test_m1_gate.py · tests/test_ui_flow.py · tests/test_m2_gate.py --self-check

  Next: run the M2 gate. Needs an ANTHROPIC_API_KEY, which this box does not have.
    ANTHROPIC_API_KEY=sk-... .venv/bin/python tests/test_m2_gate.py

  Waiting on you: that key; hosting (spec §13.2) before the card can become a link; and
  whether to sweep em-dashes from all career page titles or exempt them from the gate

...
**Auto-updated:** 2026-08-25 07:59 EDT | branch: master | dirty files: 65

### Latest commits (auto)
- 38b5e3fc5 Both pending issues verified closed, and a way to settle the office one on site
- 22200afd6 Full suite green: six gates, four missions, Horizon and the console
- b5743fb51 SITREP: the instance console is fixed, and the two traps that nearly hid it
- 579ddaeb1 The VNC console never worked; now it does, proven in a browser
- 6da9327d4 Punctuation in my own comments, fixed rather than forced past the gate
<!-- AUTO:END -->
...
## NOW: ALL GREEN, deployed and re-verified (2026-08-25 08:00)

Deployed at `38b5e3fc5`. Note: **0 `_app` files changed** since the previous deploy, so hosting
shipped identical content -- the run was for the gates. Chris was skipped on that factual basis
(audit-logged), NOT as a convenience. All 7 deploy stages passed: Nexus, EduScan critical+high
clean, CTF flag cross-check, Firestore-rules bypass check, 34/34 Skill Maps, 10/10 content-leak
smoke, post-verify PASSED.

| Surface | Status |
|---|---|
| 6 lab QC gates (chain, cinder, neutron, project, rescue, secgroup) | GREEN -- all passed |
| Sprint 4 missions, STUDENT creds, no admin | GREEN -- password_leaked=0 |
| Horizon dashboard login | GREEN |
| Horizon instance console (noVNC) | GREEN -- fixed today, 2 slots verified in a browser |
| Off-tailscale reachability | GREEN -- 7/7, proven via eth0 |
| Production + deploy gates | GREEN -- all 7 stages |
| bc1 lab-manager, vnc-proxy, horizon-proxy / bc2 claim, vnc bridge | all up |

**The only thing not verifiable from here:** the office's own firewall. Run
`bash _tools/openstack-bridge/office-reachability-check.sh` ON that network -- a websocket-only
failure means a proxy strips Upgrade, which breaks the console and nothing else.


### 1. Horizon instance console -- FIXED, and yes it WAS an IP issue
Nova handed the browser `novncproxy_base_url`, which pointed at the DevStack VM's PRIVATE
address on 6080. No student could route to it; the console page loaded and only the VNC view
stayed black, so nothing ever errored. Now returns `sandbox.hexworth.tech`.
Verified in a REAL BROWSER on two different slots and instances (student-11/lab5-vm,
student-13/proj-vm): websocket 101, stays open, canvas paints, "Connected to QEMU".
`node _tools/openstack-bridge/verify-novnc-console.js <pw> <cookie> [slot]`

Swept for the same class of bug: `spice-html5` still carries a private base_url BUT is
DISABLED -- measured, Nova returns HTTP 400 for it while novnc returns 200 with the public host.
Inert. Private IPs in handouts/presentations are teaching examples (10.0.0.x, 172.24.4.x
floating range), not browser endpoints.

### 2. Access from the office, WITHOUT tailscale -- WORKS
Proven, not assumed:
- `hexworth.com` -> 199.36.158.100 (Firebase), `sandbox.hexworth.tech` -> Cloudflare. Public
  addresses, not tailnet.
- `ip route get` for both goes via **eth0, not tailscale0**; `ExitNodeStatus: None`.
- Whole student flow re-run with `curl --interface eth0`: lab pages 200, Horizon gate 401 anon /
  200 with cookie, noVNC page 200, console websocket **101**.

**The one thing that cannot be tested from here** is the office's own firewall. A filter that
blocks the `.tech` TLD, or a proxy that strips WebSocket Upgrade, would break the console while
everything else stays green. Run this ON the office network to settle it in ten seconds:
`bash _tools/openstack-bridge/office-reachability-check.sh` (7 checks, no credentials, changes
nothing; a websocket-only failure points at an Upgrade-stripping proxy).

### Full suite, same session
6 lab QC gates ALL PASSED in parallel (chain, cinder, neutron, project, rescue, secgroup) --
cheats rejected, honest path twice, checks discriminating 4/4/4/4/3/4. Sprint 4 missions on
STUDENT creds: M4 password_leaked=0. Production 200s, all services up.

### OPEN -- operator's call, nothing blocked
- **Tailscale ACL bc1->bc2 permits only two ports** (measured). The console tunnels through a
  permitted one rather than exposing 6080; widening it would simplify `setup-novnc-console.sh`.
  bc2's subnet route approval also still pending.
- **19 pool slots hold test debris**, including the operator's `Killa-B`. Untouched deliberately.
- **5 of 7 adversarial harnesses lack a do-nothing case** (chain, neutron, project, secgroup,
  wall). That blind spot hid check 3 for a month.

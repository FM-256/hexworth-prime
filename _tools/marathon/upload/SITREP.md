...
**Auto-updated:** 2026-08-15 15:30 EDT | branch: master | dirty files: 44
...
**Manual section updated:** 2026-08-15 (CHAOS Phase 1 code complete; remaining items are operator decisions).
...
## ★ NOW: CHAOS PHASE 1 — CODE COMPLETE. Repo @ a9af097 pushed, 322 checks, 0 gaps.
   Host /opt/chaos + /opt/oss-crs on bc4. Target json-c OSV-2020-252.
   A campaign now runs where CHAOS DECIDES, and the decision produces what executes:
     gate -> select[1/3] -> prepare -> build-target -> run -> collect -> reproduce -> correlate
     2 confirmed findings, json_tokener.c:651 and :800, correctly separated.
   CHAOS chooses its own CRS from 44 live registry entries, excludes atlantis-c-bullseye on
   VERIFIED obtainability before wasting a prepare, GENERATES the compose from that choice
   (drives oss-crs gen-compose, does not emit YAML), and cannot exceed its authorization at
   EITHER end. Selection narrows crs_enabled and never widens it; compose generation refuses to
   name an unauthorized CRS.
   ⚠ TEN DEFECTS FOUND BY RUNNING IT, none visible to a green suite. The last one was mine and
   the worst shape: I hardened the orchestrator's narrowing rule, then bypassed it one layer
   down by generating a compose for an unauthorized CRS. The substrate obeys the COMPOSE, not
   crs_enabled, so the audit trail would have recorded "dropped 42-directed" while 42-directed
   was exactly what ran. An audit trail that contradicts the run is worse than none.
   TWICE now a guard was bypassed by a path I built AFTER it. The orchestrator check and the
   generator check are NOT redundant: they sit on opposite sides of what executes.

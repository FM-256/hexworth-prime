...
**Auto-updated:** 2026-08-24 00:20 EDT | branch: master | dirty files: 46

### Latest commits (auto)
- 18bc1d19c Sprint image: let cloud-init write /etc/hosts, so sudo stops warning on every command
- 61b63b3e5 Sprint image: add openssh-server, and enable services properly (systemctl is a no-op in a chroot)
- d460db4e9 SITREP: the sprint runs end to end; record the no-egress finding and the five build bugs
- 616ea29e2 Five real bugs in build-sprint-image.sh, all found by finally running its DO path
- 504ff6839 Bake the sprint image, because student instances have no internet and apt cannot work
<!-- AUTO:END -->

**Manual section updated:** 2026-08-24 - ALL FOUR MISSIONS VERIFIED with the shipped assets. Read
the 08-24 block, then 08-23 (process/tools), then 08-22 (capacity/image), then 08-21, then 08-20.

## 2026-08-24: all four missions PROVEN end to end. Nothing in the student flow is unverified.

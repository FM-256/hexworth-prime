...
**Auto-updated:** 2026-08-20 01:43 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 7f1f81921 Taskboard 311: rotate the neon CIFS share password — the value is compromised
- 527e57dbe Move the CIFS credential out of world-readable fstab, and lock the backups too
- 1b231092a Prometheus recovery: a second mechanism, and detection from a host that is not it
- 740005bd2 Power loss #2: the resume fix held, and Prometheus was down while its port said 200
- e856819fb Playable box for qual-w1-lockout: the spine now has a scenario on it
<!-- AUTO:END -->

**Manual section updated:** 2026-08-20 01:55 EDT - power loss #2 recovered; tourney QC running; all pushed.

## ★★ NOW - POWER LOSS #2 RECOVERED. Everything pushed. Tourney adversarial QC RUNNING.

**ALL 35 COMMITS PUSHED** (origin/master == HEAD, verified by re-reading the remote, not the
push output). Nothing today lives only on this machine any more.

### The power cut tested yesterday's fixes. Two held, one did not.
| fix | result |
|---|---|
| resume_guests_state_on_host_boot | **HELD** - 7/7 guests back ACTIVE unattended (08-18: all SHUTOFF, found by a user a day later) |
| devstack Restart=on-failure x20 | **HELD** - 0 failed units |
| prometheus `unless-stopped` | **FAILED SILENTLY** - see below |

Config survival verified on disk, not just by outcome: resume flag present, 20/20 units carry
Restart=on-failure, 20 drop-ins present, bc2 compute-probe cron + textfile drop-in present,
bc1 probe cron.d present, emergency bc1->bc2:22 grant still REVERTED.

### ⚠⚠ PROMETHEUS WAS DOWN AND ITS PORT SAID 200
Exited(255) from 03:22:14, did NOT restart despite unless-stopped, while 13 siblings with the
IDENTICAL policy did. Started cleanly by hand -> it was never restarted, not failing to start.
Cause UNKNOWN: no container logs from that boot, no docker journal entries for the window.

**With prometheus down NO RULES EVALUATE.** Every alert built 08-18/08-19 was silent and could
not have fired. `curl :9090/-/healthy` returned **200 from COCKPIT-TLS**, which owns 9090 there;
prometheus publishes on **9091**. Nearly recorded "prometheus fine" on that 200.

Now double-covered: `ensure-monitoring-up.sh` (systemd OnBootSec=90s + 10min timer on neon, exits
NON-ZERO when it had to rescue, so a recurring failure cannot become invisible) AND a `prometheus`
check in the bc1 probe that asserts the BODY text, mutation-tested against port 9090.
Probe is now 8/8.

### Also fixed tonight
- **/mnt/neon-shared** unmounted: boot DNS race (_netdev waits for network, not resolution, and a
  failed fstab mount never retries) AND the hostname now resolves via MagicDNS to the tailnet
  address where 445 is ACL-blocked (correct - SMB is a LAN service). Now LAN address +
  nofail,x-systemd.automount.
- **CIFS credential out of world-readable fstab** -> /etc/cifs-neon.creds 0600. /etc/fstab is 644
  and `sudo -u nobody test -r` succeeded, so every local account could read it. MY OWN BACKUPS
  re-leaked it at 644 - both chmod 600, kept not deleted.
  ⚠ **TASKBOARD 311: THE PASSWORD MUST STILL BE ROTATED.** Moving a leaked secret does not un-leak
  it. Needs a coordinated Samba-side + every-mounting-host change; deliberately not attempted.

### OpenStack, fully swept (`functions/openstack-full-check.sh`)
6/6 API endpoints · 20/20 devstack units · nova 4/4 up+enabled · neutron 2/2 · cinder 2/2 ·
scheduler CAN place · 9 ACTIVE + 2 deliberately-stopped · Horizon real login form ·
public GUI: 401 anon, **302 + sessionid authenticated**.

### ⚠ QUALIFIER vs TOURNEY PROCESS - decision pending
The new box (`_app/arena/boxes/qual-w1-lockout/`, committed, NOT deployed - 404 on prod) calls
validateFlag + recordMissionFinding, which have **ZERO tournament references**. So it is NOT
covered by admin activation/visibility. Meanwhile ctfSubmitFlag IS activation-gated but knows
nothing about mission_gates, so registering the box as a challenge would let a bare correct guess
score. verify-qual-box.js FAILS if that is ever configured.
PROPOSED: make ctfSubmitFlag call assertGateSatisfied when the challenge names a boxId. Then
admin activation AND anti-guessing both hold. Backward compatible. NOT BUILT - awaiting the
tourney QC now running.

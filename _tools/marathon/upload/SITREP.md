...
**Auto-updated:** 2026-08-07 14:27 EDT | branch: master | dirty files: 104

### Latest commits (auto)
- a5b6c78f6 Respect the separators a student typed; only infer the ones they did not
- d49e233b6 Stop guessing where the OS word ends, and ask whether it is next to the number
- c0304cc5b The name fix only fixed the front of the number
- 5a2b12aea SITREP: name-detection fix in flight, suites now tracked in _tools/lab-tests
- c186afda0 Task 1 could not read an ordinary machine name, and the suites now live in git
<!-- AUTO:END -->
...
## NOW: hexworth ch25 DONE + LIVE. Lodestar intake at Nancy+Chris, commit on pass.

  HEXWORTH, ALL LIVE AND VERIFIED ON HEXWORTH.COM. Nothing pending.
    ch25 deck + 3-stage lab + server-graded quiz. Core 1 #294 gates fixed.
    Guest-name detection took FOUR Chris rounds: only spaces counted, then the trailing
    boundary was still required, then a prefix with no case change, then my substring
    redesign swallowed "Darwin 11"/"Baldwin 11". Final rule: the OS word as its own token
    with the version next, OR the version glued to a run ENDING in the OS word. Separators
    the student TYPED are respected; only untyped ones are inferred. Prod-verified: 16
    spellings detect, 10 non-matches stay out, PCwin11 satisfies task 1.
    Lab also has: Use as Lab VM + Remove, self-driving task rail, Settings > General rename.

  TEST SUITES ARE NOW IN GIT: _tools/lab-tests/ (9 suites + run-all.js, ~253 checks).
    Paths derive from the repo root, 0 absolute home paths, verified from a clean run.
    run-all.js proven to exit 1 on a failing suite. Run it before touching any A+ lab.

  LODESTAR (~/job-campaign-stack, SEPARATE REPO, uncommitted, 3 files +260)
    Server was DEAD ~9 days: no crash, no SERVER EXITED marker; the supervisor itself died on
    a reboot. It auto-restarts the server but nothing restarts IT. Restarted, all routes 200.
    OFFERED, NOT DONE: make the supervisor start on boot so this stops recurring.
    /intake showed 1 of 22 emails (review queue only). Now a mailbox: scope tabs, type chips,
    sender dropdown, search, all combining with counts; handled mail sinks below open with an
    Archive divider; Read message fetches ONE body (same-site gated, textContent, 20k cap).
    REVERSES the prior "bodies never leave the server" stance -> Nancy is judging that.
    tests/test_intake_mailbox.py 25/25 (repo runs tests as plain scripts, no pytest).
    Nancy + Chris in flight; owner said commit on pass.

  THE LESSON THIS SESSION KEPT TEACHING (11+ review blocks, all real):
    1. Grading/logic that trusts state re-read or frozen at the wrong moment.
    2. Proving a claim for the path I edited and asserting it everywhere else.
    3. Detectors that cannot see what they check: a grep blind to &#8212;, a regex blind to
       text-transform:uppercase, a fixture sharing my own blind spot.
    RULE: enumerate every path that writes the state a check reads, test from each, and vary
    the FIXTURE, not just the code.

  OPEN
    #295 gradeQuiz cannot score a drawn subset (415 serverGrading files; functions deploy +
         regression sweep). #296 sweep labs for the finishStage double-award shape.

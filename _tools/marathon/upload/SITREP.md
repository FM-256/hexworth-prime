...
**Auto-updated:** 2026-08-14 15:36 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 874069789 BUG-115 FIXED: 12 forensics modules were unreachable, confirmed 404 on production
- c7e737f41 Wire the two completion keepers as real gates, and log Chris's forensics finding
- ce50e759d BUG-114: correct the rationale on a live graded question, keep the key
...
**Manual section updated:** 2026-08-14 (BUG-115 + BUG-116 fixed and gated; awaiting Chris round 2).

## ✅ DEPLOY FREEZE LIFTED. One deploy already shipped this session (BUG-114 rationale).
   The 2026-08-13 freeze was a live-students hold on the cloudmaster class, not a technical
   block. The operator authorised a deploy during this session and it completed. Rule 10
   still applies per-operation: the authorisation covered THAT deploy, not the next one.

## ⏳ NOW: LINK INTEGRITY. Two commits ready, Chris round 2 running on d7fd7a7cd.
   Operator: "we need to fix it. get it done."

   BUG-115 (874069789) -- the forensics hub linked 12 modules that do not exist. Verified
   against PRODUCTION: 62 hrefs fetched from hexworth.com, exactly 12 returned 404. The
   content existed the whole time; only filenames had drifted (df-05-cfaa-laws vs the real
   df-05-cfaa-federal-laws, and eleven more). Rewritten by DERIVING from disk, 12 lines,
   every one an href, id list byte-identical. Chris independently re-derived the same 12.

   BUG-116 (d7fd7a7cd) -- CHRIS BLOCKED THE ABOVE ON THE GATE, AND HE WAS RIGHT. The gate I
   wrote to prevent BUG-115 read TWO data files, printed "4/4 passed", and deploy.sh called
   it "every hub href resolves". Real coverage: 95 of 5,727 hrefs, about 2%.
   THE GAP WAS NOT HYPOTHETICAL -- 108 MORE LINKS WERE ALREADY BROKEN.
   - LearningPaths 66 dead, FIXED. The file mixes root-relative and house-relative hrefs;
     resolveModuleHref():6286 is a NO-OP whose own comment claims it prepends the house dir.
     Fixed the 66 data lines, NOT the shared resolver -- the resolver serves 664 others.
     8 sampled targets now return HTTP 200 on prod where they returned 404.
   - ContentCatalog 42 dead, all confirmed 404 on prod, BASELINED NOT FIXED. Forge
     "Intro to Computers" advertises a 4-week course with only index.html + 3 presentations
     built (23 links). Shield PIS wants pis-01..20 where disk has pis-r1..r5 (17). No 1:1
     mapping exists, so repointing is a CONTENT DECISION and needs the operator.

   ⚠ TWO FILES WERE MISSED BY MY OWN SWEEP; CHRIS FOUND THEM. I filtered candidates to
   files declaring .module.html hrefs -- a property of the EXAMPLE, not the CLASS. ForgeData
   (116) and SignalData (112) link plain .html. Both clean, but that was luck, not method.

   ⚠ A WRONG BASE INVENTS WORK RATHER THAN HIDING IT. Resolving ContentCatalog against
   houses/<house>/ reported 558 dead; its real HOUSES table maps matrix->operator/ and
   forensics->houses/eye/forensics/, true count 42. Caught the same class again in review
   before it ran: parsing SignalData by line shape reads PROJECT ids as SECTION ids and
   fabricates 47 dead links, because SignalData's projects are multi-line and ForgeData's
   are one-liners. Every base is now read from the consuming code and cited in a comment.

   ⚠ I ASSERTED SOMETHING FALSE FROM THE NARROW GATE. I told the operator df-61 was an
   unreachable module. ContentCatalog links it; it returns 200. A detector narrower than its
   claim does not only miss things, it asserts false ones. Corrected in the tracker.

   Gate now: 7 data files, 5,727 hrefs, 22/22, deploy.sh 3.2 no bypass, ablation-tested on
   three paths with every fixture restored byte-identical.

## ⏭ NEXT (blocked on Chris round 2, then operator authorisation to deploy)
   1. Chris PASS on d7fd7a7cd -> ./deploy.sh (gate 1.5 checks HEAD).
   2. OPERATOR DECISION NEEDED: the 42 baselined links are 404s a student can hit TODAY.
      Forge intro-computers and Shield PIS each need build-or-delist.
   3. Still open: BUG-101, BUG-102, BUG-108, BUG-109, BUG-112, BUG-113, BUG-114 root question.
   4. Debris: 15 probe files at repo root + _tools/ need archive-then-remove (rm denied to me).

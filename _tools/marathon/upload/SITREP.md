...
**Auto-updated:** 2026-08-15 18:42 EDT | branch: master | dirty files: 44
...
**Manual section updated:** 2026-08-15 (write primitive demonstrated; disclosure half not found).
...
## ★ NOW: CHAOS Phase 1 code complete + FIRST WRITE-CLASS FINDING. Repo @ 16b64be pushed.
   ...
   ★ WRITE PRIMITIVE DEMONSTRATED — libxml2 OSV-2025-457, heap-buffer-overflow WRITE size 1 in
     xmlParsePubidLiteral parser.c:4458. A/B: 91 of 91 PoVs discriminate (100%, vs json-c's 13%),
     0 crash both, 0 regressions, all on one line. Matches the advisory on crash class, access
     type, access SIZE and function.
   ⚠ DISCLOSURE HALF NOT FOUND. OSV-2024-634 htmlCurrentChar: 0 PoVs at cov 3077. I picked it
     for CONVENIENCE (harness already built) not reachability — 42 seeds vs xml's 366, half the
     coverage. Composability stays UNDEMONSTRATED: capability.py sees both classes only from
     advisory metadata, never from reproduced findings.
   ⚠ SIX TARGET ATTEMPTS, FIVE FAILED, each teaching a criterion invisible until you run:
     libpng reachability · bullseye obtainability (404 submodule) · libarchive buildability
     (-Werror @2019) · libarchive semantic-fix (OSV "fixed" was a MERGE = where OSS-Fuzz stopped
     reproducing, NOT where the bug was repaired) · libxml2-2023 build-script skew (source
     predates fuzz/oss-fuzz-build.sh by ONE DAY) · libarchive findability (0 crashes/150s).
     tools/vet_target.py screens three mechanically. Reachability and findability have NO check.

## ⚠ MY OWN ERRORS TODAY, KEPT VISIBLE
   ...
   verified). Committed with a RED suite once. Bypassed my own authorization gate one layer down
   via compose generation — the audit trail would have said "dropped 42-directed" while
   42-directed ran. Wrote a >=2019 build threshold from ONE data point that admits a commit which
   demonstrably does not compile; demoted to a reported risk. A bounded wait loop that proceeded
   anyway, so a fuzz ran against a build competing for CPU. An A/B sweep that read a PoV dir the
   fuzzer was still writing to.

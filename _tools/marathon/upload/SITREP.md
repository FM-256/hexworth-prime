...
**Auto-updated:** 2026-08-14 20:09 EDT | branch: master | dirty files: 48
...
- cb1f8350c BUG-120: three targeted edits missed a fourth instance, so this time I grepped
- 711c0e029 BUG-120: 245/105 was two different scopes reported as one number, and my repro did not run
- 85e613333 BUG-120: I put an unverified count in the entry that exists to stop unverified counts
<!-- AUTO:END -->

**Manual section updated:** 2026-08-14 (all deploys landed; nexus full running).
...
## SHIPPED AND VERIFIED LIVE TODAY
   ...
   BUG-119: the finals-week "GUI Quiz" card repointed to the real final exam, and 3 false
     "covers all 9 objectives" claims corrected, including the subtitle a student reads
     immediately before sitting the exam.
   Em-dash sweep: 83 occurrences across the two files I authored today, including one LIVE
     student-facing string ("3 / 3 objectives complete -- lab complete.", dating from
     2026-07-30). Verified by running the SERVED expression against real page titles, not by
     assuming byte-identical means working.

## NOW: nexus full (operator-requested, master, publishes to production Firestore per rule 10).

## OPERATOR DECISIONS OUTSTANDING
   1. BUG-119: the Python for IT final tests Obj 1-6 ONLY. Zero questions on Tkinter GUI
      (the subject of the week it is sat in), sockets/threading, or sorting/search. Extend the
      exam, which requires re-seeding quiz_keys/pfi-w4-final-exam or every student is
      misgraded, or accept the narrower scope.
   2. BUG-120: the dash gate cannot see a "--" at a line wrap. The naive fix arms 243 false
      blocks across 104 files. Needs a decision, not a patch. Documented with a measured
      per-file table and a repro script that actually runs.

## THE FINDING OF THIS SESSION: WHEN I REPORT A CLEAN RESULT, SUSPECT THE INSTRUMENT.
   Ten review rounds on the last deploy alone. EVERY block was in my claims or my instruments,
   never in the product change.
   THREE detectors I trusted were themselves the defect:
     tree-mapper UNDER-REPORTS (600 nodes -> 31, depth 5 -> 2); its "Broken: 44" is a shallow
       crawl, not an improvement. Shallow regen archived, NOT committed.
     audit-hub-deadrefs-v2.js prints refs=0: filters severity 'high', HUB-001 is now 'medium'.
       It went blind BECAUSE the platform improved.
     dash-hygiene-gate could not see a "--" at a line wrap, so its clean report was false, and
       my own grep had the identical blind spot: my count said 79, the truth was 83.
   FOUR times I called GATED content broken (df-61, 42 catalog entries, 11 isc2-cc cards where
     I EDITED THE FILE before finding the gate below them, the GUI Quiz card).
   FOUR times a probe reported clean without REACHING its target (collapsed accordion,
     display:none slide twice, an ablation planting a value that was actually CORRECT, a no-op
     ablation).
   FOUR times a figure in ONE bug entry did not survive derivation, including two scopes
     reported as one number and a label I characterised instead of reading.
   FOUR times I asked a regex to do a lexer's job, after 3bdbf7a80 THIS MORNING ruled "stop
     parsing with regexes". Two of those I deleted rather than patch again.
...
## NEXT
   1. Read the nexus full results.
   2. Fix tree-mapper before wiring any gate to it. Fix audit-hub-deadrefs-v2's severity
      filter, base resolution and quote handling.
   3. GAP: presentation-level hrefs are not covered by gate 3.2, which reads DATA files only.
      Only the course-tree crawler sees that class, which is the argument for repairing it.
   4. Debris needing operator rm (denied to me and to Chris): _tools/qa/_chris_ablation_tmp5.js,

...
**Auto-updated:** 2026-08-19 10:43 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- c98014af3 Close the monitoring gap: alert on whether a student can LAUNCH, not whether keystone answers
- 343e89abe The broken keys never reached a student: 0 attempts on all 94 corrected quizzes
- 16033f49f Read all 1244 questions in the repaired batch: 11 keys were wrong
- c40bf6654 OpenStack confirmed recovered: server-a ACTIVE, ERROR count 0
- da54f5c68 nova-compute was dead for 15 hours and every monitor was green
<!-- AUTO:END -->

**Manual section updated:** 2026-08-19 13:45 EDT — quiz keys repaired (0 students harmed) and OpenStack fixed + now actually monitored.

## ★★ NOW — BOTH STUDENT-FACING FIRES ARE OUT. Nothing is mid-flight; pick up anywhere.

| # | item | state |
|---|------|-------|
| 1 | OpenStack `nova-compute` dead 15h | **FIXED + VERIFIED** — server-a ACTIVE, 0 in ERROR |
| 2 | All 20 devstack units had no `Restart=` | **FIXED** — drop-ins, proved by `kill -9` |
| 3 | Monitor was green through the whole outage | **CLOSED** — bc2 compute probe live in prometheus |
| 4 | 83 quizzes graded against a placeholder key | **FIXED** — restored from registry, verified live |
| 5 | 11 more keys individually wrong | **FIXED** — found by READING all 102 flagged questions |
| 6 | Students harmed by any of it | **ZERO** — 0 attempts on all 94 corrected quizzes |

**OPEN, none urgent, none student-facing:**
- **~1140 questions in the repaired batch were never individually read.** The content audit
  missed 8 of its 11 real defects in its own confident tier, so its silence is WEAK evidence.
  No student has ever attempted these quizzes, so there is no time pressure — but this is the
  honest residual risk and it is not closed.
- 9 OpenStack instances still `SHUTOFF` from the power-loss reboot. Nothing deleted or restarted
  — operator call. (`server-a` is fine; that one is ACTIVE.)
- 15 `eth-*` keys drift from the registry. They have ZERO HTML callsites — unreachable, so the
  drift harms nobody. Left in place deliberately: we archive, we do not destroy.
- 4 rotation keys are IN SYNC and verified authored (az900-ch03, ceh-01, fw-final, fl-final).
  DO NOT "fix" them — fl-final Q1-Q4 really is 443 / SYN,SYN-ACK,ACK / UDP / IMAP.

**THE LESSON FROM TODAY, worth more than either fix:** three separate times a check was TRUE
while the claim written above it was FALSE. `openstack_token` proved keystone answers HTTP, not
that a lab can launch. `verify-quiz-keys` proved a key EXISTS, not that it is right.
`quiz-key-drift-audit` proved two stores AGREE, not that either matches the content. Every one
passed while students were broken or would have been. **Ask what a green check actually
measured, not what it is named.**



## ⚠⚠ THIS REPO IS **PUBLIC**, NOT PRIVATE. isPrivate:false · 1 fork · 4 stars.
   Verified 2026-08-16 by unauthenticated curl: anyone can read any file in it. This was
   BELIEVED PRIVATE, and the belief was load-bearing — it is why the exposure audit kept being
   deferred as "before we flip public". Anything committed here is published on push.
   NEW GATE: `_tools/security/scan-exposure.py`, wired to .git/hooks/pre-commit. Reads what to
   hunt from ~/hexworth-infra-private (NEVER hardcoded — a scanner that hardcodes what it hunts
   publishes it on first commit; mine did, and caught itself). Fails LOUD if it cannot load that
   list. Baselines the 26 known files so only NEW leaks block.
   ⚠ IT CAUGHT 3 THINGS I MISSED WITHIN A MINUTE, TWO OF THEM MINE: a THIRD PARTY's full name +
   personal gmail live on the public FAQ (swapped to jorden@hexworth.com — CONFIRM WITH THEM,
   I changed someone's published contact without asking); the scanner leaking the local-part it
   hunts, in its own comment AND unmasked in its own output; and `git add -A _docs/` sweeping in
   the node inventory with ALL SIX real addresses (never committed, now gitignored).
   NOT exposed, verified: no service-account JSON, no real .env, no deployable credential.
   Nothing needs rotating. Node addrs are CGNAT — unroutable, recon-grade not access-grade.
   ⚠ STILL EXPOSED BY DESIGN until migrated: operator email in admin-emails.js, firestore.rules
   :13, FirebaseAuth.js, dr-hex-quality.html + 3 coupled tests. index.js:65 DERIVES the admin
   custom claim from that allowlist ON EVERY SIGN-IN, so DELETING THE ENTRY REVOKES THE OPERATOR.
   Two-deploy migration required — see ~/hexworth-infra-private/operator-identity.md.
   ⚠ NOTHING IS DEPLOYED. Production still serves both addresses.

...
**Auto-updated:** 2026-08-07 01:16 EDT | branch: master | dirty files: 99

### Latest commits (auto)
- 1c5227db8 Lab: a rename escape hatch, an isolation task, and two traps I built myself
- 3d2ac82ba ch25 quiz goes live, after it turned out to be unpassable by design
- 893e4c74b ch25 quiz, unlinked until its key exists, and stop a style rule failing post-verify
- 2965d7f51 ch25 virtualization lab: three hypervisors, and three doors Chris found in the grading
- 267b5eb9c Restore six chapters my domain "fix" clobbered, and stop reusing a Core 2 domain number
<!-- AUTO:END -->
...
## NOW: ch25 COMPLETE and LIVE (deck + lab + quiz). Lab improvements at Chris round 2.

  DEPLOYED AND VERIFIED IN PRODUCTION
    Lab   commit 2965d7f51. Cold open redirects to tourist-visa (AccessGuard works); as a
          sorted student it boots 3 stages, briefing opens, 0 page scroll, no JS errors.
    Quiz  commit 3d2ac82ba. 20 of 20 served, poolSize absent, serverGraded, no key in HTML.
          quiz_keys/core2-ch25 seeded, verify-quiz-keys.js PASSED. Hub links all 3 cards.
    post-verify PASSED (the --fail-on-except fix worked; it had flagged on the lab deploy
    purely because I promoted HEUR-035 to HIGH, creating 3228 legacy HIGH findings).

  THE QUIZ WAS UNPASSABLE AND I ALMOST SHIPPED IT. gradeQuiz uses
  `const total = answerKey.length` (functions/index.js:1704) = the full bank, while
  QuizEngine posts only the DRAWN questions. With poolSize 12 of 20 a perfect student
  scored 60% and failed, every attempt. I named that risk in my own Chris brief and then
  never tested it; my 20/20 QC had never once scored a submission. poolSize removed, all 20
  served. Taskboard #295 carries the platform fix (415 serverGrading files, needs a
  functions deploy + regression sweep). quiz-qc.js now simulates the grader with a negative
  control, and a regression assertion fires if anyone re-adds poolSize.

  LAB IMPROVEMENTS (built, NOT deployed, Chris round 2 in flight)
    1. Settings gains General + rename. THE TRAP: labVmId pins the FIRST machine of a stage,
       and with no rename and no delete a student who named it "test" could never satisfy
       task 1. Reset lab was the only escape and it wiped the stage.
    2. Stage 3 gains an isolation task (s3t7); the term task renumbered s3t8. The ESXi port
       groups had been rendering with no task using them.
    CHRIS BLOCKED round 1 on the thing I flagged as my weakest: s3t7 re-scanned VM NAMES at
    grading time, so an isolated "db-decoy" credited it while the real db01 stayed exposed.
    FIXED by deriving s3t7 purely from current state: every db-named stage-3 guest must be
    isolated and at least one must exist. An intermediate design pinned LAB.dbVmId at
    creation; it was REJECTED and REMOVED because it recreated the labVmId trap one field
    over (an honest rename locked the student out). grep dbVmId returns 0. Frozen-at-creation
    identity was the defect, not the field it lived in.
    SUITES: lab-s3 52/52, lab-check 32/32, lab-s2 29/29, lab-brief 14/14, lab-cheat 23/23.

  CHRIS HAS NOW BLOCKED 5 TIMES ACROSS THIS CHAPTER, every one a real defect:
  tick-cache bypass, model-fabrication overclaim, stage-sequencing bypass, unpassable quiz
  scoring, name-scanned isolation check. Four were the same class: trusting mutable state
  that is re-read at grading time. The gate is earning its keep.

  NEXT: on PASS, commit + record-chris-pass + ./deploy.sh (master, operator authorized).
  OPEN: #293 done, #294 Core 1 rubber-stamped gates still live, #295 gradeQuiz pool-draw.

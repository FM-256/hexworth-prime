...
**Auto-updated:** 2026-08-24 22:09 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 586433b09 The line every finishing student reads still made the claim I had just removed
- 8b5d4669e SITREP: BUG-058 is NARROWED, not closed - the witness cannot name the volume
- fec35aa81 The witness cannot name the volume, so the claim had to shrink to fit it
- d8225021e BUG-058: check 6 graded a claim it could not test, and a shortcut scored it
- 231145d86 All eight OpenStack lab pages now read the same: 91 characters, 15.2px, docked terminal
<!-- AUTO:END -->

**Manual section updated:** 2026-08-24 late. RESUME block is first: operator reports remaining issues, unnamed.
first, then 08-23 (process/tools), then 08-22 (capacity/image), then 08-21, then 08-20.

## BUG-058 NARROWED (NOT closed): check 6 no longer scores a shortcut (2026-08-25)

Operator: "fix it", on `qc-lab.sh cinder` failing with CHEAT A BEAT CHECK 6.

Check 6 claims the volume outlived its first server. It compared the volume's creation time
against that of whatever server currently holds it, which ANY first server satisfies, so the
five-command shortcut scored it. The gate had failed on that deliberately since 2026-07-31.

The grader's own note said the claim was unprovable on this cloud. That was measured against
the USER api. Nova keeps the instance row AND the action log for a DELETED server, and
/verify already runs server-side with admin credentials. So:
- `claim_service.py` /verify carries `deleted_servers` (+ `deleted_servers_ok`)
- check 6 requires a DELETED server of yours that attached a volume AFTER lab-vol was created
- `adversarial-cinder.js` grades an untouched tenant first (cheat 0)

**Two traps that were measured, not assumed.** `deleted=True` is admin-only and Nova SILENTLY
IGNORES it for a project user: asked with student-48's own token it returned that slot's LIVE
ACTIVE server. Trusting it would grade a running server as a deleted one. And Cinder/Nova
return different timestamp shapes -- `created_at` and action `start_time` carry no timezone
while a server's `created` ends in Z -- which JS reads as local time, a four-hour skew in the
direction that grades a cheat as honest.

**OPERATOR DECISION 2026-08-25: SHIP AS-IS, residual bypass accepted as known risk.** Asked
explicitly, with the bypass and the two closing options (seeded first server / observation
ledger) on the table. Reason it is the right call: what is LIVE today is beaten by a
five-command shortcut, the new check is strictly stronger, every student-facing claim now
matches what it actually tests, and the bypass costs MORE work than the honest path. The two
closing options remain open as future work, NOT as blockers.

**NANCY REJECTED THE FIRST VERSION, AND SHE WAS RIGHT. READ THIS BEFORE TRUSTING CHECK 6.**
Nova's action log records THAT a server attached a volume, never WHICH volume. So check 6 is
still beatable: attach a THROWAWAY volume to the server you delete, attach lab-vol elsewhere.
Measured four ways before conceding -- action detail has no volume id; os-volume_attachments
and servers/{id} both 404 once deleted; Cinder /attachments lists current records only (max
microversion here is 3.71). My first /attachments 404 was my own instrument: `_os` sends a
Nova microversion header and never a Cinder one.

So the CLAIM shrank to fit the witness, on the page and in the check's desc: "a server you
attached a volume to has since been deleted". The predicate is unchanged and is still strictly
stronger than the timestamp compare it replaced. **BUG-058 is NARROWED, NOT CLOSED.** Closing
it needs a seeded first server (Chris: duplicates Rescue) or a grader-side observation ledger.
**That is an operator decision and it is still open.**

**State:** `qc-lab.sh cinder` GATE PASSED, `qc-lab.sh rescue` GATE PASSED (regression on the
shared /verify). bc2 bridge + bc1 lab-manager restarted and verified; both files archived first
(`*.bak-bug058-*`). Commits `d8225021e`, `fec35aa81`.

**Two honest-student false negatives Nancy caught, both fixed:** the 24h window was measured
from NOW every call, so a student who built their first server Friday and checked Monday lost
it permanently -- now a relevance floor (oldest volume the slot holds), proven on student-09's
7-day-old server-a. And the cap sorted newest-first, evicting the FIRST-built server, which is
the one check 6 needs.

**Chris BLOCKed the deploy on two page defects, both fixed:** the cinder page's step 4 still
said the grader "compares it to your evidence" three lines above the sidebar I had corrected to
say the opposite; and "docked terminal" was FALSE for 3 of 8 pages (launch-chain, neutron,
secgroup put the terminal AFTER the whole steps card, so sticky did nothing). All 8 now render
DOCKED at 1600px, verified by rendering over HTTP with AccessGuard blocked -- the first render
reported "no terminal card" on all eleven pages, which was the guard swapping in the sorting
prompt, not a finding.

**Fixing check 6 unmasked check 3.** Stage 3 had never been reachable, and the first time it
ran it reported check 3 as PASS 4x FAIL 0x -- never once observed refusing anything. Cheat 0
fixes that for this lab. THE SAME QUESTION IS OPEN FOR EVERY OTHER LAB whose gate has never
reached stage 3.

**Deploy state:** `231145d86` (8-lab layout), `d8225021e` (check 6 objective text), `fec35aa81`
(claim walked back + dock order on 3 pages), `8b5d4669e` (this SITREP) and the wording fixes
from Nancy's PAUSE. Verified on preview channel `bug058-labs`; see below for whether the
production deploy has run.

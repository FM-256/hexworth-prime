...
**Auto-updated:** 2026-08-19 16:41 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- c9b95b61b Guests stayed stopped after the power loss; started them and made it self-heal
- 3ae2cbb2e Dash hygiene: rewrite the punctuation rather than swap dash forms
- 69851e6fe The withdrawn claim survived in the one line students actually read
- 0efa09f58 Round 2 of Chris: I traded one bug for a worse one, and shipped a false claim
- aefea5865 Console lab + docs, and the two defects Chris found in the panel
<!-- AUTO:END -->

**Manual section updated:** 2026-08-19 20:40 EDT - OpenStack fully recovered; web console live; board near-clear.

## ★★ NOW - BOARD IS CLEAR OF STUDENT-FACING WORK. Next: Hexworth Qualifier CTF (Windows).

| # | item | state |
|---|------|-------|
| 1 | OpenStack nova-compute dead 15h | FIXED, verified |
| 2 | All 20 devstack units had no Restart= | FIXED, proved by kill -9 |
| 3 | Monitor green through the whole outage | CLOSED, bc2 compute probe in prometheus |
| 4 | Guests stayed SHUTOFF after power loss | FIXED - 7 started, 7/7 ACTIVE |
| 5 | Will it recur on reboot | NO - resume_guests_state_on_host_boot=True |
| 6 | 83 + 11 quiz keys wrong | FIXED, 0 students harmed |
| 7 | OpenStack web console for students | LIVE + gated + lab + docs |

**OPEN, none student-facing:**
- ~1140 questions in the repaired quiz batch never individually read. The content audit missed
  8 of its 11 real defects in its own confident tier, so its silence is WEAK evidence. Zero
  student attempts on those quizzes, so no urgency - but NOT closed.
- 13+ commits unpushed. This repo is PUBLIC; pushing is the operator's call.
- Console BROWSER leg unverified: panel render -> cookie mint -> click-through needs a real
  signed-in session. Components all proven separately; the assembled flow is not.
- Debris the operator owns: aclverify-srv + 4 other aclverify resources; demo-instance and
  canary-admin-project are deliberately stopped (2026-07-29) and must NOT be started.
- Operator network blocks the ENTIRE tailscale.com domain, so bc2/neon are unreachable from
  the admin node. bc1 stays reachable over cloudflared. Emergency bc1->bc2:22 grant was used
  once with approval and REVERTED byte-for-byte the same session.

**NEXT UP - first ever Hexworth Qualifier CTF, Windows-based. SCOPING, nothing built.**
Existing: Arena tournament UI (lobby/board/broadcast/podium/spectator), BoxEngine, 241 boxes in
functions/box_flags.json, server-side validateFlag. Hard constraint: the only sandbox images are
linux-mastery and linux-sandbox, and the OpenStack per-student quota is 1 vCPU / 192MB - no
Windows runtime exists and the current cloud cannot host Windows VMs at scale.

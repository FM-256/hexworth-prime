...
**Auto-updated:** 2026-08-24 23:50 EDT | branch: master | dirty files: 64

### Latest commits (auto)
- 6da9327d4 Punctuation in my own comments, fixed rather than forced past the gate
- 20af0ceae The control came to the instruction, instead of the instruction chasing it
- b5eb3ecd9 SITREP: lab-day status first - what works on student creds, what does not, and why noVNC waits
- 216517acb The one click a student cannot take back was off-screen on a school laptop
- e9dc4d760 BUG-058 actually closed: the grader writes down what only it can see
<!-- AUTO:END -->
...
## LAB DAY (2026-08-25). Read this block first.

**Operator: "we need to have a successful lab day tomorrow."**

### VERIFIED WORKING on STUDENT credentials, no admin account
Cloud Security Sprint, all four missions, cold-run with `sprint-student-walkthrough.sh`:
- M1 nginx serving; peer loads the page; `nmap` sees 2 open ports
- M2 SFTP lands; proof file ON the Cinder volume
- M3 Flask unit active; `{"status":"healthy"}`; consumed cross-project by the peer
- M4 honeypot: GETs=6, LOGIN_ATTEMPTS=4, usernames captured, password_leaked_MUST_BE_0=0

Horizon dashboard: a pool slot logs in for real (POST -> 302, authenticated pages render).

### THE INSTANCE CONSOLE IS FIXED (was broken since it shipped)
Horizon repeats Nova's `novncproxy_base_url` back to the browser, and it pointed at the VM's
PRIVATE address on 6080. No student could route to it. The console PAGE loaded and only the VNC
view stayed black -- nothing 404s, nothing errors, it just never paints, which is why it went
unnoticed.

Chain now: browser -> cloudflared -> traefik -> **vnc-proxy nginx on bc1 (new)** -> bc2 tailnet
forward -> VM apache `/novnc/` (`upgrade=websocket`) -> nova-novncproxy. Behind the SAME console
cookie as /dashboard (anonymous = 401). Rebuild/verify:
`_tools/openstack-bridge/setup-novnc-console.sh` and `verify-novnc-console.js`.

**TWO TRAPS -- do not lose these.**
1. **curl is NOT a sufficient test.** It sends no Origin header, so Nova's origin check never
   fires: I had 101 Switching Protocols AND the RFB 003.008 greeting through the full public
   chain while every real browser was closed with "Origin header does not match". Needed
   `[console] allowed_origins`. **Test with a BROWSER.**
2. **The websocket is ROOT-relative.** noVNC builds `protocol://host + '/' + path` and Horizon
   passes `path="?token=..."`, so it dials `wss://host/?token=...` whatever prefix the page is
   under. Hence a second traefik router on the bare path + token query with addPrefix.

It tunnels through the already-permitted tailnet port rather than exposing 6080: the ACL
between bc1 and bc2 allows only two ports (measured -- the others time out). **Widening that is
an operator action in the Tailscale admin console**, and would let this be simplified.

Verified as a student: Horizon login -> instance -> Console tab -> "Connected to QEMU", boot log
and login prompt. Screenshots taken.

### Instrument failures worth remembering
- I reported Horizon unreachable. My URL was wrong: the code's fallback is `sandbox.hexworth.dev`,
  the real domain is **`.tech`**.
- A 401 from `/dashboard` is the CONSOLE GATE working, not an outage.
- I measured the cinder lab's layout with the terminal CLOSED and reported it as fixed; the tell
  was 221px identical at every viewport height, which no vh clamp can produce.

...
**Auto-updated:** 2026-08-10 21:14 EDT | branch: master | dirty files: 109

### Latest commits (auto)
- d5bef98e8 #307 closed: createRSV teardown proven
- cf3e28bf2 createRSV returns destroy(), and a test that reproduces the bug before fixing it
- bd5366c85 Close #302 and #308: both verified done, board was stale not the work
- ffa1c8cf0 #308 closed: m6/m8 walk-downs verified booting in production
- c2126e7fe #306 CLOSED: reveal gate armed and verified in production
<!-- AUTO:END -->

**Manual section updated:** 2026-08-10 16:20 EDT. Acts LIVE. Board reworked. #306 built, NOT armed.

## NOW: fly-or-gateway is the first choice on the board. Deploying.
   Operator opened the box on production: "why is it so hard to find... why do I have to
   complete the level one before having access to the index and why is the fly component
   inside the box? from the index I should choose fly or gateway."

   THERE WAS NEVER A GATE ON MISSION 1. arena/index.html:1378 lists the box status:'available'
   linking straight to the index; nothing checks a flag. The perception came from LAYOUT: the
   only route to a sortie was a link inside mission 1's card, so mission 1 read as the door.
   The perception was right even though the mechanism was not there. Worse, the two walk-down
   environments were not on the board AT ALL, only inside a mission's evidence panel.

   FIX (3f51096ff + c7b61ea8f): board opens with two panels, FLY (Line of Sight, Antenna Farm
   Walk-down, Rack Cable Count) and WORK THE GROUND SEGMENT (Platform Control, Terran Gateway,
   Ground Segment Console). Sorties declared ONCE in config-shared.js so board, mission cards
   and act tiles read one source. Zone links derive from cfg.zones. Chris PASS, screenshot-
   confirmed as the first content block; his one follow-up (mission 12's card had no Fly link
   because the lookup keyed on `mission` not `satisfies`) fixed before deploy rather than
   logged, since a findability gap on the findability fix is not one to defer.

## ✅ LAGRANGE BOARD CLEAR 2026-08-10 21:10 EDT. #302 #306 #307 #308 all closed and verified.
   #307 createRSV().destroy() DEPLOYED and live. Named handlers, removes all five listeners,
     clears held keys, idempotent. rsv-teardown-test.js 6/6, which REPRODUCES the double-apply
     with two live instances BEFORE proving destroy fixes it: a teardown test that only ever
     shows green cannot tell "destroy works" from "the second instance never listened".
     Chris ran his OWN mutation (disabled one removeEventListener -> 4/6) rather than trusting
     mine, and cleared the TDZ question (onKeyDown calls killRel only in its callback body).
     NOT called from the two live pages, deliberately: both restart via location.reload(), so
     a call there is unexercisable by any test and would read as load-bearing dead code.
     Honest scope boundary, flagged not hidden: destroy does NOT drain the command queue,
     release pointer lock, or reset recall state. Zero production call sites today, so not
     load-bearing; revisit when a shell first reuses an instance.
   #308 m6/m8 walk-downs verified booting in production 8/8. #302 was a stale BLOCKER on
     shipped work (42 flags, 27 aliases seeded). Neither needed new work, only checking.
   POST-DEPLOY: destroy live, both walk-downs 8/8, thermal sortie boots clean, 0 page errors.

## ✅ #306 CLOSED 2026-08-10 20:35 EDT. GATE IS ARMED AND VERIFIED IN PRODUCTION.
   armedAt 2026-08-11T00:35:16Z · mission_gates/le-01-cold-horizon · 12 gates.
   Hosting deployed with the client wiring (Chris PASS on 258dda142, post-verify PASSED).

   VERIFIED ARMED, AGAINST DEPLOYED PRODUCTION, IN THIS ORDER:
     UI test 3/3   clicking Test on the real page records both findings; the earned
                   corroborator reports. This is the check that caught the wiring never
                   having shipped, and it CANNOT pass while disarmed (recordMissionFinding
                   throws not-found with no gate doc), so it must run armed.
     browser e2e 9/9  the CORRECT flag on an empty ledger is REFUSED with nothing credited;
                   a FALSE grouping is refused; both findings and the corroborator record as
                   NESTED maps; the SAME flag after the work is CREDITED.
   Mallory's original finding, a flag submitted on frame one being credited, is now refused.

   ROLLBACK, one command, drilled before it was needed:
     node functions/seed-mission-gates.js --disarm      (archives the doc, then deletes it)
   Code fails open with no gate doc, so disarming restores pre-#306 behaviour instantly.

   NOT GATED, by design, behaving exactly as before: m1, m12, m13. Their second necessary is
   not an axis comparison (m3-style physical bounds, no shared axis among sensors), and
   gen-mission-gates.js REPORTS them rather than guessing a spec that would reject correct
   answers.

## HOW IT WENT, because the failures are the reusable part
   ARMED AND DISARMED TWICE before this stuck. Production never left broken; each rollback
   was one command because the drill was run BEFORE it was needed.
   ARM 1: set() does not expand dot notation the way update() does, so findings landed as a
     field literally named "findings.x" while evaluateGate read progress.findings. Armed, a
     student could do ALL the work and still be refused, on 12 missions.
   ARM 2: the client wiring had never been deployed to hosting. Everything since 4c623d3f7 was
     --only functions / --only firestore:rules. The API passed 9/9 against a game that could
     not satisfy it.

   ⚠⚠ EVERY ONE OF THE FOUR FAILURES WAS AN INSTRUMENT THAT COULD NOT DETECT WHAT IT CLAIMED:
     1. asserted "the gate no longer blocks" with a WRONG answer, but validateFlag checks
        correctness first and the gate sits inside if (isCorrect). Could not fail.
     2. page.select on an option that does not exist is a SILENT no-op, and the distractor
        sources are act-gated. A deliberately sabotaged build passed 8/8.
     3. derived only pairs that SHARE an axis, so independenceOf never returned ok:true and
        NEC[1], half of every gate, was never executed. 36/36 meant nothing about it.
     4. dismissed claim-poisoning as "a wasted call" without testing it. It rejected TRUE
        findings, on comparisons the missions' own text invites.
   Chris caught 2, 3 and 4, and re-proved non-vacuity with HIS OWN sabotage rather than
   trusting mine. The lesson is not "test more", it is: before trusting green, ask what input
   would make this check fail, and confirm the harness can actually produce it.

## SUPERSEDED 19:40 status
   ARMED TWICE TODAY, DISARMED BOTH TIMES INSIDE A MINUTE. Production never left broken.
   Rollback was drilled BEFORE arming (arm, disarm, confirm gone, re-arm), which is why each
   response was one command.

   ARM 1 found a real bug: recordMissionFinding built update['findings.'+id] and called
   set(...,{merge:true}). set() does NOT expand dot notation the way update() does, so Firestore
   held a field literally named "findings.three-channels-one-front-end" while evaluateGate read
   progress.findings and got undefined. Armed, a student could do ALL the work and still be
   refused, on 12 missions. FIXED: both write paths build nested objects.

   ⚠⚠ AND MY OWN TEST COULD NOT HAVE FAILED. I asserted "the gate no longer blocks" using a
   WRONG answer, but validateFlag checks correctness FIRST and the gate sits inside
   if (isCorrect), so a wrong submission returns before the gate is consulted. It went green
   while the ledger was broken. Only reading the doc back out of Firestore caught the bug. The
   harness now reads the real flag from flag_registry and submits it TWICE: refused on an empty
   ledger, credited after the work. 9/9.

   ARM 2 found the client wiring was NEVER DEPLOYED TO HOSTING. 4c623d3f7 landed after the last
   hosting deploy and everything since was --only functions / --only firestore:rules. Prod
   gateway.html has zero reportTest/waitForAuth; LagrangeEngine.js has no reportFinding. The API
   e2e passed 9/9 and would have left the gate armed against a game that could never satisfy it.

   CHRIS BLOCKED TWICE, both load-bearing things I ASSERTED instead of testing:
   1. reportTest accumulated every dependent pair into ONE set regardless of which axis made it
      dependent, so one unrelated pair poisoned every later claim for the session. The missions
      INVITE that pair (m3's text: "gs-maser shares a pipeline with the ranging fix"). A player
      following the game's own instruction had their TRUE finding rejected. I had called this
      "a wasted call". FIXED: buckets keyed by AXIS AND VALUE, unpoisonable by construction.
   2. Coverage: the harness drove m2/m3 and inferred the other ten. FIXED: cases DERIVED from
      the spec, all 12 gated missions driven, 36/36, including drone (m8) and satellite (m5).

   DERIVED FACT, previously inference: 6 of 12 missions have NO poisonable pair at all.
   Also fixed (Chris c): buckets resent every click on an endpoint with NO rate limit, unlike
   validateFlag. Now resent only when grown. MEASURED: m2 12->7, m3 10->6, m9 12->8 claims.

   ⚠ THE TEST WAS VACUOUS TWICE BEFORE IT WAS REAL. page.select on a missing option is a SILENT
   no-op, and the distractor sources are each mission's EARNED corroborator, which the act layer
   hides until obtained. A deliberately sabotaged build passed 8/8. Now it seeds the act and
   asserts selectability first. Chris re-proved non-vacuity at a STRICTER bar than mine, using
   the original accumulator from git history rather than trusting my sabotage variant.

   NEXT: Chris pass -> deploy HOSTING -> re-run the UI test against deployed prod -> then arm.

## SUPERSEDED 16:55 status, kept for the reasoning
   DEPLOYED: recordMissionFinding is live (v2 callable, confirmed via functions:list, not
   inferred from a deploy log), validateFlag redeployed with the gate check on BOTH crediting
   paths, firestore.rules live with users/{uid}/mission_progress read-only to clients.

   VERIFIED UNARMED: mission_gates/le-01-cold-horizon does NOT exist, so assertGateSatisfied
   returns null and behaviour is byte-identical to before. Students see no change.

   RULES PROVEN 11/11 (_tools/rules-test/mission-progress-rules.test.js, emulator, real rules
   file). The owner may READ and may not create, update, merge one finding, nominate their own
   corroborator, or DELETE (a ledger that can be reset is not a record). Others and anon: no
   read, no write. Admin SDK still writes, so the gate is secure rather than unsatisfiable.
   ⚠ First run failed "Not Found" which reads like a rules error and is not one: firebase.json
   puts the Firestore emulator on 8181, not 8080.

   CLIENT WIRED (4c623d3f7): independence tests report the PAIR compared, sorties report the
   corroborator obtained, server re-derives. GATED NO LONGER MEANS WRONG, which was a real bug:
   a gated submission returns correct:false and the old handler docked wrongAnswerPenalty and
   told a player with the RIGHT answer they were incorrect.
   ⚠ Corroborator reporting waits on FirebaseAuth.waitForAuth. Calling it from boot LOOKS right
   and is useless: auth initialises async, isSignedIn() is false for the first moments, the
   report fails soft and the corroborator silently never registers = a locked box that looks
   healthy.

   HARDENED 55b2c276b+: the mission_progress read inside assertGateSatisfied was unguarded.
   validateFlag is the submission path for EVERY CTF box, so a transient Firestore error there
   would have broken flag submission platform-wide. Latent (never runs unarmed) but fixed
   before arming, not after.

## ⚠ ARMING IS THE ONLY STEP LEFT AND IT IS THE DANGEROUS ONE.
   Seeding mission_gates/{boxId} makes 12 missions require a real evidence ledger. If anything
   in the browser -> recordMissionFinding -> Firestore chain is wrong, those missions become
   UNCOMPLETABLE for every student. NOT YET OBSERVED END TO END: unit tests cover the evaluator
   thoroughly and the rules test covers the rule, but no finding has been watched through the
   full path, because that needs the gate seeded.
   PLAN: seed -> verify one mission by hand immediately -> Mallory (her frame-one submission is
   the exact thing to re-run). ROLLBACK: delete the single gate doc, instant disarm, code fails
   open by design. Archive the generated JSON first per never-destroy.

## SUPERSEDED, kept for the reasoning: #306 as originally built
   functions/mission-gates.js (pure evaluator), test-mission-gates.js 21/21,
   gen-mission-gates.js (derives the server spec from the mission data), validateFlag wired on
   BOTH crediting paths, new recordMissionFinding callable.

   The server never accepts a client CONCLUSION. The client reports an ACTION ("I compared
   these sources") and the server re-derives the finding from its own copy of the provenance.
   Derives 12 of 15 missions; m3, m12, m13 are REPORTED not guessed and emit no gate (m3's
   second necessary is a physical bound from ranging, no axis comparison expresses it).

   ⚠⚠ SEEDING mission_gates/{boxId} IS THE ARMING STEP AND WOULD BREAK THE BOX TODAY. The
   client does not yet call recordMissionFinding, so seeding makes all 12 gated missions
   UNCOMPLETABLE for every student. Code is safe deployed (no gate doc = fails open, exactly
   today's behaviour). Seeding is NOT safe.
   REMAINING BEFORE ARMING: client wiring in gateway.html to report independence tests and
   earned corroborators, firestore.rules for users/{uid}/mission_progress, emulator e2e, and
   MALLORY to confirm the frame-one submission is now refused (her finding opened #306).

## MISSION ACTS. Every z1 level now makes you GO AND GET the evidence.
   Operator, 2026-08-10: "fix the levels they all need us to move something a satellite, a
   drone, what about the terminal from control room" and "we need playable and completable
   levels. this is an open world concept that needs multiple working environments".

   THE DIAGNOSIS WAS MECHANICAL, NOT AESTHETIC. Every z1 level was pick-a-card. The mission's
   revealGate demands a corroborator from the PHYSICAL family and the game handed it over as a
   tile. Mission 1 felt different only because it is the one level where you fly out and take
   the reading yourself. Related and already proven: #306, Mallory 2026-08-09, a player can
   type a flag and Transmit on frame one with zero independence tests.

   THE BUILD. A corroborator may declare `earnedBy`. Until the act is performed it renders
   NOT YET OBTAINED with the value WITHHELD (showing it would make the trip decoration) and
   cannot be selected for an independence test. 14/14 levels have one:
     terminal  9   m2 tm BADGE · m3 ranging · m4 sdls · m7 tm BMC-SEL · m9 tm LAYER-HASH
                   m10 tm TAPE · m11 tm REPLAY · m14 tm SEAL · m15 tm TLS-FP
     satellite 2   m5 tc PING_UPLINK · m13 tc SAFE_MODE   (light-time delayed)
     drone     3   m6 · m8 · m12
   New: `acts.js` (act layer + per-mission console platform), gateway lock/unlock rendering,
   console standing-orders + completion detection, sortie records drone acts.

   ⚠ THE CONSOLE COULD NOT ANSWER MOST OF THEM. `tm` knew only mission 1's thermal points, so
   nine levels would have dead-ended on "no such telemetry point". Fixed via the component's
   existing `options.platform` seam, per mission. Every injected reading carries provenance
   INDEPENDENT of the spacecraft downlink (badge via moc-badge-system on a facility CA, not
   ka-1) because that independence IS the lesson and had to be real in the data.

   GATES: acts-test.js 67/67, 0 runtime errors. Covers lock, value-not-leaked, not-selectable,
   orders shown, command completes, level unlocks, for all 11 console/satellite levels. Plus
   the adversarial case: a REFUSED telecommand does NOT credit the act. Drone act verified
   separately end to end (12:ir-survey records, kind=drone, 0 page errors).

   ⚠ CHRIS BLOCKED THE FIRST ATTEMPT AND WAS RIGHT. #308 is now CLOSED by 0af916c4f.
   m6 and m8 briefly credited against mission 1's THERMAL survey: a player sent to establish
   an RF topology or a cable count flew a panel-temperature mission and was handed "seen with
   your own optics" for something never seen. I had picked stand-in over dead end because the
   ask was COMPLETABLE levels. Wrong trade. A level that credits you with something you never
   observed is not completable, it is counterfeit, and this repo has an explicit rule against
   accepting that debt (feedback_no_architectural_debt.md).

   FIX: _app/houses/cloud/games/lagrange-inspect.html, one page holding TWO environments,
   composing lib/orbital-scene.js + lib/rsv-flight.js:
     m6  antenna farm walk-down. Follow each feed run to its front end. Three converge on
         KA-FE-1; the S-band omni is the only independent chain, at 48 bytes a minute.
     m8  rack cable count. Port 14 carries no cable while the controller counts it.
   Findings are DERIVED from what was inspected, never asserted, so changed geometry changes
   the finding rather than silently lying. The provenance had already asked for this: both
   corroborators are stamped `ops-walkdown` and m8 reads "a human wrote this on the deck".

   THE BUG WAS ROUTING, SO THE GUARDS ARE STRUCTURAL. cloud-cold-horizon.html credits exactly
   one act by ALLOWLIST (`ACT_SATISFIED_HERE = '12:ir-survey'`), replacing a truthy check on
   `act`. lagrange-inspect.html refuses an act it has no environment for and halts.

   GATES NOW: acts-test.js 77/77, 0 runtime errors, covering all 14 levels plus: finding
   MATCHES the corroborator it credits, routes to its OWN environment, unknown act gets no
   environment, unknown-act guard halts, REFUSED telecommand does not credit.

   NEXT: #306 server-side revealGate (deploy 2, operator-sequenced). Note the act layer is
   localStorage and shapes the path without defending it, so the levels now LOOK gated while
   credit still rests entirely on server-side validateFlag. A mechanism that looks like a
   control without being one RAISES the value of wiring revealGate, it does not lower it.

## FLIGHT ENGINE EXTRACTED (1d878afe1). Committed, NOT deployed.
   The RSV flight model left cloud-cold-horizon.html for `houses/cloud/games/lib/rsv-flight.js`:
   ship, teleoperation command queue, pointer-lock look, [X] kill-relative-velocity, Newtonian
   integration, sphere collision, NO-BRICK auto-recall. 2381 -> 2271 lines, module is 231.
   Mission keeps what it owns and passes it in: station geometry (collision set), every HUD
   element, every EIDOLON line, every non-movement key. Mission 2 is now a config, not a copy.

   GATES: playthrough 26/26 + 0 runtime errors (identical to the pre-extraction baseline),
   strand-test PASS (auto-recall still tugs a stranded RSV home and re-services it, across the
   new callback boundary).

   ⚠ TWO LESSONS, both worth keeping:
   1. The first edit left the OLD `const ship` above the destructured one. Duplicate lexical
      declaration is a SyntaxError, so the module never evaluated and the QA seam never
      attached. The playthrough died on check #1 and named no line. When an extraction MOVES a
      binding, DELETE the original in the SAME edit.
   2. The thermal check called `toggleThermal()` through the QA seam, so it would still have
      passed with the new onKey wire CUT. It now presses the real 'v'. Proven non-vacuous by
      cutting the wire: 26/26 -> 21/26, IR check fails by name.

## AWAITING DEPLOY (2 commits, verified absent from production)
   - 06d9b9859 sortie -> console hand-off (`hexworth_le01_sortie`): prod grep returns 0
   - 1d878afe1 flight extraction (`lib/rsv-flight.js`): prod returns 404
   Already live and re-verified this session: SecurityTerminal standalone fix (prod ships
   `class SecurityTerminal {`, the only `extends LinuxTerminal` left is the comment at line 36)
   and the Lagrange console zone (console.html 200).

## ⚠ SECURITYTERMINAL NEVER LOADED. Five lab terminals were dead in production.
   cdf7a3869 · DEPLOYED AND PRODUCTION-VERIFIED 2026-08-10 (see the deploy block above).
   Found by accident: I tried to extend it for the Lagrange console.

  THE BUG: `class SecurityTerminal extends LinuxTerminal`, and LinuxTerminal is
  `const LinuxTerminal = (function(){...})()` returning a plain OBJECT. Not a constructor, so
  the extends clause threw at SCRIPT LOAD and the failed class declaration left the name in a
  permanent TDZ where even `typeof SecurityTerminal` raised.

  ⚠ IT LOOKED COMPLETELY FINE. Banner, prompt, blinking cursor, HTTP 200. On the nmap lab a
  student typed "nmap 10.0.0.0/24", pressed Enter, and the output stayed at 591 chars with the
  text still in the box. Silent. No visible error. THIS is why "the page loads" is not a test.
  Affected: eye-exploring-nmap.lab.html + 4 dark-arts bug-hunting labs.

  FIX: standalone, not composed, and the reason is MEASURED. The five labs use nmap/host/whois/
  dig/traceroute/tcpdump and NOT the 60+ Linux commands. LinuxTerminal is also a SINGLETON that
  owns a container selector while every one of these labs overrides _displayOutput to write to
  that same element, so composing would put two writers on one node. Dependency on the phantom
  parent was ONE method (_parseCommand) + 5 properties; the class already defined the other 45.

  ⚠ MY PROBE LIED TO ME TWICE, both mine, both looked like lab bugs:
   1. filtering candidates on `offsetParent !== null` -- which is NULL for any position:fixed
      element -- turned a WORKING lab into a FAIL;
   2. the input selector required /terminal/ and one lab names its input "term-input", so a
      live terminal read as having no input at all.
   Three of five name their terminal element three different ways. A RED result from a probe I
   wrote an hour ago deserves the same suspicion as a green one.

  ⚠ AND THE LESSON THAT KEEPS RECURRING: LagrangeTerminal's own 30-check suite STUBS BOTH
  PARENTS, so it proved my command logic against a fiction and could never have caught this.
  Only running the REAL three-file chain found it.

  PARKED, deliberately kept OUT of this deploy: stash@{0} holds the z2 "Ground Segment Console"
  zone. console.html is NOT built, and deploy ships the WORKING DIR, so shipping it would have
  advertised an active zone that 404s. Restore the stash when building console.html.

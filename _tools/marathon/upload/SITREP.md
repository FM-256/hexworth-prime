...
**Auto-updated:** 2026-08-14 21:15 EDT | branch: master | dirty files: 47

### Latest commits (auto)
- 019dff7e0 SITREP: infra pass complete, four operator decisions outstanding
- 6cabea9b6 Document the ssh access layer: aliases, the neon duplicate fix, and addressing policy
- d77b12863 Enrol bc4 and hexclass on the tailnet; correct two figures I fabricated in the doc
- 837d14ec9 Document the five compute nodes: inventory, addressing, and reboot survival
- d76e4cf04 SITREP: everything shipped and verified; two operator decisions outstanding
<!-- AUTO:END -->

**Manual section updated:** 2026-08-14 (infra pass complete; one admin-console item left).

## INFRA PASS COMPLETE. All five compute nodes reachable by name over the tailnet.
   All five now on the tailnet; addresses and host keys are NOT recorded here.
   ⚠ DOCUMENTED PRIVATELY, NOT IN THIS REPO. The repo is PUBLIC, so node addresses, host key
   fingerprints, internal hostnames and topology do not belong in it. See
   _docs/operations/compute-nodes-inventory.private.md (gitignored via *.private.md) and the
   copy at ~/hexworth-infra-private/. ~17TB free across the estate.

   WHAT WAS BROKEN: bc4 and hexclass were the only two logged out of the tailnet, so they were
   the only two pinned to DHCP LAN addresses, which is the one thing a reboot moves. Both
   re-enrolled. bc4's old config entry was ONE DIGIT off its real address, which
   TIMED OUT instead of failing loudly, and a timeout reads as "host is down".
   Also merged two `Host neon` blocks (ssh takes the first, so the second was dead config). The
   dead one held the BETTER hostname, so it was a merge not a deletion: MagicDNS `neon-server`
   survives an address reassignment where a hardcoded IP does not. `bc3` aliased, port 2222
   recorded as deliberate (sshd refuses 22).

## ⚠ OPERATOR, THE ONLY ITEM LEFT: disable key expiry for bc4 and hexclass in the Tailscale
   admin console. Not CLI-doable. Expiry is the most plausible cause of the original logout, so
   without it both revert to LAN-only in roughly ninety days and this work undoes itself.
...
## OTHER OPERATOR DECISIONS OUTSTANDING
   BUG-119: the Python for IT final tests Obj 1-6 ONLY. Zero questions on Tkinter GUI (the
     ...
     which requires re-seeding quiz_keys/pfi-w4-final-exam or every student is misgraded, or
     accept the narrower scope.
   ...
   From nexus full: HEUR-042 flags 248 files where the correct answer is systematically the
     longest option (one at 10/15 questions vs ~25% by chance). Real assessment-integrity defect.
     QC-57 (95 client-graded quizzes) and ES-1150 (a quiz that grades everyone 0%) sit in backlog.
...
## THE FINDING OF THIS SESSION: WHEN A RESULT LOOKS CLEAN, SUSPECT THE INSTRUMENT.
   ...
   the product change. Then the infra pass repeated it: tailscale status called neon "offline"
   while it answered ping in 4ms, and I reported bc4 down on a stale IP plus a missing peer when
   a one-command LAN sweep found it immediately.
   Detectors that were themselves the defect: tree-mapper (under-reports, 600 nodes -> 31),
   audit-hub-deadrefs-v2 (prints refs=0, filters 'high' when HUB-001 is now 'medium'),
   dash-hygiene-gate (blind to a line-wrap "--", so its clean report was false).
   ...
   asked a regex to do a lexer's job after 3bdbf7a80 that morning ruled "stop parsing with
   regexes". And I committed a node doc claiming "measured, not recalled" that carried two
   fabricated figures, caught by re-deriving after writing.
...
## NEXT
   1. Operator: the four decisions above.
   2. Fix tree-mapper before wiring any gate to it; fix audit-hub-deadrefs-v2's severity filter,
      base resolution and quote handling.
   3. GAP: presentation-level hrefs are not covered by deploy gate 3.2, which reads DATA files
      only. Only the course-tree crawler sees that class, which is the argument for repairing it.

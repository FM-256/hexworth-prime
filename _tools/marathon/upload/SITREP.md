...
**Auto-updated:** 2026-08-13 19:15 EDT | branch: master | dirty files: 46

### Latest commits (auto)
- 66dfd4b66 Record the deploy freeze, the Chris block on BUG-107, and two new bugs
- a1cddce05 BUG-107: the learning path said 7, the hub said 12, for the same course
- 0f4ec9a97 BUG-105 RESOLVED, and not by new code
- 76133688a BUG-104 RESOLVED: all three labs live and verified on production
- 6140c3095 advanced-ops tasks 3 and 4: re-derive, do not trust the class the check painted
<!-- AUTO:END -->
...
## NOW: HEXWORTH IS PARKED. ACTIVE PROJECT IS CHAOS.
   Everything Hexworth below is written down and waiting; nothing is in flight here.
   Hexworth: 83 commits UNPUSHED to origin (never pushed this session, nobody asked).
   Deploy freeze still in force from the class; BUG-107 committed, blocked on a Chris
   re-submission (3 harness fixes listed in its tracker entry). BUG-101/102/108/109 open.

## ★ CHAOS — 16 commits, all pushed to tntxtra/chaos (private). Phase 0 CLOSED.
   Repo /home/eq/ai-content/chaos · forks at /home/eq/ai-content/chaos-forks/
   Read chaos/docs/chaos-scope.md then architecture.md. Do NOT re-derive from here.

   BOTH FORKS READ AND DECIDED. oss-crs is the SUBSTRATE (pinned 905a011b6, our fork is
   at exactly upstream main, no divergence). crs-2/ACDAN contributes DESIGN ONLY: all 35
   files read, and its RAG context never reaches the LLM, its errors return HTTP 200 with
   risk_level CRITICAL, and its DQN has four reachable inputs. CHAOS drives the oss-crs
   CLI FROM ABOVE, because a correlation layer that can only see its own container cannot
   correlate.

   THE THESIS, MEASURED NOT ASSUMED: oss-crs dedups by FILENAME (exchange/main.py:9,
   "files are already hash-named"), composes CRSs statically, and has zero prioritisation
   or decision logic (control-tested search: priorit 0, rank 0, decide 0, strategy 0).
   That gap is CHAOS.

   BUILT AND TESTED, 142 GREEN, stdlib only:
     campaign.py     33  authorization + spend ceiling + audit trail
     signature.py    21  crash correlation across engines, over-collapse guarded
     driver.py       26  the oss-crs seam, injected executor so no Docker needed
     orchestrator.py 28  the spine end to end; asserts ORDER, not just outcome
     inventory.py    34  supply-chain gate, 153 artifacts / 130 pinned, FAILS by design

   V0.1 TARGET PINNED: libpng CVE-2018-13785, vulnerable a74aa9a, fixed 8a05766. Chosen on
   REACHABILITY (harness read) not fame; c-ares eliminated because neither of its two
   OSS-Fuzz harnesses reaches the CVE. Build it with UBSAN not ASan: a divide-by-zero is
   not a memory error.

   ⚠⚠ NOTHING HAS BEEN RUN. Every suite is against FIXTURES. Machinery tested only against
   fixtures is the exact ACDAN failure. NO CLAIM THAT CHAOS FINDS BUGS until the
   ground-truth run happens, and that needs the deferred build host ("we will add a server
   later"). Also unsigned: 4 base images carry digests and no verdict, 2 of them 422 days
   old on an EOL distro.

   NEW REPO: tntxtra/chaos (PRIVATE), working tree /home/eq/ai-content/chaos.
   Forks cloned for assessment at /home/eq/ai-content/chaos-forks/{oss-crs,crs-2,oss-fuzz}.
   CHAOS is a cyber reasoning system; it has its own docs and its own board. Read
   chaos/docs/chaos-scope.md first, then trust-model.md. Do NOT re-derive it from here.

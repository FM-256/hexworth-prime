...
**Auto-updated:** 2026-08-07 22:01 EDT | branch: master | dirty files: 105
...
**Manual section updated:** 2026-08-07 23:40 EDT — Trial Run M1+M2+M3 all PASSED. M4 is next.

## NOW: Trial Run — M3 passed. Next milestone is M4 (the only gate that matters).

  ~/hexworth-shared/carreer_launchpad/trial-run/    spec: ../TRIAL_RUN_BUILD_SPEC.md
  Gate artifacts: docs/m1_gate.md · m2_gate.md · m3_gate.md · m3_review.md (40 transcripts).
  In hexworth-prime only one thing changed: the Career Launchpad card (36f973e3a),
  coming-soon, deliberately not a link.

  Run it:  ALLOW_STUB_LLM=1 .venv/bin/uvicorn api.main:app --port 8137   then http://127.0.0.1:8137
  Tests:   ./run-tests.sh  (7 suites, no key, all passing)
  Key:     grep ANTHROPIC_API_KEY ~/job-campaign-stack/.env    (Lodestar's, works)

  Before M4, two things carried forward and written down, not silently fixed:
  1. Every one of 40 questions opens "You said / You mentioned". Robotic. The relevance
     metric REWARDS the tic. Fixing persona_em.md invalidates the M3 numbers (~$1.31 to
     re-measure). See docs/m3_gate.md.
  2. claims_verify.md rule 1 runs over-generous — par-09 claimed production experience the
     corpus never mentions and got partially_supported. See docs/m2_gate.md.

  Waiting on you: hosting (spec §13.2) before the card can become a link; and whether to
  sweep em-dashes from all career page titles or exempt them from the gate
  (career/index.html trips it on 3 pre-existing ones).

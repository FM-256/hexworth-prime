...
**Auto-updated:** 2026-08-14 17:34 EDT | branch: master | dirty files: 46

### Latest commits (auto)
- a6f5b8c80 BUG-118: remove the stale-count checker. It needed a lexer, and I wrote a regex twice.
- e517a0bc5 BUG-118: my own stale-count checker had the bug it checks for, and deploy.sh had a false one
- d17bf8ec6 BUG-118: two MORE stale counts in the gate, and a real check so it stops recurring
- af980949e BUG-118: fix the stale 74 in the gate's own comment — where the number started
- 8a5c36de7 BUG-118 tracker: record the 6 fixes, and the number I shipped after disproving it
<!-- AUTO:END -->

**Manual section updated:** 2026-08-14 (BUG-118 redirect fixes awaiting a Chris PASS; round 6).

## ✅ SHIPPED + VERIFIED LIVE EARLIER TODAY: BUG-115 + BUG-116. 689/689 URLs return 200 on prod.
   12 forensics hrefs and 66 LearningPaths hrefs were dead; both fixed and deployed. Deployed
   data files fetched back from hexworth.com and confirmed byte-identical to local.

## ⏳ NOW: BUG-118 FIX IS READY AND HAS BEEN CORRECT SINCE ROUND ONE. Chris round 6 running.
   Operator said "go ahead and deploy it". 20c554fab: five meta-refresh stubs that returned 200
   and then bounced the student onto a 404, plus one dead post-lab returnUrl. Chris verified the
   _app change on round 1 and every round since -- six files, byte-identical, ablation-tested.

   ⚠⚠ SIX ROUNDS, AND EVERY SINGLE BLOCK HAS BEEN IN MY CLAIMS, NEVER IN THE FIX.
   r1 the gate covered 2 files while deploy.sh said "every hub href" (95 of 5,727, ~2%)
   r2 commit said "74 stubs" -- the RAW GREP count, including two quizzes that merely TEACH
      the meta-refresh tag. The gate said 72 the whole time. Excluded from the CODE, not the PROSE
   r3 tracker still said "open, 4 redirects" after 5 were fixed
   r4 ArcticData documented as 79 hrefs (never fewer than 359 -- wrong the day it was typed)
   r5 my stale-count CHECKER contained the bug it checks for: its own implementation line held
      3 raw backticks, flipping pair-parity so an injected false claim passed silently
   r6 and after deleting that checker I still called the file clean while :71 read
      "this file has 711 double-quoted hrefs"

## ⚠⚠ THE LESSON, AND IT IS THE SECOND TIME TODAY.
   I wrote a regex to lex JavaScript comments. Five rounds, five parsing gaps: backtick parity,
   double-quoted narration, one stray backtick swallowing 370 chars including a planted claim,
   `case 1://comment` glued to a colon, and an exemption anyone invokes by typing a backtick.
   3bdbf7a80 THIS MORNING: "BUG-107 round 5: stop parsing with regexes. Let the browser do it."
   Chris then: there is no round that ends it, because the list is the grammar of two languages.
   I had it written down and repeated it INSIDE THE FILE THAT DOCUMENTS NOT MAKING UNVERIFIED
   CLAIMS. Checker REMOVED, not patched again -- an unsound check that can emit PASS is worse
   than none, it manufactures confidence exactly where the evidence is weakest.
   Rule saved: memory/feedback_a_count_in_prose_is_stale_on_arrival.md -- comments state
   INVARIANTS, the script reports QUANTITIES.
   ⚠ And my ablations lied twice: one planted a value that was actually CORRECT, one was a
   no-op. A green ablation is not evidence unless the plant is verified present AND wrong.

## ⏭ NEXT
   1. Chris PASS -> record-chris-pass.sh -> ./deploy.sh -> verify the 5 stubs on production.
   2. OPEN, needs operator: pfi-w4-gui.quiz.html is linked as "GUI Quiz" from two live
      Python-for-IT presentations and does not exist. That course has w1/w2/w3 quizzes and no
      w4 quiz at all -- nothing to derive. Build it or drop the link.
   3. OPEN: the hub-tree tooling regressed (BUG-118) -- tree-mapper crawls 600 nodes -> 31,
      depth 5 -> 2, losing prev/course-home/returnUrl/next. Its "Broken: 44" UNDER-REPORTS.
      Shallow regen archived, NOT committed. Fix before wiring any gate to it.
   4. OPEN: audit-hub-deadrefs-v2.js prints refs=0 -- filters severity 'high', HUB-001 is now
      'medium'. Went blind BECAUSE the platform improved.
   5. Debris needing operator rm (denied to me AND to Chris): _tools/qa/_chris_ablation_tmp5.js,
      _chris_ablation_tmp5b.js (both archived byte-verified), plus 15 older probe files.
   6. Still open: BUG-101, BUG-102, BUG-108, BUG-109, BUG-112, BUG-113, BUG-114 root question.

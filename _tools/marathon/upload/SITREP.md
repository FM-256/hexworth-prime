...
**Auto-updated:** 2026-08-16 15:52 EDT | branch: master | dirty files: 46

### Latest commits (auto)
- 2a37c11e8 SITREP: seed staging proven end-to-end, and this repo is PUBLIC
- be94de6a3 Personal identifiers out of a repo that is PUBLIC, and a gate so they stay out
...
**Manual section updated:** 2026-08-16 (full loop closed + OSV-matched CYAN on libxml2; exposure gate shipped; repo is PUBLIC).
...
## ★ NOW: CHAOS — THE FULL LOOP CLOSED, CORRECTLY LABELLED. Repo @ fa988fa. Host bc4.

### ✅ END-TO-END ON libxml2, EVERY LAYER HONEST AT ONCE (first time)
...
### NEXT (specified, not built)
   0. ✅ DONE — seed staging now targets the fuzzed build (fbcb0e7).
   1. ✅ DONE — libxml2 ran VERIFIED end-to-end, 2 confirmed CYAN OSV-2025-457 (fa988fa).
      The parser.c fixture was ARCHIVED (evidence/archive/injected-parser-2026-08-16.diff) and
      STASHED on bc4 — `git stash list` has 1 entry, restore with `git stash pop`.
   1b. ⚠ NEW, FROM THAT RUN: decide SIGNATURE GRANULARITY. Two findings shared one site, one
      set of visible frames, and ONE advisory. The count is sampling-dependent (3 -> 2 across
      runs at --max-repro 80 of 289/322). Until this is settled, findings counts are not
      defect counts and must not be reported as such. Options: collapse on advisory id,
      collapse on top frame, or keep call-path splitting and rename the field.
   1c. Raise/remove --max-repro, or make the cap adaptive. 242 of 322 PoVs were never replayed;
      the cap IS reported in the phase name, which is why this is visible rather than silent.

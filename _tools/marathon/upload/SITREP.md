...
**Auto-updated:** 2026-08-25 07:25 EDT | branch: master | dirty files: 69
...
## NOW: everything green on a full suite run (2026-08-25 07:30)

Operator: *"run the full test suite one more time"*. Done. Nothing is outstanding from it.

### The board
| Item | State |
|---|---|
| 6 lab QC gates (chain, cinder, neutron, project, rescue, secgroup) | **ALL PASSED**, run in parallel |
| Sprint 4 missions, STUDENT creds, no admin | **ALL PASS** |
| Horizon dashboard login (pool slot) | **PASS** (302, authenticated pages render) |
| Horizon instance Console tab (noVNC) | **PASS** — fixed today, was broken since it shipped |
| Production (hexworth.com, lab pages, console gate) | 200 / 200 / 401-for-anon |
| bc1 lab-manager + vnc-proxy, bc2 claim + vnc bridge | all up |

Every gate proves three things independently: named cheats fail, the honest walkthrough passes
TWICE, and every check has been seen both passing AND failing. Discriminating checks: 4/4/4/4/3/4.

Sprint evidence: M1 nginx + peer nmap=2 · M2 SFTP landed ON the volume, "Cinder survived" ·
M3 `{"status":"healthy"}` cross-project · M4 GETs=6, LOGIN_ATTEMPTS=4, **password_leaked=0**.

### OPEN — needs the operator, nothing blocked without it
- **Tailscale ACL bc1->bc2 permits only two ports.** Measured: the others time out. The console
  tunnels through the permitted one instead. Widening it in the Tailscale admin console would
  let `setup-novnc-console.sh` be simplified. Also still pending from earlier: approving bc2's
  subnet route.
- **19 pool slots hold leftover resources**, including the operator's own `Killa-B`. Test debris
  from many sessions. NOT touched: deleting is the operator's call.
- **5 of 7 adversarial harnesses have no do-nothing case** (chain, neutron, project, secgroup,
  wall). Only cinder and rescue prove an untouched tenant scores zero. That blind spot is what
  hid check 3 for a month. A sweep, not a fire.

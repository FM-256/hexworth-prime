...
**Auto-updated:** 2026-08-08 13:07 EDT | branch: master | dirty files: 103

### Latest commits (auto)
- 915843c51 SITREP: practice sheet now triages; sample report labelled
...
**Manual section updated:** 2026-08-08 13:05 EDT — Trial Run QC'd live. Manual + roadmap written.

## NOW: Trial Run is feature-complete and documented. M4 needs 3 humans + a consent call.

  ~/hexworth-shared/carreer_launchpad/  — its OWN git repo, LOCAL ONLY, still no remote.
  Serving on http://127.0.0.1:8137 (BYOK required; server key NOT used for users).
  Tests: ./run-tests.sh  — 13 offline suites, no key needed, all passing.
  READ FIRST: docs/MANUAL.md · docs/ROADMAP.md · docs/sample_report.md (a real report).

### QC RUN 2026-08-08 — drove the whole journey over HTTP. TWO BLOCKERS, both fixed.
  1. THE PRODUCT WAS UNUSABLE ON ANY EXISTING DATABASE. POST /session -> 500,
     "table session has no column named preferences". create_all() makes missing TABLES
     and ignores missing COLUMNS, so adding a model field worked on every fresh db and
     broke every existing one, at step 4 of 6.
     ALL 13 SUITES WERE STRUCTURALLY BLIND: each points DATABASE_URL at a fresh temp file,
     so every one tests the ONE config where this cannot happen. init_db() now adds missing
     NULLABLE columns loudly; a missing NOT NULL is reported, never guessed at.
     tests/test_schema_drift.py builds an OLD schema by hand and proves the upgrade path.
  2. A CARRIAGE RETURN SPLIT A TABLE ROW in section 1 of a live report. The artifact
     detector added earlier that day looked for a literal backslash only — but a spurious
     \r is a VALID JSON escape, so json.loads makes a real control char and the backslash
     is gone. Same bug, second costume; the retry never fired. Detector now catches C0
     controls (newline excepted, prose wraps); _cell() collapses all whitespace.

### THE INTERVIEWER IS ACTUALLY WORKING — the reason to run it
  I dodged a question and got back: "I still want the log contents. What error or message
  told you the backend was out of space?"  I overclaimed six years and got: "name one
  specific storage failure you diagnosed and the exact error you saw." One question per
  turn, zero quote-backs. A passed Cinder lab correctly graded UNSUPPORTED for a METHOD
  claim: "your Cinder lab pass shows the outcome only."
  Clean re-run: 0 blockers, report in 54s, $0.57, md + pdf both valid.

### GATE STATE (all live-measured on claude-opus-5 — no other model has been gated)
  M1 PASSED · M2 recall 1.000, agreement 0.946, residual errs STRICT (the safe way)
  M3 grounded 40/40 · one question per turn 40/40 · zero quote-back openers
  M4 NOT RUN.
...
### WAITING ON YOU
  ...
  2. Hosting (analysis in _docs/operations/trial-run-hosting.md, NOT decided). Recommended
     shape: static frontend in _app/career/trial-run/, API on Cloud Run behind a Firebase
     rewrite at /api/trial-run/** — SAME-ORIGIN keeps the Sec-Fetch-Site boundary alive.
     NOT before M4.
...
### TRAPS WORTH REMEMBERING
  - build_client falls back to OPENAI_API_KEY / GOOGLE_API_KEY too. An ambient key sent the
    OFFLINE suites at a real API. THE GATES HAVE THE SAME EXPOSURE — with an ambient OpenAI
    key, M2/M3 would run on GPT-5 while the artifact says claude-opus-5. Both gates PRINT
    the model used; check that line.
  - OpenAI + Google clients have NEVER made a successful live call (no funded key here).
    Only their error handling is proven.
  - A generator that write_text()s a doc a human also edits WILL destroy the human part.
    The M3 gate blanked a recorded manual verdict twice before this was fixed.

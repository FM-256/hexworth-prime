...
**Auto-updated:** 2026-08-14 16:36 EDT | branch: master | dirty files: 46

### Latest commits (auto)
- f1063c778 Flip 3 catalog entries to available: the content was already built and hidden
- 6b931ed29 The "42 broken links" were not broken. The gate was measuring the wrong thing.
- 9537f6a80 BUG-115 + BUG-116 deployed: 689/689 declared links verified 200 on production
...
**Manual section updated:** 2026-08-14 (link integrity DEPLOYED; Forge build is the only real gap).

## ✅ DEPLOYED + VERIFIED LIVE: BUG-115 + BUG-116. 689/689 declared URLs return 200 on prod.
   12 forensics hrefs and 66 LearningPaths hrefs were dead; both fixed, derived from disk, ids
   byte-identical, Chris PASS round 2. Deployed data files fetched back from hexworth.com and
   confirmed byte-identical to local. Link gate broadened 2 files -> 7 files / 5,727 hrefs,
   deploy.sh gate 3.2, no bypass.

## ⚠ THE HEADLINE LESSON OF THIS SESSION: I CALLED GATED CONTENT "BROKEN" THREE TIMES.
   1. df-61 reported as an unreachable module. ContentCatalog links it; it returns 200.
   2. 42 ContentCatalog links reported as "404s a student can hit TODAY" -- I asked the operator
      to choose build-or-delist for two courses on that basis. ALL 42 are status:'coming-soon'.
      Nothing renders them as followable links. Browser-verified: 45 coming-soon, ZERO clickable.
   3. 11 hardcoded anchors in houses/shield/isc2-cc/index.html reported as clickable 404s -- and
      I EDITED THE FILE on that premise. BUG-012's gate sits right below them. Browser-verified
      after reverting: 11/11 dimmed (opacity 0.5, cursor not-allowed), badged, aria-disabled,
      click raises "This module is coming soon." and does NOT navigate. Hub honestly reports
      "47 of 58 modules built - 11 more coming soon".
   Restored byte-identical from the archive taken first. NEVER-DESTROY IS WHY THAT WAS RECOVERABLE.
   Rule written: memory/feedback_check_the_gate_before_calling_it_broken.md -- read the status
   field AND the click handler before calling any dead-looking link a defect.
   ⚠ A PROBE THAT DOES NOT REACH ITS TARGET MEASURES NOTHING: my first click test reported "no
   dialog" because the cards sit in COLLAPSED accordions. Opening them first produced the alert.

## OPERATOR RULING 2026-08-14: a DUP is not an INCOMPLETE HUB. Different things, different remedy.
   DUPLICATES -- archive the pointer, content is already live (27 total):
     - 17 Shield PIS catalog aliases: every topic already taught by a live presentation
       (pis-07 Malware Taxonomy -> pis-w1-malware, pis-17 PKI -> pis-w3-pki, etc.)
     - 9 of the 11 isc2-cc cards: target is ALREADY a working card on the SAME hub
     - 1 key-hashing entry: key-hashing-algorithms is live at 200 (~20 hashing modules exist)
   INCOMPLETE HUB -- build it:
     - Forge "Intro to Computers": 3 of 26. Missing 5 presentations, 8 labs, 8 quizzes, 2 exams.
       OPERATOR SAID BUILD. Proposal at _docs/operations/hub-001-forge-intro-computers-proposal.md
     - pod-crossing: single game concept, cover art only.

## ⏭ NEXT
   1. Scope and build the 23 Forge intro-computers modules (operator directive).
   2. Archive the 27 duplicate pointers (cp to _tools/archive first, verify, then remove).
   3. Open: BUG-101, BUG-102, BUG-108, BUG-109, BUG-112, BUG-113, BUG-114 root question.

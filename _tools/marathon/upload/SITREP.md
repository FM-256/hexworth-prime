...
**Auto-updated:** 2026-08-07 10:33 EDT | branch: master | dirty files: 103
...
- ce44675bb The rail fixes, fixed: answers flash, and a finished card is not yanked away
- 78f16faff The task rail drives itself instead of making the student drive it
- e0913a022 Award the lab's module credit once, not once per Finish press
<!-- AUTO:END -->
...
## NOW: ch25 COMPLETE + LIVE. #294 CLOSED + LIVE. Nothing in flight.

  ALL DEPLOYED AND VERIFIED IN PRODUCTION TODAY
    ch25 deck + 3-stage lab + server-graded quiz (quiz_keys/core2-ch25 seeded, bridge PASSED).
    Lab rail now drives itself: completing a task flashes it, opens the next unfinished one
    and scrolls it into view; free-text answers flash too; re-reading a finished task no
    longer gets yanked away. Verified on hexworth.com, not just locally.
    Core 1 forge-vm-setup.lab.html #294: gates 3 and 4 stopped rubber-stamping.
      Live check: on load ram=512 disk=20, both gates FALSE; still false for Linux untouched;
      true once sized to 2048/30; Start Over returns to 512/20.

  WHAT THE REVIEWERS CAUGHT THAT I DID NOT (the pattern, worth reading before the next task)
    Chris blocked 8 times across this chapter, every one a real defect. Nancy blocked twice.
    Four were the same class: grading that trusts state re-read or frozen at the wrong moment
    (tick cache, model graph, stage flag, name scan). Three more were the same TESTING error:
    my suite was green because it exercised the layer I had just changed and not the path
    around it. The quiz would have scored every perfect student 60% and failed them, because
    gradeQuiz divides by the full bank while the client posts only the drawn subset, and I had
    named that exact risk in my own review brief and then never tested it. #294's first fix
    was defeated by the Start Over button; my 23 checks never called resetLab() once.
    RULE FOR NEXT TIME: after fixing a gate, list every code path that writes the state the
    gate reads, and test from each one. Not just the path I edited.

  OPEN
    #295 gradeQuiz cannot score a drawn subset. Blocks pool-draw delivery for all 415
         serverGrading files. Needs a served-count, a functions deploy and a regression sweep.
    #296 sweep other labs for the finishStage double-completion shape Nancy found.
    Nancy's open design note: s3t7 identity is a student-chosen name. GUI-reachable residual
    is disclosed in-file. She agreed C is the right design and found no design D.

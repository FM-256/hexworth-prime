...
**Auto-updated:** 2026-08-29 00:16 EDT | branch: master | dirty files: 91
...
- fb9cdec6c A lab The Rig sells as practice must be capped as practice
- 818490fc2 BUG-233: the free-play cap is advisory under a burst
- b05fd13a8 SITREP: concurrency moves from NOT PROVEN to tested, broken, fixed
<!-- AUTO:END -->
...
subnet (`192.168.233.0/24`, never 0.0.0.0/0) on all pool projects. TCP still needs the
...
(after the 80-slot extension: `added=30, already-had=50, failed=0`).

### `ssh bc2` fixed — it was never sshd
Admin access to bc2 died repeatedly on 2026-08-28/29 while the cloud stayed healthy, blocking
verification twice. bc2 had not rebooted, 8080/9711 answered throughout, `tailscale ping`
returned in 1ms. bc1 had **no `~/.ssh/config` at all**, so `bc2` resolved via the router's DNS
to a global IPv6 under an **ISP-delegated prefix that the ISP rotated**. DNS returned the old
/64; tailscale was reaching a different one. The map was stale, nothing was broken.

Tailscale is not a fallback: its ACL permits only 8080/9711, so port 22 times out by policy.
`ssh bc2` now goes through `_tools/openstack-bridge/bc2-connect.sh` (`~/bin/bc2-connect`),
which tries LAN IPv4 → second NIC → **MAC-derived link-local** (cannot rotate; the only
address that survived the outage) and takes the first that answers. The DNS name is
deliberately not in the list. Addresses in `BC2_ADDRS` in the private infra env.
Fallback PROVEN, not just the happy path — bogus first addresses fall through to link-local,
and all-bogus fails fast naming what it tried. Restore steps for a fresh bc1 are in the runbook.

### The practice cap did not hold either — BUG-233, fixed and deployed 2026-08-29
Extending the pool fixed `POOL_EXHAUSTED` and exposed a quieter failure: `FREE_PLAY_CAP=32`,
yet 34 simultaneous launches **all** succeeded with zero refusals and not one log line.
Check-then-act — every request counted Docker's *running* list before any of the burst's
containers existed.

**Deployed to bc1** (`server.js` md5 `a72f01ff…`; prior archived at
`_backups/server.js.pre-bug233-2026-08-29T03-32-59Z`). Two parts: an admission lock plus a
reservation map counting admitted-but-not-yet-running launches; and `openstack-cli` moved from
`FREE_PLAY_LABS` to `CONTEXT_FREE_PLAY_LABS` because it has BOTH launch paths.

Verified live: `concurrency-test.js 34` → **34/34** (a class starts) · `… 34 --freeplay` →
**32 admitted, 2 refused**, reproducible (the cap fires). Lock cost measured at 46ms.

**Nearly shipped a worse bug.** Removing `openstack-cli` from `FREE_PLAY_LABS` without adding
it to `CONTEXT_FREE_PLAY_LABS` would have left Rig practice launches UNCAPPED against a graded
class. I missed it by grepping the Rig's HTML for "openstack" — it never names labs, it
projects `getBrowsableLabs()`. `_tools/rules-test/freeplay-classification.test.js` automates
that cross-repo trace now; **neither repo can see the invariant alone.**

⚠ **Do NOT lower `FREE_PLAY_CAP` to the code default of 12** — 12 throttled the 2026-08-25
class. Residual gap, named not closed: 32 of 40 leaves coursework a floor of 8 under
contention. That is a capacity decision, not a bug.

### Concurrency: TESTED, was BROKEN, now fixed
20 simultaneous launches returned **6 OK and 14 `POOL_EXHAUSTED`** — 50 projects, 44 already
bound, six free, six succeeded. A class of twenty could not have started.

**Pool extended to 80.** Same test now: **20/20, 20 distinct slots, 6.4s, no double-assignment.**
Current headroom: 34 free slots against RAM for 35 more instances — balanced on purpose.

- `bash _tools/openstack-bridge/pool-capacity.sh` (bc2) — both ceilings. **Free slots**
  (`POOL_EXHAUSTED`) caps distinct users *ever*, since a slot binds to a uid for life and only
  ever decreases. **Free RAM** (`CLOUD_FULL`) caps instances *at once* and recovers. Either one
  alone refuses every claim; that confusion is what hid this.
- `node _tools/openstack-bridge/concurrency-test.js [N]` (bc1) — N fixed identities, every slot
  handed back in `finally` (verified: pool returns to its exact prior count). Also asserts no
  two uids got the same slot — `claim()`'s lock had never been tested under real concurrency.
- I missed it originally by trusting `dump-slot-uids.py`, which **reports only the first 30
  slots**. A capacity number off a paginated source is not a capacity number.
- `provision-pool.sh` set `--ram 192`, under the 512MB the sprint image needs. Now 512.

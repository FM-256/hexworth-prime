...
**Auto-updated:** 2026-08-19 22:58 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 3a6a68cec mission_gates gets an explicit rules block instead of an implicit default
- 59170734b The flag must not BE a value the gate handles: answer is now the device, not the IP
- 9294e084a Mallory proved a leak I shipped: the success message handed back the flag
- f60704f10 SITREP: qualifier CTF scoping, the flag-integrity find, and what is still unproven
- 91e7c9b48 Seeded the reference competition box; gate proven live, disclosure refusal not yet
<!-- AUTO:END -->

**Manual section updated:** 2026-08-20 01:20 EDT - qualifier integrity spine audited by Mallory and closed; 12/12 live.

## ★★ NOW - QUALIFIER INTEGRITY SPINE DONE AND AUDITED. Next: playable box content.

`node functions/verify-qual-box.js` -> 12/12 against PRODUCTION documents. Run it after any
change to the box, the gate, or the registry.

**THE CHAIN, and what each link stops**
| control | stops |
|---|---|
| deliveryDisabled on flag_registry | deliverFlag handing the value to any signed-in caller |
| mission_gates + recordMissionFinding | a correct GUESS being credited (findings-only is refused) |
| no value echoed in finding reasons | the success message disclosing the answer |
| flag != any gate axis value | one careless interpolation re-disclosing it |
| explicit mission_gates rules block | a future permissive catch-all exposing every axis |
| not-also-a-tournament-challenge | ctfSubmitFlag crediting a bare guess, gate bypassed |

**MALLORY PROVED A LEAK I SHIPPED.** recordMissionFinding returned
`detail: "all share sourceIp=192.168.1.150"` - and that string WAS the flag. One call, fresh
account, no corroborator, no second finding. Root cause was mine and worth remembering: I made
the flag IDENTICAL to a gate axis value, so the disclosure guard on deliverFlag was never the
only writer of "competition boxes never disclose". Fixed at source for every box; answer changed
to the device name (finance-mfp-01), which requires the physical inventory - the corroborator the
gate already demands. The collision check I added FAILED ON ITS FIRST RUN, which is the point.

She also PROVED the claim I could not: deliverFlag returns 403 for the competition box and still
delivers normally for the teaching box.

**CLEARED, not exploitable** (proven, not reviewed): direct Firestore reads of flag_registry and
mission_gates (403), direct mission_progress writes (403), validateFlag leaking on wrong/gated
submissions, BoxEngine prefetch (only path is deliverFlag), wrong-family corroborators (recorded
but inert - evaluateGate re-checks independently).

**OPEN**
- No playable UI for qual-w1-lockout yet. The spine is built and audited; no scenario content.
- ~1140 unread questions in the repaired quiz batch. Zero attempts, so no urgency; the content
  audit missed 8 of its 11 real defects, so its silence is weak evidence.
- 29 commits unpushed. PUBLIC repo - operator's call.
- Console browser leg (panel -> cookie -> click-through) still unconfirmed by a real session.
- Operator network blocks ALL of tailscale.com, so bc2/neon are reachable only via bc1.

**LADDER** (mapped, not built): T1 dispatch triage 10min -> T2 dispatch diagnosis 25min
(ad001 has FOUR randomised root causes) -> T3 open-world Windows investigation 60-90min ->
T4 finals on VMs, gated on bc4 RAM. Corpus: 95 dispatch boxes shipped, 7 open-world, 149 arena,
tournament backend with team scoring and point decay. Blue-team/DFIR, which is what exists.

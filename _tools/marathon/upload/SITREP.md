...
**Auto-updated:** 2026-08-08 02:05 EDT | branch: master | dirty files: 103

### Latest commits (auto)
- 1f0138867 SITREP: all three carried-forward Trial Run defects now fixed
- f529c8161 SITREP: M2 and M3 re-run and improved; M4 is all that is left
- 3c5404341 Career Launchpad is now its own repo; record it
- f10892a8f Record where Trial Run would be deployed, and why not yet
- 86e5ecea6 SITREP: #272 investigated, needs a curriculum call not a link
<!-- AUTO:END -->

**Manual section updated:** 2026-08-08 02:10 EDT — Trial Run gained setup/model/voice/PDF. M4 still needs 3 humans.

## NOW: Trial Run — product round done (ef3d2f2). M4 is still the only gate left.

  Repo: ~/hexworth-shared/carreer_launchpad (own git repo, LOCAL ONLY, no remote yet).
  11 offline suites, all passing: ./run-tests.sh

  SHIPPED THIS ROUND (ef3d2f2) — four asked for, one found:
   - FOUND: MIN_QUESTIONS was DEAD CONFIG (declared, read by nothing). A candidate who
     had said everything at question 6 could not stop, typed filler to escape, and the
     report counted that filler against them — the sample read "concrete context 1 of 10"
     where 4 of the 10 were the same 9-word sentence. The product was manufacturing the
     weakness it reported. Now: interview.can_finish() + POST /session/{id}/finish.
   - Setup dropdowns (seniority / round / pressure). Each CHANGES THE PROMPT — asserted.
     Validated server-side; an unknown value is refused, never silently defaulted.
   - Model dropdown for BYOK users, with real per-M-token prices, marking that ONLY
     claude-opus-5 has been gate-measured.
   - MULTI-PROVIDER (9c84a61): Anthropic, OpenAI and Google keys all work. Provider is
     decided BY THE KEY PREFIX (sk-ant- / sk- / AIza), not a second dropdown; a key/model
     mismatch is refused, never silently corrected. Three incompatible structured-output
     dialects adapted from ONE canonical schema (_strictify / _geminify) rather than three
     copies that would drift. EVERY non-Anthropic option is labelled UNMEASURED in the UI
     and a test asserts exactly one model is gate-measured.
     TRAP FOUND: build_client now also falls back to OPENAI_API_KEY / GOOGLE_API_KEY, so an
     ambient key sent the OFFLINE suites at a real API. All six now clear all three keys.
     THE SAME APPLIES TO THE GATES — with an ambient OpenAI key, M2/M3 would run on GPT-5
     while the artifact said claude-opus-5. Both gates print the model they used; check it.
   - Voice: Web Speech API, browser-native, no second vendor. Read-aloud is local and
     universal; dictation is Chrome/Edge and CHROME SENDS AUDIO TO GOOGLE — said in the UI
     before the mic opens, because this app holds resumes. Transcript identical either way.
   - PDF + markdown download. weasyprint was pinned since commit 1 and NEVER INSTALLED, so
     §6.3's PDF path was declared and unreachable all build. Pin moved 63.1 -> 69.0.
     Downloads serve the STORED report, never generate (asserted: 2 downloads, 0 calls).
   - Practice sheet, a §6.3 extension: "Answer these again" with the real question and what
     to fix. Deterministic, free, no model call.

  EARLIER (f1bc7c3, f605e8c): fixed the interviewer, the verifier, and the M3 gate that hid
  both. The old M3 metric REWARDED the robotic tic — quoting the candidate back guarantees
  lexical overlap — and never noticed 78% of questions stacked two asks. Replaced with a
  verified `references` field. Report voice unified to second person.
  ...
  Tests:   ./run-tests.sh  (9 suites, no key, all passing)
  Key:     grep ANTHROPIC_API_KEY ~/job-campaign-stack/.env    (Lodestar's, works)

  NOW IN GIT (fixed 2026-08-08, commit 248a68e in ~/hexworth-shared/carreer_launchpad).
    Own repo, branch master, 122 files / 808KB from a 427MB tree. .gitignore written
    BEFORE init: three .venv dirs (~400MB), __pycache__, and *.db are excluded — the db
    exclusion matters because llm_call stores full request/response JSON, i.e. whole
    resumes and transcripts. Verified no real credential is tracked; the only sk-ant-
    string is the deliberate fake sentinel in tests/test_byok.py. interview-ops/ and
    trial-run-staging/ committed too, superseded but not destroyed.
    NO REMOTE YET — it is local-only, so it survives an rm but not a disk loss.

  HOSTING — analysed 2026-08-08, deliberately NOT decided. Full reasoning committed at
    _docs/operations/trial-run-hosting.md (kept in hexworth-prime because that is where the
    decision lands AND because that tree is under version control).
    Headlines: spec §13.4 is wrong on a fact — there is NO React SPA, web/index.html is one
    25KB self-contained file with no bundler, so the FRONTEND satisfies rule 3 and only the
    API needs a Python host. Recommended shape: static frontend in _app/career/trial-run/,
    API on Cloud Run behind a Firebase rewrite at /api/trial-run/** — the reason is
    SAME-ORIGIN, which keeps the Sec-Fetch-Site trust boundary in api/access.py working.
    Any other host forfeits it. NOT bc1/2/3: #275 says slot reclamation is dead there.
    Recommended timing: NOT before M4. M4 needs 3 students in a room, not a deployment.

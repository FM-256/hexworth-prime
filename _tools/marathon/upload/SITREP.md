...
**Auto-updated:** 2026-08-07 23:01 EDT | branch: master | dirty files: 104

### Latest commits (auto)
- 20f8e7e11 SITREP: M4 pipeline built, gate blocked on real subjects
- b232e9178 SITREP: Trial Run M3 passed, M4 next
- 1deaba811 SITREP: Trial Run has a usable UI; M2 still needs a key
- 36f973e3a Career Launchpad: a card for Trial Run, not yet a link
- 33c196dd4 SITREP: make the NOW block a bookmark, not a record
<!-- AUTO:END -->
...
### PICK UP HERE — ranked, with why
  ...
  3. #296 — sweep labs for the finishStage double-award shape. Pattern already proven on
     ch25 (sticky LAB.completed persisted as completedOnce). Mechanical, well understood.
  4. #297 — pool-draw is scoreable but NOT safe, and revealToAll+partial is a live key
     harvest on 122 of 621 quizzes. Both need a server-recorded served-set; likely one
     solution. DO NOT enable poolSize on any quiz, forge-ch25 included, until this lands.
  5. #298 — seed-quiz-key.js non-merge .set() strips explanations/revealToAll/
     reviewAfterFails from any quiz it touches.
  6. CHEAP — neither push-quiz-keys.js nor seed-quiz-key.js prints which project it targets.
     That is what let an agent write to production believing it was on the emulator.

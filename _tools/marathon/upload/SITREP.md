...
**Auto-updated:** 2026-08-14 19:36 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 5e8d096ea Revert the gate regex: closing the blind spot armed 245 false blocks
- 509b65681 Em-dash sweep, and the gate that enforces the rule could not see a quarter of them
- 950cd18df Dash hygiene gate: colon, not em-dash, in the three new descriptions
- 6d6dfdbc7 BUG-119: the numbers were right, my explanation of them was not
- fe1bc4d84 BUG-119: my breakdown summed to 20, and the false claim was on two more surfaces
<!-- AUTO:END -->

**Manual section updated:** 2026-08-14 (em-dash sweep awaiting Chris round 6; BUG-118/119 shipped).
...
## SHIPPED AND VERIFIED LIVE TODAY
   ...
   dead post-lab returnUrl. Verified by resolving each meta refresh the way a BROWSER does;
   curl -L does NOT follow a meta refresh, it refetches the stub and reports 200.
   BUG-119: 3 false "covers all 9 objectives" claims corrected, including the subtitle a student
   reads immediately before sitting the exam. The final tests Obj 1-6 only; zero questions on
   Tkinter GUI, sockets/threading or sorting/search. OPERATOR DECISION STILL OPEN: extend the
   exam (then re-seed quiz_keys/pfi-w4-final-exam or every student is misgraded) or accept scope.

## NOW: em-dash sweep of the two files I authored today. Operator: "we do not like em dashes."
   83 occurrences removed, ModuleProgress.js 63 and the OpenStack hub 20. One was STUDENT-FACING
   and live since 2026-07-30: the lab panel rendered "3 / 3 objectives complete -- lab complete."
   One em dash is NOT punctuation and was kept: document.title.split(' U+2014 ') extracts a clean
   title and 856 pages carry that character in their <title>. Written as the \u2014 escape.

## THE DEFINING PATTERN OF THIS SESSION: WHEN I REPORT A CLEAN RESULT, SUSPECT THE INSTRUMENT.
   Five blocks in a row landed on my claims or my detectors, never on the product change.
   - the hub-tree crawler UNDER-REPORTS (600 nodes -> 31, depth 5 -> 2); its "Broken: 44" is a
     shallow crawl, not an improvement. Shallow regen archived, NOT committed.
   - audit-hub-deadrefs-v2.js prints refs=0: it filters severity 'high' and HUB-001 is now
     'medium'. It went blind BECAUSE the platform improved.
   - my stale-count checker contained the bug it checks for: its own implementation line held
     three backticks, flipping pair-parity so an injected false claim passed. Deleted, not patched.
   - dash-hygiene-gate could not see a "--" at a line wrap, so its clean report was false and my
     grep had the identical blind spot. My count said 79; the truth was 83.
   - and my fix for THAT armed 245 false blocks across 105 files, including the SQL lab where the
     trailing "--" is the lesson. Reverted. BUG-120 logged.
   FOUR TIMES I ASKED A REGEX TO DO A LEXER'S JOB, after 3bdbf7a80 THIS MORNING ruled "stop
   parsing with regexes". A selftest of synthetic cases cannot find the case its author did not
   imagine; the second fixture has to come from the tree.
   Also: I called GATED content broken four times (df-61, 42 catalog entries, 11 isc2-cc cards,
   the GUI Quiz card), and my probes reported clean without reaching their target four times
   (collapsed accordion, display:none slide twice, an ablation planting a CORRECT value, a no-op
   ablation). Rules saved: feedback_check_the_gate_before_calling_it_broken,
   feedback_a_count_in_prose_is_stale_on_arrival, feedback_no_em_dashes (chat-prose section).

## NEXT
   1. Chris PASS -> record-chris-pass.sh -> ./deploy.sh -> verify both files on production.
   2. OPERATOR: the Obj 7/8/9 exam coverage gap (BUG-119).
   3. OPEN: fix tree-mapper before wiring any gate to it. Fix audit-hub-deadrefs-v2's severity
      filter, base resolution and quote handling. BUG-120 needs a decision, not a patch.
   4. GAP: presentation-level hrefs are not covered by gate 3.2, which reads DATA files only.
   5. Debris needing operator rm (denied to me and to Chris): _tools/qa/_chris_ablation_tmp5.js,
      _chris_ablation_tmp5b.js, plus 15 older probe files. All archived.

...
**Auto-updated:** 2026-08-19 20:09 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- 91e7c9b48 Seeded the reference competition box; gate proven live, disclosure refusal not yet
- c687979ba Reference competition box: a ranked flag that cannot be guessed or harvested
- 103fcd2c1 Competition boxes must not disclose flag values: deliverFlag can now refuse
- aa4a6a500 SITREP: board clear of student-facing work; next is the Windows qualifier CTF
- c9b95b61b Guests stayed stopped after the power loss; started them and made it self-heal
<!-- AUTO:END -->

**Manual section updated:** 2026-08-19 23:00 EDT - qualifier CTF scoping + flag-integrity fix; Mallory audit running.

## ★★ NOW - HEXWORTH QUALIFIER (Windows CTF). Flag integrity fixed; Mallory audit RUNNING.

**THE FIND THAT MATTERS.** deliverFlag returned a flag VALUE and checked only that the caller
was signed in, while boxId+flagId sit in client-side config. LagrangeEngine's header already
documented it - "231 boxes hand a student every answer before they do anything" - and correctly
graded it SPOILER-class, because captures stay server-compared by validateFlag. Fine for
teaching. FATAL for a ranked competition, where seeing the flag IS having it.

**FIXED + DEPLOYED**
- deliverFlag refuses when the registry doc has a truthy `deliveryDisabled` (guard placed BEFORE
  the value is read; any truthy value blocks, so a hand-typed "true" fails CLOSED).
- OPT-IN, not default-on: the prefetch exists for VS/CoOp cache validation and labs display the
  flag as a reward. Flipping all 231 boxes would break both, silently.
- seed-box-flags.js carries the field from box_flags.json, only ever writing TRUE - writing
  false under merge:true would silently RE-ENABLE disclosure on a box locked down by hand.

**REFERENCE BOX SEEDED: qual-w1-lockout** (competition variant of ad001-lockout-storm; the
TEACHING box is untouched and still discloses). 10 checks pass against PRODUCTION documents,
not local files:
  gate refuses with no work / refuses FINDINGS-ONLY / permits full work / rejects the decoy /
  deliveryDisabled live / le-01 gate untouched.
The findings-only refusal is the one that matters: the correct answer alone is not enough, which
is what stops enumeration beating ctfSubmitFlag's 10s-per-team rate limit.

⚠ NOT DEMONSTRATED: that the DEPLOYED deliverFlag refuses an authenticated caller. Guard is in
the deployed revision by timestamp, unit-tested, registry field live - three facts that each look
like proof and together still are not someone being told no. Could not mint a token (needs a
service account with signBlob; local ADC is user credentials).
RUN THIS SIGNED IN ON hexworth.com:
  await FirebaseAuth.callFunction('deliverFlag',{boxId:'qual-w1-lockout',flagId:'source-host'})
  expect permission-denied. A flag value back = a real hole.
Or: node functions/verify-deliverflag-guard.js <keyfile> where a service account exists.

**WHAT THE PLATFORM ALREADY HAD, and I nearly rebuilt.** mission_gates + recordMissionFinding:
"The client reports an ACTION, never a conclusion. The server re-derives the result from its own
copy... a client that can be edited to skip the work can be edited to claim it." The qualifier is
that machinery pointed at Windows evidence instead of orbital telemetry.

**CONTENT INVENTORY for the ladder** (all simulated - no Windows VM exists and the per-student
OpenStack quota is 1 vCPU / 192MB, so VMs are a FINALS question gated on bc4's RAM):
  Dispatch  95 boxes SHIPPED (config.js + index.html each). 6 AD, 6 OS, 5 srv, 5 cloud.
            ad001-lockout-storm alone is 2137 lines with FOUR randomised root causes.
  OpenWorld 7 boxes on OpenWorldEngine - multi-device, evidence, red herrings scored NEGATIVE,
            game clock, adversary triggers incl. a timed remote wipe.
  Arena     149 boxes, 242 registry entries, full tournament backend (ctfSubmitFlag with team
            scoring, duplicate rejection, 10s rate limit, dynamic point decay, Discord wire).
  Gate 8    "Operation Gone Dark" - the multi-device architecture the ow-* boxes descend from.

**PROPOSED LADDER** (not built): T1 triage (dispatch, 10min) -> T2 diagnosis (ad001 etc, 25min)
-> T3 open-world Windows investigation (60-90min) -> T4 finals on VMs. All blue-team/DFIR, which
is what the existing corpus is.

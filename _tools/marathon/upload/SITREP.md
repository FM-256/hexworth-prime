...
**Auto-updated:** 2026-08-24 23:11 EDT | branch: master | dirty files: 50

### Latest commits (auto)
- e9dc4d760 BUG-058 actually closed: the grader writes down what only it can see
- 2be6c0821 Punctuation the dash gate flagged, fixed rather than forced past
- 2385775f2 Operator authorized shipping check 6 with the documented bypass as known risk
...
## LAB DAY IS TOMORROW (2026-08-25). Read this block first.
...
- M1 nginx serving; the peer loads the page; `nmap` sees 2 open ports
- M2 SFTP lands; proof file sits ON the Cinder volume ("Cinder survived")
...
Horizon dashboard: a student slot logs in for real (POST -> 302, authenticated pages render).
The console lab needs ONLY this -- it has no graded checks and is about reading the dashboard
and proving the same numbers from the CLI.

### KNOWN BROKEN, and deliberately NOT fixed before the class
**The instance noVNC console does not work and never has.** Horizon builds the iframe pointing
at the DevStack VM's private libvirt address on port 6080, which no student browser can route
to. There is NO public route to that proxy in traefik or the tunnel. The console PAGE loads, so
it looks healthy until someone actually opens the VNC view.
Fix needs: a socat listener on bc2, a traefik route on bc1, nova `novncproxy_base_url` changed,
nova restarted, websockets proven through cloudflared. That is a multi-host change and NOTHING
tomorrow depends on it -- not the console lab, not any Sprint mission (they use SSH). Do it
AFTER lab day.

### Two instrument failures worth remembering
- I reported Horizon unreachable. My URL was wrong: the code's fallback default is
  `sandbox.hexworth.dev`, the real domain is **`.tech`**.
- A 401 from `/dashboard` is the CONSOLE GATE working (traefik forwardAuth wants the signed
  launch cookie). It is not an outage. Mint the cookie the way `/console-session` does.

...
**Auto-updated:** 2026-08-15 14:28 EDT | branch: master | dirty files: 44

### Latest commits (auto)
- d8feffca9 SITREP: tailnet segmented; infra detail recorded privately, not in this public repo
- 1d2256c0c Infra pass recorded; node addressing kept OUT of this repo
- d76e4cf04 SITREP: everything shipped and verified; two operator decisions outstanding
- df0fc5ad2 BUG-120: I mislabelled the largest row by not reading it
- 2719dc83f BUG-120: stop characterising the corpus, publish the measurement
<!-- AUTO:END -->

**Manual section updated:** 2026-08-15 (CHAOS Phase 1 functionally complete; granularity gap closed).
...
## ★ NOW: CHAOS PHASE 1 IS FUNCTIONALLY COMPLETE. Repo @ 6d00272, 271 checks, 0 gaps.
   Host: /opt/chaos + /opt/oss-crs on bc4. Target json-c OSV-2020-252 (libpng RETIRED: its
   harness cannot satisfy the fault condition).
   V0.1 LOOP CLOSED AND PROVEN THROUGH THE CODE, not by hand:
     gate -> prepare -> build-target -> run -> collect -> reproduce -> correlate -> report
   Criterion MET at PoV level: 7/60 PoVs crash vulnerable and run clean on fixed.
   ★ GRANULARITY GAP CLOSED 2026-08-15. Identity is the crash SITE (LINE only, top target frame
   only). CVE at json_tokener.c:800 now separates from its neighbour at :651, where before they
   merged into ONE finding. My framing was wrong, not just the code: I demanded signatures
   survive a refactor, which applies them to a CROSS-COMMIT question they were never built for.
   The cross-commit question is the A/B, and the A/B is per-PoV and uses no signature.
   ⚠ LINE ONLY, NEVER COLUMN — the existing suites established that. Keeping :line:col turned 3
   suites red: two CRSs reached ONE defect at :210:14 and :210:9, and the column split them,
   breaking the property correlation exists for.

## BUILT TODAY THAT DID NOT EXIST 2026-08-13 (11 modules)
   reproduce (evidence receipts) · differential (PoV-level A/B) · status (novelty taxonomy)
   advisory (per-finding OSV match) · capability (what a defect GRANTS + composability)
   outcome (non-crashing outcomes) · ccc (read-only dashboard, localhost, cannot act)
   + tools: run_v01_campaign, run_ab, fetch_advisories, verify_substrate_contract

## ⚠ NINE DEFECTS IN CHAOS'S OWN CODE, ALL FOUND BY RUNNING IT, NONE VISIBLE TO A GREEN SUITE
   reproduction never implemented · 3 invented CLI flags · exit 124 = SUCCESS not failure ·
   --work-dir never passed (a LOST campaign read as clean) · collect() wrong layout (0 of 541
   PoVs) · reproduce/correlate INVERTED (a PoV is an input, not a crash report) · signature keyed
   on ASan's strcmp interceptor · `^_start$` anchor that could never match · RE_ACCESS matching
   one of two real formats (made 8 projects look identical).
   THE PATTERN: the TEST AND THE BUG SHARED A SOURCE. Fixtures were WRITTEN, never CAPTURED.
   tests/fixtures/real-*.txt now hold byte-for-byte output from real runs.
   FOUR detectors returned confident wrong answers; THREE had the same tell — a suspiciously
   uniform result. When every input gives the same answer, check the instrument.

## ⚠ OPEN, and where each belongs
   PHASE 1: second input vector (input-vector-decision.md open question A) — decided in
     principle, unbuilt. Mixed-primitive target selected by capability sweep: libarchive /
     harfbuzz / openssl / libxml2 / freetype2 / libpcap all carry >=2 capability classes;
     json-c carries ONE, which is why composability could not be demonstrated on it.
   PHASE 2 (remediation), PHASE 3 (multi-agent ensemble = the "one CRS" item), PHASE 4
     (network), PHASE 5 (range): untouched, and correctly so — they depend on Phase 1 closing.
   NOT A GAP: one CRS is Phase 1 behaving as scoped; architecture.md excludes ensembles from V0.1.
   ⚠ NO NOVEL FINDING YET. Everything found is a known CVE re-found. That is the actual product.
   ⚠ README.md:11 still says "Nothing has been run against a real target yet" — STALE, fix it.
   Scope decisions open: 5 spend ceiling, 6 authorization model, 9 trust floor.
   OPERATOR rm (denied to me): chaos/campaigns/libpng-cve-2018-13785.yaml.
   Infra record: ~/hexworth-infra-private/compute-nodes-inventory.md

...
**Auto-updated:** 2026-08-16 12:54 EDT | branch: master | dirty files: 64

### Latest commits (auto)
- dd364f4fa SITREP: a campaign was labelling builds with a commit it never built
- 7cc342f97 SITREP handoff at 99% context: write primitive, pipeline fixture, CCC complete; composability open
- a1e0b88cd SITREP handoff: write primitive + pipeline fixture demonstrated, composability still open
...
**Manual section updated:** 2026-08-16 (seed staging PROVEN 27 PoVs/45s; exposure gate shipped; repo is PUBLIC).
...
## ★ NOW: CHAOS — a campaign now has to BUILD the commit it claims. Repo @ bfd8caa. 381 checks.
   Host bc4: /opt/chaos + /opt/oss-crs.

### ⚠ THE HEADLINE: EVERY CAMPAIGN REPORT BEFORE bfd8caa NAMES A COMMIT IT CANNOT VOUCH FOR
...
### ✅ SEED STAGING FIXED AND PROVEN END-TO-END (chaos fbcb0e7)
   45 SECONDS WITH SEEDS -> 27 PoVs, reproduce[27/27]. 1800 SECONDS WITHOUT -> 0.
   Phases now read: gate -> prepare -> build-target -> stage-seeds[2595] -> run -> collect ->
   reproduce[27/27] -> correlate. The count rides IN the phase name so a zero is visible.
...
### DEMONSTRATED ON REAL TARGETS
   ...
   Loop closes from a SURFACE GRANT: gate -> select -> generate compose -> build -> run ->
     collect -> reproduce -> correlate. Authorization enforced at THREE layers.
...
### NEXT (specified, not built)
   0. ⚠ FIRST: make seed staging target the build the CRS ACTUALLY FUZZES, not --build-dir.
      Until then the campaign fuzzes a differently-seeded binary than anything proven by hand,
      and no campaign result is comparable to the by-hand ones.
   1. Re-run libxml2 through run_v01_campaign.py — now HALTS on the dirty tree (M parser.c from
      the injection fixture). Operator decides: archive that diff, or --allow-dirty (UNVERIFIED).
      The 91/91 write finding still has NOT entered the record/CCC/history.
...
### OPERATOR ACTIONS OWED (open all session)
   ...
   ★ DECIDE on /data/chaos/targets/libxml2 `M parser.c` (the injection fixture edit). It now
     HALTS campaigns by design. Archive the diff, or re-run with --allow-dirty (UNVERIFIED).
     I did not touch it — we do not destroy, and it is your experiment.
   ...
   hexworth-prime is ~10 commits ahead of origin, NOT pushed (public repo, third-party fork).

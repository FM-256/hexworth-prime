...
**Auto-updated:** 2026-08-25 12:18 EDT | branch: master | dirty files: 93
...
- 05166801f Sprint page: use the full screen (phantom column, callouts, text-only cards)
- 7673cfea2 Punctuation in my own CSS comments, fixed rather than forced past the gate
<!-- AUTO:END -->

**Manual section updated:** 2026-08-25 ~15:00 EDT, after a LIVE CLASS DAY that included an outage I caused. Newest block first; older dated blocks follow below.


## NOW: class ran, platform healthy, one outage was MINE (2026-08-25 ~15:00)

Everything is up: 6/6 lab gates PASSED, hexworth.com 200, lab-manager healthy, students working.
Read the two incidents below before touching bc2 or any image.

### FIXED TODAY (student-facing)
- **The Horizon credentials panel never survived a reload.** Credentials were attached ONLY where
  the cloud slot is first claimed; every later response for the same session dropped them, and
  the page skips building the whole panel when `horizonPassword` is absent, so the area was not
  empty -- it was never created. Reproduced: 1st launch PRESENT, 2nd launch on the SAME session
  MISSING. Session record now keeps them and both existing-session return paths re-issue them.
  Verified: both launches PRESENT, same user/password. Commit `52cc450b5`.
- **Launches were failing outright at 12 concurrent.** `FREE_PLAY_CAP=12` while
  `MAX_TOTAL_CONTAINERS=40`, and `FREE_PLAY_LABS` includes `openstack-cli` -- i.e. every
  OpenStack lab was classed as "practice" and throttled while 28 slots sat idle. Raised to 32.
- **Sprint page layout**: phantom 382px column removed, callouts hug their text, `.wrap` widened.
  Live. NOTE: below ~1500px viewport this changes nothing (the bug only existed above that).

### ⚠ INCIDENT 1: I took the cloud down mid-class
`build-sprint-image.sh` run during a live class -> host OOM killer terminated the DevStack VM at
11:17:36 -> every student's CLI died with "Remote end closed connection without response".
Arithmetic, not bad luck: **the VM is allocated 26G of bc2's 31G**, leaving ~5G, and the build
adds qemu-nbd + a chroot running apt + a 4G image write.
Recovered with `sudo virsh start openstack-stage1`. A guard is now committed (`834c8ac99`): the
build REFUSES while the VM runs unless `ALLOW_LIVE_BUILD=1`.
**Do not run that build while the cloud is serving anyone.**

### ⚠ INCIDENT 2: I renamed the working image before proving the rebuild
Archived `ubuntu-24.04-sprint` to make room, the rebuild then failed on wedged nbd devices, and
students were left with NO image of that name -- they booted something else and reported
"systemctl and apt are not working" (CirrOS has neither). Restored by renaming back.
**Build to a NEW name and only swap after verifying.**

### OPEN: the console login, still unsolved
Students cannot log in at the noVNC console. It is the GUEST OS prompt, not Horizon, and the
sprint image bakes NO credential by design -- so `ubuntu login:` has no valid answer, and
`sudo passwd ubuntu` needs a shell they cannot reach. Confirmed no way in on RUNNING instances:
qemu-guest-agent is absent, so Nova/libvirt cannot set a password.
- CirrOS labs are fine: `cirros` / `gocubsgo`, printed on the console itself.
- Autologin IS written into `build-sprint-image.sh` (asserted on BOTH gettys) but is **in no
  image and untested** -- the build never completed.
- Works today with no image change: create with `--user-data` (cloud-config setting autologin +
  password). Proven on a real instance: `console-test-vm login: ubuntu (automatic login)`.
- **`m1.nano` fails with this image** ("Flavor's memory is too small") -- needs `ds512M`. Real
  quota is `maxTotalRAMSize 512`, NOT the 192 MB the build-reference handout still claims in
  three places.
- Any fix requires the student to CREATE a new instance. Cinder volume data survives; root-disk
  work does not.

### NOT IN GIT, and it matters
`lab-manager/server.js` -- the grader for every cloud lab -- lives ONLY on bc1. The exposure gate
correctly refuses it (real node address, public repo), so today's check-6 witness work and the
Horizon fix have no version history and would die with that host. Right home: the private infra
repo.

### Access note
bc2 is tailscale-only and **Zscaler runs on the operator's laptop**, so it intercepts on office
wifi AND on a phone hotspot (egress 165.225.x.x and 136.226.x.x -- both Zscaler). Tailscale stays
`offline` with `rx 0` either way. Route through bc1 instead: `ssh bc1-cf` then `ssh bc2`.
Students are unaffected -- they reach everything via Cloudflare.

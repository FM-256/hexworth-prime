...
**Auto-updated:** 2026-08-14 18:11 EDT | branch: master | dirty files: 48

### Latest commits (auto)
- ae670cfce Week 4 is finals week: the "GUI Quiz" card should never have been a quiz
- 0c43f90f1 BUG-118: a floor is still a live claim — ":25 has never had fewer than 359"
- 5a21dcc3f SITREP: BUG-118 round 6; every block has been in my claims, not the fix
- 5b5d27caf BUG-118: the third time I called this file clean, it still had a live count in it
- a6f5b8c80 BUG-118: remove the stale-count checker. It needed a lexer, and I wrote a regex twice.
<!-- AUTO:END -->

**Manual section updated:** 2026-08-14 (BUG-118 SHIPPED; finals-week card awaiting Chris).

## ✅ SHIPPED + VERIFIED LIVE TODAY
   BUG-115 + BUG-116: 12 forensics hrefs + 66 LearningPaths hrefs. 689/689 URLs 200 on prod.
   BUG-118: 5 meta-refresh stubs that returned 200 then bounced the student onto a 404, plus
   one dead post-lab returnUrl. All six verified on production by resolving each meta refresh
   the way a BROWSER does -- curl -L does NOT follow a meta refresh, it just refetches the stub
   and reports 200, which would have been a green result proving nothing.

## ⏳ NOW: finals-week card fix (ae670cfce), Chris running. Operator said deploy.
   OPERATOR RULING: "week 4 is finals week and instead of a quiz there should be a final or a
   project." Python-for-IT w4 has BOTH -- exams/pfi-w4-final-exam.exam.html and
   labs/pfi-w4-final-project.lab.html -- and the course index links both. Two w4 GUI
   presentations still offered a "GUI Quiz" card for a quiz that should never exist. The
   project card was already beside it, so the dead card is now the FINAL EXAM, worded from the
   course index rather than invented. Empty COMING_SOON gate block removed from both.
   Rule 9 checked BEFORE routing students there: verify-quiz-keys pfi-w4-final-exam ->
   25/25, pass 70, PASSED. Without that bridge every student scores 0/25.

## ⚠⚠ THE DEFINING MISTAKE OF THIS SESSION: I CALLED GATED CONTENT BROKEN FOUR TIMES.
   df-61 "unreachable" (ContentCatalog links it, 200) · 42 catalog entries "404s students hit
   today" (all status coming-soon, browser-verified ZERO reachable) · 11 isc2-cc cards
   "clickable 404s" -- AND I EDITED THE FILE on that premise before finding BUG-012's gate
   right below them, reverted byte-identical from the archive taken first · the GUI Quiz card
   "a live 404" (COMING_SOON + preventDefault).
   RULE: a link that does not resolve is not a defect until something can FOLLOW it. Read the
   status field AND the click handler. memory/feedback_check_the_gate_before_calling_it_broken.md

## ⚠ AND MY PROBES LIE IN A SPECIFIC WAY: THEY DO NOT REACH THE TARGET, THEN REPORT CLEAN.
   A click on a collapsed accordion (isc2-cc), a click on a display:none slide (this fix,
   twice -- goToSlide is 1-indexed and never applied .active), an ablation planting a value
   that was actually CORRECT, and an ablation that was a silent no-op. Each printed a
   pass-shaped result. A probe is evidence only if the target is verified REACHED and the
   plant verified WRONG. Closed this one properly: drove .slide.active, measured the card at
   240x146, then clicked -> navigated to the exam, no dialog.

## ⏭ NEXT
   1. Chris PASS -> record -> ./deploy.sh -> verify both decks on production.
   2. OPEN: hub-tree tooling regressed -- tree-mapper 600 nodes -> 31, depth 5 -> 2, lost
      prev/course-home/returnUrl/next. Its "Broken: 44" UNDER-REPORTS. Shallow regen archived,
      NOT committed. Fix before wiring any gate to it. audit-hub-deadrefs-v2.js prints refs=0
      (filters severity 'high'; HUB-001 is now 'medium') -- went blind BECAUSE things improved.
   3. GAP: presentation-level hrefs are NOT covered by gate 3.2, which reads DATA files only.
      Only the crawler sees that class -- which is the argument for fixing it.
   4. Debris needing operator rm (denied to me AND Chris): _tools/qa/_chris_ablation_tmp5.js,
      _chris_ablation_tmp5b.js (archived), plus 15 older probe files.

...
**Auto-updated:** 2026-08-07 00:10 EDT | branch: master | dirty files: 95
...
- 29eb80b5e A+ Core 2 ch25: Virtualization & Hypervisors
- 409516ad3 Archive 82 orphaned probes -- copied, byte-verified, then removed. 21 spared.
- 8820b7701 Archive candidates, not deletion candidates -- I wrote destroy-first into the tooling
<!-- AUTO:END -->
...
## NOW: ch25 lab, Chris round 3 in flight. Deploy authorized on PASS. NOT deployed.

  CHRIS BLOCKED TWICE, both times correctly, both the same bug class one layer apart.

  R1: stageComplete()/finishStage() read LAB.tasksDone, a plain writable object. Six devtools
      lines completed all 3 stages with ZERO VMs and fired ModuleProgress.complete().
      My own "DIRECT-CALL BYPASS" self-test passed the whole time: it called the advance
      functions but never forged the cache the gate actually read. Wrong door.
      FIXED: stageComplete(n) re-derives every state-derived task from t.check() under a
      temporary LAB.stage swap, restored in finally. finishStage() routes through it.
      Also fixed my own find: s3t7 matcher rejected "type one" / "Type I" / "a type 1
      hypervisor". Now normalises numerals and matches the concept token anywhere.

  R2: same class deeper. LAB.vms / labVmId / hostRefused are equally forgeable; Chris pushed
      fake VM objects and finished the lab without pressing New. I REPRODUCED IT MYSELF
      (chris-forge-debug.js) before touching anything. Chris explicitly did NOT require code
      changes, only that the file stop claiming a guarantee the architecture cannot deliver.
      FIXED BY TELLING THE TRUTH, not by theatre: honesty rule 2 now names the forgeable
      objects, says reading the checks is enough to fabricate them, says every mitigation is
      another plain object one layer down, and points at the server-graded exam path
      (gradeQuiz + quiz_keys) as where real integrity lives. Also corrected a second
      overclaim Chris did not flag ("no path to credit except building the thing").
      lab-cheat.js header now states what it does NOT cover, so 18/18 cannot read as
      "unforgeable". Added the REVOCATION test, the property re-derivation actually buys.

  SUITES: lab-cheat 18/18, lab-check 25/25, lab-s2 29/29, lab-s3 42/42, lab-brief 14/14.
          dash-hygiene-gate exit 0. Engine parses.

  ALSO SHIPPED THIS SESSION (not yet committed): HEUR-035 low -> HIGH + 5 encodings,
  _tools/eduscan/dash-hygiene-gate.js wired as deploy.sh gate 2.6, selftest 13/13.
  ~1587 legacy files become HIGH; default deploy unaffected, --strict will fail until burned.

  LOGGED: taskboard #293 ch25 server-graded quiz, #294 Core 1 forge-vm-setup.lab.html still
  rubber-stamps 2 of 5 gates and is live.
  MEMORY: feedback_gate_must_rederive_not_trust_cache (both layers).

  NEXT ON PASS: commit -> record-chris-pass.sh -> ./deploy.sh (master, authorized).

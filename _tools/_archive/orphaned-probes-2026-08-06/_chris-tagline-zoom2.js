...
(async () => {
  ...
  const text = await page.evaluate(() => {
    const els = Array.from(document.body.querySelectorAll('*')).filter(e => e.children.length===0 && /DECODE|DISCOVER|DEFEND/i.test(e.textContent||''));
    return els.map(e => ({ tag: e.tagName, cls: e.className, text: e.textContent }));
  });

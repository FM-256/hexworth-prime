...
/* COMPLETION REGISTRY (BUG-264). The canonical set of ids the platform can legitimately award,
   generated by _tools/content/gen-completion-registry.js and kept in sync by a --check in
   deploy.sh gate 3.8. Without it, recordProgress and syncProgress accepted ANY string as a
   completed module and deriveXP paid XP for it -- proven in production.

   FAILS OPEN, DELIBERATELY. If the file is missing or unparseable, validation is SKIPPED and a
   loud error is logged. The alternative -- rejecting every completion when a config file fails to
   load -- would stop every student on the platform from recording work, which is far worse than
   the forgery it would prevent. The deploy gate is what guarantees the file is present and
   current; this runtime guard exists only so a packaging accident degrades to today's behaviour
   instead of an outage. If you see this warning in logs, the gate has been bypassed somehow. */
let _COMPLETION_IDS = null;
try {
    _COMPLETION_IDS = new Set(require('./completion-registry.json').ids);
} catch (e) {
...
exports.recordProgress = onCall(cfOptions, async (request) => {
    ...
    switch (type) {
        case 'module':
            /* BUG-264. An id nothing in the platform defines is not a completion. This branch used
               to arrayUnion ANY string, and deriveXP pays XP per entry -- proven live: a fabricated
               id like 'anything-i-want-not-even-house-shaped' was accepted and paid. */
            if (!_isKnownCompletion(itemId)) {
        ...
        case 'lab':
            // BUG-264, same reasoning as the module branch above.
            if (!_isKnownCompletion(itemId)) {
...
exports.syncProgress = onCall(cfOptions, async (request) => {
    ...
    /* LAB TRACKER KEYS ARE VALID IDS, NOT GARBAGE. BUG-267, second and larger half.
       ...
       lands in labsCompleted when a student finishes the lab. All 271 box configs app-wide that declare a
       trackerKey use this format, and every single one was being counted as garbage by the rule
       below -- so a student completing FIVE of them crosses XPCalculator's threshold and
       IntegrityLockscreen.js locks them out for doing real coursework.
       MECHANISM, NOT A HEADCOUNT: _checkIntegrity reads the LOCALSTORAGE completedModules array,
       which no server snapshot can see, so how many students this actually hit is NOT measurable
       from here. An earlier version of this comment claimed "one production account was already at
       14" -- that was FALSE. That account's modulesCompleted was EMPTY; the 14 was its
       labsCompleted total, an array _checkIntegrity never scans.
       ...
       `LAB_ALA_L02` and `lab-ala-l02`. Verified against all 271 real trackerKeys (135 lab_*, 136 ctf_*; all match) and a
...
/* DOUBLED-PREFIX REJECTION REMOVED 2026-09-07, WITH EVIDENCE, AFTER I RESTORED IT IN ERROR.
   ...
   cannot misjudge legitimate content. */
    ...
    /* BUG-264. Shape was never enough: `shield-fabricated-xyz-999` is shape-valid and was accepted
       and paid for. Now the id must also be one the platform actually defines.
       INCOMING PAYLOAD ONLY. The cloud side of the merge below is never filtered -- that is
       GUARD-04, and filtering it is what deleted earned completions in BUG-266. */
    const localModules = rawModules.filter(_isValidModuleId).filter(_isKnownCompletion);
    const garbageModules = rawModules.length - localModules.length;

    const localLabs = sanitizeStringArray(localData.labsCompleted).filter(_isValidModuleId).filter(_isKnownCompletion);

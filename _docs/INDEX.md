...
| [security-findings-2026-08-09-progress-integrity.md](operations/security-findings-2026-08-09-progress-integrity.md) | 150 | DOCUMENTED NOT FIXED, deferred to 2026-08-10. Mallory, emulator-proven: syncProgress unions client-supplied achievement ids into Firestore with no catalog check (a made-up id persisted; gate_99 added 500 real XP with no gate doc), and firestore.rules permits it on a STATED ASSUMPTION THAT IS FALSE. AchievementManager/GameTracker are console-callable globals, so any game's achievement + 1000 XP can be forged without playing. revealGate is declared server-side and enforced nowhere. Tasks #304/#305/#306 |
| [user-transcript-and-skill-anchoring.md](architecture/user-transcript-and-skill-anchoring.md) | 160 | SCOPED NOT BUILT (tasks #300/#301). College-style transcript: completed courses + proven skills. Measured: no course-completion fact and no grade anywhere (gradebook/finalGrade/letterGrade/gpa = 0 hits); ModuleProgress is localStorage with fail-silent sync (3,909 pages); item.attempt is a schema with no producer or consumer; 34 lab skill maps with a four-layer cognitive ladder, 26 flag-proven server-side. Design: anchor on two axes (ATT&CK/objective + Recognition/Hypothesis/Execution/Transfer), badges as non-detachable wrapper. Full scope in ~/hexworth-shared/User-History/ |
| [lagrange-space-cyber-series-scope.md](architecture/lagrange-space-cyber-series-scope.md) | 150 | SCOPED NOT BUILT. Lagrange as a SERIES: build satellites, pen test them, defend them. Less speculative than it feels: real anchors exist (MITRE SPARTA as the spacecraft ATT&CK, NIST IR 8270, CCSDS, DEF CON Hack-A-Sat, Viasat KA-SAT 2022) and real open-source flight software runs in Docker (NASA cFS, Yamcs, OpenC3 COSMOS, gr-satellites). Measured: the platform already has SEVEN terminals (SecurityTerminal extends LinuxTerminal is the pattern) plus SandboxLauncher for real bc1 containers, so no new terminal engine is needed. THE open decision is what the student attacks, simulated vs real cFS; recommendation is real, proven by ONE end-to-end mission before committing to a series shape. Three tracks (build/red/blue) plus Cold Horizon as the investigation entry point |
...
| [lab-realism-enhancement-protocol.md](operations/lab-realism-enhancement-protocol.md) | — | LREP — methodology for re-skinning lab tool surfaces to mirror real-world security products (VirusTotal, Mandiant, Rapid7, NVD, SCCM, Splunk, etc.). §1–12 covers visual realism (brand mapping, wave structure, shared-helper patterns, smoke-contract preservation, walkthrough sync). **§13–14 (added 2026-06-04) covers reactivity** — the four layers of "click feels dead", why smoke that bypasses the engine is a polite lie, docs/engine contract enforcement, the cross-tool completion-reveal pattern, and a pre-flight reactivity checklist. Canonical example: PIS-Final Patient Zero (38 routes, 13 tool surfaces, 73/73 smoke checkpoints, plus the reactivity-debugging incidents `3e370cb0d`→`3816e6ed5`). |

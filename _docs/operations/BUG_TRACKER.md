...
- **Triage / buckets:**
  ...
  - **(D) Scattered singles — RESOLVED 2026-07-22 (operator "gate the bucket-D singles too"):** 8 files fixed so no student hits a 404.
    ...
    - **Verified:** all gated/redirect targets confirmed non-existent on disk (eth-r3's remap target confirmed to EXIST); headless check on divergent-field-terminal (6 anchors gated, click blocked) PASS; diff 105 ins / 4 del across the 8 files. Nancy PROCEED (caught the eth-r3 remap misclassification + a `continueBtn.onclick=null` cert-button hardening, both applied). **Un-gate:** delete the href from that file's `COMING_SOON` (or restore the JS redirect) once the content ships. **Scan caveat:** future dead-link scans should skip hrefs inside `<code>`/`<pre>`/`//` comments/JS strings.

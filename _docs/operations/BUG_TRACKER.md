...
- **Triage / buckets:**
  ...
  - **(B) DECISION: unbuilt `intro-computers` course (23 dead `fb-*` links from `houses/forge/intro-computers/index.html`):** the hub lists wk1-4 labs/quizzes/presentations/exams (`fb-w1-fundamentals.lab` … `fb-midterm.exam` …) that exist NOWHERE. Either the course is a roadmap placeholder (should the links be gated "coming soon" like the dark-arts vault, or the hub trimmed?) or it's meant to be built. **Operator call.**
  - **(C) DECISION: `isc2-cc` hub → 11 `../infosec/pis-01..20.html`** but served infosec dir only has `pis-r1..r5.html` (+ `_source/pis-01.html`, not served). Numbering mismatch — the hub references a module scheme that isn't the built content. **Operator call: remap to pis-r* or is PIS content elsewhere?**
  - **(A2) FIXABLE-NOW rename (shipping this session):** `matrix/protocore/index.html` links `sg-103-t-display-s3-setup.html` + `sg-105-wifi-recon-scanner.html` but the Signal files were renamed to `sg-103-s3-setup.html` + `sg-105-wifi-recon-s3.html` (content EXISTS; SG-number/topic/title all match; sibling sg-106/sg-111 links resolve, proving only the filenames drifted). → filename correction.
  - **(D) Scattered singles — refined (3 were SCAN FALSE POSITIVES; rest are missing-content decisions):** FALSE POSITIVES (scanner matched hrefs inside `<code>`/teaching text, not real links): `admin/console.html`→`...index.html` (a documentation table literally showing `<a href="...index.html">` as example text), and both `page-2.html` hits (`darkarts-web-scraping`/`script-web-crawler` — inside web-scraping teaching content + log samples, e.g. `books.toscrape.com/catalogue/page-2.html`). GENUINELY MISSING content (build vs remove-link decision): `code/incubator`→`games/pod-crossing.html`; `python-engineering`→`code-pye-certificate.module.html`; `code-git-basics.presentation`→`../quizzes/git-quiz.html`; `pfi-w4-gui*.presentation`→`../quizzes/pfi-w4-gui.quiz.html` (×2); `eth-r3`→`eth-01.html`; `key-encryption-basics.presentation`→`../quizzes/encryption-quiz.html`; `script-python-exam-chapter8.exam`→`python-course.html`; `divergent-field-terminal`→several `divergent/districts/*/index.html`. **Caveat for a future scan: filter hrefs inside `<code>`/`<pre>` and JS string/log content to cut false positives.**

...
### BUG-104 — lab completion gate counts tasks instead of checking which ones, so it is spoofable from the console  ·  [P2]  ·  2 of 3 labs FIXED + LIVE, advanced-ops still open
...
- **LIVE on production:** `cloud-openstack-install` and `cloud-openstack-launch-vm`, both verified by running the exploit against hexworth.com (`credited=false set=0`). **`cloud-openstack-advanced-ops` is committed but NOT deployed and is STILL EXPLOITABLE in production** (`credited=true set=10`).

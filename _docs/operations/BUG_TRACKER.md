...
- **VERIFIED, READY-TO-REMOVE (item 22 — de-risked, low value so not yet executed):** cloud-iam-debugger has 18 provably-dead CSS classes (0 references anywhere outside `<style>`, incl. no dynamic construction): `.json-key/-string/-number/-boolean/-bracket`, `.problem-highlight`, `.fix-options/-option(+.selected/.correct/.incorrect/:hover)/-label/-code`, `.diff-add/-remove`, `.pulse` (+ its now-orphan `@keyframes pulse`), `.selected`, `.timer-bar/-fill` — leftovers from removed features (JSON highlighter, timer bar, diff view, fix-options panel). 5 contiguous regions (one nested in a `@media`). Removal is invisible to students (pure hygiene) so it's parked to batch with the next iam deploy rather than solo-shipped. `_tools/eduscan/smoke/cyberops-tab-render.js` render harness can confirm no visual delta.

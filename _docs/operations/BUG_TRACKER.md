...
- **Triage / buckets:**
  ...
  - **(D) Scattered singles (need per-target intent, mostly missing content):** `admin/console.html`→`...index.html` (clear typo/garbage — 3-dot); `code/incubator`→`games/pod-crossing.html`; `python-engineering`→`code-pye-certificate.module.html`; `code-git-basics.presentation`→`../quizzes/git-quiz.html`; `pfi-w4-gui*.presentation`→`../quizzes/pfi-w4-gui.quiz.html` (×2); `eth-r3`→`eth-01.html`; `key-encryption-basics.presentation`→`../quizzes/encryption-quiz.html`; `matrix/protocore`→2 cross-house `signal/.../sg-10x` pages; `script-python-exam-chapter8.exam`→`python-course.html`; `divergent-field-terminal`→several `divergent/districts/*/index.html`; `darkarts-web-scraping`/`script-web-crawler`→`page-2.html` (likely intentional pagination stubs).
...
- **Fix:** bucket A (9 files) shipping now (Nancy PROCEED + Chris gated); each redirect verified to resolve to a real page (root `dashboard.html` / house `index.html`); the 2 deeper script applets that *correctly* use `../../index.html` were confirmed untouched. Buckets B/C/D/E await operator decisions (build vs coming-soon-gate vs trim vs remap vs dedup).

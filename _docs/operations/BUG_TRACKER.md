...
- ⚠ **The gap was not hypothetical — 108 more links were already broken.**
  ...
  - **ContentCatalog, 42 dead, all confirmed 404 on production.** These are **not** filename drift and cannot be repointed: Forge *"Intro to Computers"* advertises a four-week course of which only `index.html` and three presentations exist (23 links), and Shield *"Principles of Information Security"* asks for `pis-01…pis-20` where disk has `pis-r1…pis-r5` (17 links) — 20 requested against 5 present, so no 1:1 mapping exists. Plus `hashing-lab` and `pod-crossing`. **Baselined in `_tools/qa/hub-href-known-dead.txt`, not fixed** — repointing these is a content decision, not a mechanical one.

...
- ⚠ **A SEVENTH FORM IS ALSO UNCOVERED AND HAS THE SAME TRAP.** `--` at the START of a wrapped line matches none of the six. A naive `^\s*--` would fire on 15 files that use a line-leading `--` as a legitimate marker (SQL teaching content, log-format bullets such as `-- Impact observed`, `-- Contact:`).

...
- **Fix — arm-bash phase (2026-07-22, in-progress):** honesty `ok`-gate (`!(output||'').includes('lt-error')`) applied to the verified-clean tasks across arm-bash-01/02/03/07/08/09. Real-engine puppeteer harness (`scratchpad/armbash-verify2.js`) proves the LITERAL instructed commands complete and a failed command (`chmod +x` on a missing file) is blocked. Nancy PASS (2 rounds — caught + fixed: piped/redirected instructed commands were unreachable via `cmd===X`, fixed with a command-position regex; `stderr` un-gated as an error-teaching task; dead `let` branch removed). Chris + deploy pending.
- **Fix — remaining:** arm-bash-04/05/06/10 (conditionals/loops/functions/advanced) PINNED — the LinuxTerminal engine can't execute bash *language constructs* (`[`/test/if/for/function-defs/declare/trap all "command not found"), so ~16 tasks are hollow-passes that gating would make UNREACHABLE. Needs a C1(engine-build)-vs-C2(module-rewrite) design decision. Also pending: 10 arm-sql (different engine, likely result-honest, unverified) + 23 da-linux (most already outcome-gated).

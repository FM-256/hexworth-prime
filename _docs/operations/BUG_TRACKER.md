...
### BUG-119 — the Python for IT FINAL EXAM does not test 3 of the course's 9 objectives, and the course says it does  ·  [P2]  ·  open, needs a curriculum decision
...
- **Symptom:** `index.html:1912` describes the final as *"Cumulative — covers all 9 objectives, server-graded."* It does not. Its 25 questions cover **Objectives 1–6 only**: data types (3), control flow (3), strings/files (4), lists/dicts/tuples (1), functions/scope (6), Turtle graphics (3). **Zero questions** touch Obj 7 (Tkinter GUI), Obj 8 (sockets/threading) or Obj 9 (sorting/search algorithms).
...
- **The index's claim is untouched and still wrong** — deliberately left for this entry rather than quietly patched, because the honest fix is a curriculum call, not a copy edit.
- **Decision needed (operator):** either (a) **extend the exam** with questions on Tkinter, sockets/threading and sorting/search so it is genuinely cumulative — which is what "final exam" implies and what the week teaches; or (b) **accept the scope** and correct `index.html:1912` to say Objectives 1–6. Note (b) leaves the course with no summative assessment of three objectives, since the w4 quiz that might have covered GUI is exactly the artifact the operator ruled should not exist.
- **If (a):** the exam is server-graded, so `quiz_keys/pfi-w4-final-exam` must be re-seeded and `verify-quiz-keys.js pfi-w4-final-exam` must report PASSED before deploy — currently `answers=25, questionCount=25, passingScore=70`. Adding questions without re-seeding gives every student a wrong grade.

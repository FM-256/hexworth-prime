...
### BUG-118 — the hub-tree tooling exists, and BOTH halves of it are silently under-reporting  ·  [P2]  ·  open
...
- **The tool earned its keep immediately: 6 REAL student-reachable 404s, all confirmed on production, none covered by any existing gate.**
  - **4 broken meta-refresh redirects.** The source page returns 200 and bounces the student to a 404: `script/labs/linux/` → `script/labs/index.html`; `script/presentations/python/` → `script/presentations/index.html`; `shield/labs/linux/` → `shield/labs/index.html`; `shield/applets/crypto/hashing_steganography/` → `shield/applets/crypto/index.html`. **No href-based checker can see these** — the link is a `<meta http-equiv="refresh">`, which is exactly why the tree crawler found them and `hub-href-integrity-test.js` cannot.
  - **A "GUI Quiz" link** on two live Python-for-IT presentations → `pfi-w4-gui.quiz.html` (404).
  - **A post-lab `returnUrl`** on `script-clh-031.lab.html` → `script/clh/script-quiz.quiz.html` (404). The student finishes the lab and is returned to nothing.

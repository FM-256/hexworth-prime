...
- **Fix:** FIXED 2026-08-12, committed, NOT YET DEPLOYED (status stays `fixed-not-deployed` until it ships, per this tracker's own rule that RESOLVED means deployed AND verified). Three parts, in `index.html` and `reviews/cloud-openstack-comprehensive-review.html`:
  1. the review's `onComplete` now calls `ModuleProgress.complete('cloud','cloud-openstack-review')`, no accuracy threshold, matching every sibling review on the platform (`cloud-cse`, `eye-cysa`, `script-la`, `shield-cf`, all verified by Chris to record unconditionally on the engine's completion event);
...
- **Verified:** `_tools/qa/openstack-hub-completion-test.js` 48/48, which now PLAYS the review (all 25 clues, Final Jeopardy, and the "See Final Results" button, since it is `render()` from that click which reaches `showResults()` and fires `config.onComplete`). Chris PASS after two BLOCK rounds, reproducing the harness and both mutations himself and sweeping `content-registry.js`/`HubRegistry.js`/`ContentCatalog.js` for a fourth stale count (none). MUTATION TESTED: emptying `onComplete` fails 4 assertions where zero existed before; restoring either stale 13 fails the markup check.

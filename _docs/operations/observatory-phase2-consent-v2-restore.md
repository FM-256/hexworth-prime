...
1. **Re-consent is acceptable.** — **APPROVED by PI 2026-07-06, then SUPERSEDED by the grandfather policy** (see POLICY UPDATE above). Existing v1 signers are now grandfathered (not re-prompted); only new sign-ins get v2. Blast-radius recount via `functions/_count_consent_versions.js`: **17** consent records, **all on `cerbi-v1-2026-06-21`** (0 on v2). Re-verified AFTER the brief window where the strict-reprompt code (`3d919e3b3`) was live: still **17/17 on v1, 0 flipped to v2** — no participant was caught by the interim re-prompt, and every record's version is in `ACCEPTED_FORM_VERSIONS`, so all 17 grandfather correctly.

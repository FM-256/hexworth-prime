...
- [x] **G2c — Contra combat juice** (`0427b8e4a`, live-verified): operator asked to "improve the animations" → chose "combat juice." Additive VFX only (NO game-logic/score/collision). `spawnDeathBurst` at the Enemy.hit death callsite (expanding shockwave ring `shape:'shockwave'` + chunky arcing debris + base burst + small `triggerShake(4,6)`, replacing plain `spawnExplosion(...,10)`); juicier `spawnMuzzle` (aim-dir + weapon-colored core + directional sparks, was 4 dots); player-hit red EDGE vignette (transparent center radial gradient, drawn before `drawHUD` so HUD stays legible; kept existing shake, no duplicate); NEW `reduceMotion` (`prefers-reduced-motion`) gates ALL shake + the vignette (none existed before). Nancy PROCEED w/ guardrails (edit-in-place not duplicate shake; edge-vignette not full-wash; DROPPED the risky per-boss-hit shockwave; particle soft-cap). Chris pixel-verified PASS (all 5 effects visible in rendered pixels; a11y zeroes shake+flash; prior canvas-unblock + aim-up fixes intact; 0 errors; particles bounded ~57-98 in real play).
- [ ] **G4-backlog — Contra HUD stage counter** (cosmetic, Chris-flagged): "Stage X / 3" HUD doesn't update after `loadStage()` advances `currentStage` (only updates inside `advanceStage()` before the increment). Non-blocking.
- [ ] **G4-backlog — Contra escalator boss DOUBLE death-fire** (Nancy-found, pre-existing, real logic bug): the `escalator`-type boss routes through `Boss.hit()`'s `hp<=0` death branch (spawnExplosion×40, score+=2000, shake 12,40) AND the caller's `if (boss.dead)` fallback (spawnExplosion×50, score+=2000 AGAIN, shake 14,50) → double-explode + **score 4000 not 2000**. `breacher`/`hydra` set `boss.dead` directly and are fine. Fix: make the escalator path not double-run the death sequence.
- [ ] **G4-backlog — Contra particle soft-cap not a true bound** (Chris-flagged): the >280 soft-cap only trims the 6 debris particles, not the ~23-particle base burst+ring, so synthetic 25-simultaneous-kills → 1494 particles. NOT reachable in current `STAGE_ENEMIES` density (unreachable in real play today). Harden only if enemy density is ever increased.
- [ ] **G-next backlog — Don't Get Phished header overlap** (found mid-play-test, real): the page `<h1 class="visually-hidden">` renders as big black text overlapping the header because `.visually-hidden` is USED but never DEFINED in the file (no external stylesheet; repo standard `position:absolute;width:1px;height:1px;clip:rect(0,0,0,0)...` used in 77+ files). Fix: add the standard `.visually-hidden` rule. (Terminal SOC-analyst game otherwise plays end-to-end w/ clean win/lose endings + achievements + XP; speed-run immediate-win + full failure endings verified.)

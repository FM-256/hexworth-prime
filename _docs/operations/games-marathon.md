...
- [~] **G3 — shield-debugger (`shield-debugger.applet.html`)** — NEXT (no operator report yet; play-test to find issues). Then the rest of the shield arcade in order: dont-get-phished, dr-malware, exploit-flap, incident-response, life-force, malware-zoo, social-engineer, sql-injection-defense, text-adventure-hydra, threat-runner, threat-swarm, tor-darkweb, web-security-headers-lab.
- [x] **G2 — Contra** SHIPPED (commit `804c431d7`, live-verified). Hitbox grace so forward fire hits ground enemies.

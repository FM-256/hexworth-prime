...
| Capacity strip | Static honest limits (2 containers/student, 120 min lifetime, ~30 concurrent platform-wide) + note that graded work has priority. Live-availability display deferred (lab-manager API exposes no public count today; do not fake one) |
| Catalog + Explore All | Follows automatically from the registry entry via existing projections. Catalog dynamic-hub gap (#243) is unrelated: the-rig is a STATIC entry |

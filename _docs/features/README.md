...
| [Operator Missions](OPERATOR_MISSIONS.md) | OperatorEngine.js, AgentBridge.js, OperatorInterpreter.js | 72-mission Metroidvania: 50 Python levels, obstacles, permanent tools, persistent inventory |
...
| [Incubation Hubs](INCUBATION_HUBS.md) | `houses/<h>/incubator/index.html` × 8, `INCUBATOR_MODULES[]` pattern, graduation README | Per-house parking lot for orphan modules without a curriculum yet — promote when ≥10 modules cluster on a clear topic. Tooling lives in `_tools/eduscan/` (post-scan placement pipeline) |

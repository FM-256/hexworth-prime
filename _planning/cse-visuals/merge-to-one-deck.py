...
/* force=true is used ONLY to answer a hello. The reply must go out even while the deck is
   applying a received index, or the answer to "where are you?" is swallowed by the echo
   guard and the presenter never learns the truth. */
...
function receive(msg) {{
    ...
    /* HELLO HANDSHAKE -- handled BEFORE the numeric check, which would otherwise drop it.
       Carrying ?i= tells the presenter where the deck was AT THE MOMENT P WAS PRESSED, and
       that is all it can do: a presenter reached any other way -- a bookmark, a hand-typed
       URL, a reload that loses the query string -- had no index and sat on slide 1 until
       the next advance. Chris reproduced exactly that on the WSA decks; this deck had the
       same weakness, so the URL is now a hint and the deck is the source of truth. */
    if (msg.hello) {{
        if (!IS_PV) broadcast(idx, true);        // only the projected deck answers
        return;
    }}
...
/* Ask the deck where it is. Sent by the presenter on load, after the listeners are wired. */
function sayHello() {{
    const msg = {{ hello: true, from: IS_PV ? 'pv' : 'deck', t: Date.now() }};
    if (bc) {{ try {{ bc.postMessage(msg); }} catch (e) {{}} }}
    try {{ localStorage.setItem(CHAN, JSON.stringify(msg)); }} catch (e) {{}}
}}
...
(function start() {{
    ...
    if (IS_PV) {{
        echo = true; show(at); echo = false;
        /* ...then ASK, unconditionally. ?i= is only a hint -- absent on a bookmark or a
           hand-typed presenter link, stale after a reload. Sent here, at the end of start(),
           because bc.onmessage and the storage listener are already wired above; sending
           before that would post the question with nothing listening for the answer. */
        sayHello();
    }}
    else {{ show(at); }}
}})();
